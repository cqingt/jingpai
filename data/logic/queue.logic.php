...
class queueLogic {
    ...
    public function editGroupbuySaleCount($groupbuy_info) {
        ...
        if (!$update) {
            return callback(false,'更新藏品惠信息失败groupbuy_id:'.$groupbuy_info['groupbuy_id']);
        } else {
            return callback(true);
        }
    }
     ...
     * @throws Exception
     */
    public function editMiaoshaSaleCount($miaosha_info) {
        $model_miaosha = Model('miaosha');
        $data = array();
        $data['buyer_count'] = array('exp','buyer_count+1');
        $data['buy_quantity'] = array('exp','buy_quantity+'.$miaosha_info['quantity']);
        $update = $model_miaosha->editMiaosha($data,array('miaosha_id'=>$miaosha_info['miaosha_id']));
        if (!$update) {
            return callback(false,'更新秒杀信息失败miaosha_id:'.$miaosha_info['miaosha_id']);
        } else {
            return callback(true);
        }
    }
    ...
    public function editVipsaleSaleCount($vipsale_info) {
        $model_vipsale = Model('vipsale');
        $data = array();
        $data['buyer_count'] = array('exp','buyer_count+1');
        $data['buy_quantity'] = array('exp','buy_quantity+'.$vipsale_info['quantity']);
        $update = $model_vipsale->editMiaosha($data,array('vipsale_id'=>$vipsale_info['vipsale_id']));
        if (!$update) {
            return callback(false,'更新会员特价信息失败vipsale_id:'.$vipsale_info['vipsale_id']);
        } else {
            return callback(true);
        }
    }
	...
	// add 优惠推荐
    public function editYouHuiTuiJianSaleCount($tuijian_info) {
        $pushuser_gift = Model('pushuser_gift');
        $data = array();
        $data['buyer_count'] = array('exp','buyer_count+1');
        $data['buy_quantity'] = array('exp','buy_quantity+'.$tuijian_info['quantity']);
        $update = $pushuser_gift->editCangdouTuiJian($data,array('id'=>$tuijian_info['youhuituijian_id']));
        if (!$update) {
            return callback(false,'更新会员特价信息失败youhuituijian_id:'.$tuijian_info['youhuituijian_id']);
        } else {
            return callback(true);
        }
    }
    ...
    public function editVoucherState($voucher_list) {
        ...
        foreach ($voucher_list as $store_id => $voucher_info) {
            ...
            if ($update) {
                ...
                $param['voucher_url'] = urlShop('member_voucher', 'voucher_list');
                $send->send($param);
            } else {
     ...
     * @throws Exception
     */
    public function editPingtaiVoucherState($pingtai_voucher) {
        $model = Model('store_voucher');
        $send = new sendMemberMsg();
        $update = $model->editVoucher(array('voucher_id'=>$pingtai_voucher['voucher_id'],'voucher_owner_id'=>$pingtai_voucher['voucher_owner_id']),array('voucher_state'=>2));
        if ($update) {
            // 发送用户店铺消息
            $send->set('member_id', $pingtai_voucher['voucher_owner_id']);
            $send->set('code', 'voucher_use');
            $param = array();
            $param['voucher_code'] = $pingtai_voucher['voucher_code'];
            $param['voucher_url'] = urlShop('member_voucher', 'voucher_shop_list');
            $send->send($param);
        } else {
            return callback(false,'更新平台优惠券状态失败vcode:'.$pingtai_voucher['voucher_code']);
        }
        return callback(true);
    }
    ...
    public function sendMemberMsg($param) {
        ...
        $send->set('param_message', $param['message']);
    ...
    /**
     * 发送微信模板消息 add xin 20160314
     */
    public function sendWXTemplateMsg($param) {
        $tmp = new WeixinTemplateMsg();
        $tmp->$param['func']($param);
        return callback(true);
    }
