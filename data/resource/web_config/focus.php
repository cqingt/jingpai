...
<script type="text/javascript" src="http://resource.96567.com/js/NewLunbotu.js"></script>

<div class="home-lbout">
		<ul class="home-lbimg">
		 <?php if (is_array($output['code_screen_list']['code_info']) && !empty($output['code_screen_list']['code_info'])) { ?>
          ...
          <?php if (is_array($val) && $val['ap_id'] > 0) { ?>
			<li ap_id="<?php echo $val['ap_id'];?>">
			  <a href="<?php echo $val['pic_url'];?>" target="_blank" color="<?php echo $val['color'];?>" style="background: <?php echo $val['color'];?> url(<?php echo UPLOAD_SITE_URL.'/'.$val['pic_img'];?>) center top no-repeat" name="index_focus<?php echo $key;?>"></a>
			</li>
			<?php }else { ?>
			<li>
			  <a href="<?php echo $val['pic_url'];?>" target="_blank" color="<?php echo $val['color'];?>" style="background: <?php echo $val['color'];?> url(<?php echo UPLOAD_SITE_URL.'/'.$val['pic_img'];?>) center top no-repeat" name="index_focus<?php echo $key;?>"></a>
