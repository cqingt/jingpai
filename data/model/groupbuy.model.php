...
class groupbuyModel extends Model{
    ...
    /**
	 * 根据藏品惠编号读取藏品惠信息
	 * @param array $groupbuy_id 藏品惠活动编号
    ...
    /**
     * 根据商品编号查询是否有可用藏品惠活动，如果有返回藏品惠信息，没有返回null
     * @param int $goods_id
     * @return array $groupbuy_info
     *
     */
    public function getGroupbuyInfoByGoodsID($goods_id) {
        $info = $this->_rGoodsGroupbuyCache($goods_id);
        if (empty($info)) {
            $condition = array();
            $condition['state'] = self::GROUPBUY_STATE_NORMAL;
            $condition['end_time'] = array('gt', TIMESTAMP);
            ...
            $this->_wGoodsGroupbuyCache($goods_id, $info);
        }
    ...
    /**
     * 根据商品编号查询是否有可用藏品惠活动，如果有返回藏品惠活动，没有返回null
     * @param string $goods_string 商品编号字符串，例：'1,22,33'
     * @return array $groupbuy_list
     *
     */
    public function getGroupbuyListByGoodsIDString($goods_id_string) {
        $groupbuy_list = $this->_getGroupbuyListByGoodsCommon($goods_id_string);
        $groupbuy_list = array_under_reset($groupbuy_list, 'goods_id');
        return $groupbuy_list;
    }
     ...
     *
     */
    private function _getGroupbuyListByGoodsCommon($goods_id_string) {
        $condition = array();
        $condition['state'] = self::GROUPBUY_STATE_NORMAL;
        $condition['start_time'] = array('lt', TIMESTAMP);
        $condition['end_time'] = array('gt', TIMESTAMP);
    ...
    public function addGroupbuy($param){
        ...
        //add 发布藏品惠锁定商品SKU及商品  xin 20151106
        ...
        if ($result) {
            // 更新商品藏品惠缓存
            $this->_dGoodsGroupbuyCache($param['goods_id']);
            return $result;
        } else {
            return false;
        }
    ...
    private function _lockGoods($goods_id) {
        $condition = array();
        $condition['goods_id'] = $goods_id;
    ...
    private function _unlockGoods($goods_id) {
        $model_goods = Model('goods');
        $model_goods->editGoodsCommonUnlock(array('goods_id' => $goods_id));
        // 添加对列 更新商品促销价格
        QueueClient::push('updateGoodsPromotionPriceByGoodsCommonId', $goods_id);
    }
    */
    ...
    public function editGroupbuy($update, $condition) {
        $groupbuy_list = $this->getGroupbuyList($condition, null, '', 'goods_id');
        $result = $this->where($condition)->update($update);
        if ($result) {
            if (!empty($groupbuy_list)) {
                foreach ($groupbuy_list as $val) {
                    // 更新商品藏品惠缓存
                    $this->_dGoodsGroupbuyCache($val['goods_id']);
                }
            }
        }
        return $result;
    ...
    public function reviewFailGroupbuy($groupbuy_id) {
        ...
        $return =  $this->editGroupbuy($update, $condition);
        if ($return) {
            Model('goods')->editGoodsSkuUnlock($groupbuy_info['goods_id']);
        }
        return $return;
    }
     ...
     *
     */
    public function closeGroupbuy($groupbuy_id) {
        // 商品解锁
        $groupbuy_info = $this->getGroupbuyInfoByID($groupbuy_id);
    ...
    public function editExpireGroupbuy($condition) {
        ...
        if (!empty($expire_groupbuy_list)) {
            ...
            foreach ($expire_groupbuy_list as $val) {
                $goodscommonid_array[] = $val['goods_id'];
            }
            // 更新商品促销价格，需要考虑藏品惠是否在进行中
        ...
        if($groupbuy_id_string != '') {
            ...
            if ($result) {
                foreach ($expire_groupbuy_list as $value) {
                    Model('goods')->editGoodsSkuUnLock($value['goods_id']);
                }
            }
        }
        return true;
    ...
    public function delGroupbuy($condition){
        ...
        if(!empty($groupbuy_list) && $result) {
            foreach ($groupbuy_list as $value) {
                ...
                Model('goods')->editGoodsSkuUnlock($value['goods_id']);
                // 更新商品藏品惠缓存
                $this->_dGoodsGroupbuyCache($value['goods_id']);
    ...
    public function getGroupbuyExtendInfo($groupbuy_info) {
		...
		$goods_info = Model('goods')->getGoodsInfo(array('goods_id'=>$groupbuy_info['goods_id']),'goods_storage');
		$groupbuy_info['goods_storage'] = $goods_info['goods_storage'];
        ...
        switch ($groupbuy_info['state']) {
            ...
            case self::GROUPBUY_STATE_NORMAL:
                if($groupbuy_info['start_time'] > TIMESTAMP) {
                    ...
                    $groupbuy_info['count_down_text'] = '距藏品惠开始';
                    ...
                    $groupbuy_info['button_text'] = '立即抢购';
					//$groupbuy_info['buynow_submit'] = 'nctype="buynow_submit"';
                    $groupbuy_info['count_down_text'] = '距藏品惠结束';
     ...
     * @return array/bool
     */
    private function _rGoodsGroupbuyCache($goods_id) {
        return rcache($goods_id, 'goods_groupbuy');
    }
     ...
     * @return boolean
     */
    private function _wGoodsGroupbuyCache($goods_id, $info) {
        return wcache($goods_id, $info, 'goods_groupbuy');
    }
     ...
     * @return boolean
     */
    private function _dGoodsGroupbuyCache($goods_id) {
        return dcache($goods_id, 'goods_groupbuy');
    }
    ...
    protected function getCachingDataByQuery($key) {
        ...
        case 'groupbuy_vr_classes': // 虚拟藏品惠分类
        ...
        case 'groupbuy_vr_cities': // 虚拟藏品惠地区
            // 一级地区 城市
