...
class storeModel extends Model {
     ...
     * @return array
	 */
    public function getStoreMemberIDList($storeid_array, $field = 'store_id,member_id,store_domain,is_own_shop') {
...
/**
    ...
    private function getGoodsListBySales($store_array) {
    	...
    	foreach ($store_array as $value) {
    		 $search_list_goods = $model->table('goods')->field($field)->where(array('store_id'=>$value['store_id'],'goods_state'=>1))->group('goods_commonid')->order('goods_salenum desc')->limit(8)->select();
			 if($search_list_goods){
				foreach($search_list_goods as $k=>$v){
					//获取促销信息
					$goods_info = Model('goods')->getChuXiao($v['goods_id']);
					if($goods_info['promotion_type'] && $goods_info['promotion_price']){
						$search_list_goods[$k]['promotion_type'] = $goods_info['promotion_type'];
						$search_list_goods[$k]['promotion_price'] = $goods_info['promotion_price'];
					}

				}
			 }
			 $store_array[$value['store_id']]['search_list_goods'] = $search_list_goods;
    	}
    	return $store_array;
    }
}
