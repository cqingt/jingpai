...
class p_bundlingModel extends Model {
     ...
     * @return boolean
     */
    public function addBundlingGoodsAll($insert,$bl_state) {
        $result = $this->table('p_bundling_goods')->insertAll($insert);
        if ($result) {
            foreach ((array)$insert as $v) {
                if (isset($v['goods_id'])){
                    //add xin 20151107
                    if($bl_state == 1){ //如果优惠套装开启，锁定商品，关闭解锁商品
                        //锁定商品
                        Model('goods')->editGoodsSkuLock($v['goods_id']);
                    }else{
                        //解锁商品
                        Model('goods')->editGoodsSkuUnlock($v['goods_id']);
                    }
                    //add end
    ...
    public function delBundlingGoods($condition) {
        ...
        foreach($list as $val){
            //解锁商品
            Model('goods')->editGoodsSkuUnlock($val['goods_id']);
        }
        //add end
