...
class goods_classModel extends Model
{
    ...
    public function getGoodsClassListByParentId($pid) {
        $data = $this->getCache();
        $ret = array();
        $admin = unserialize(decrypt(cookie('sys_key'),MD5_KEY));//判定是否管理员 add xin 20151109

        foreach ((array) $data['children'][$pid] as $i) {
                //add 非自营店铺隐藏未开启的分类 xin 20151106
                if ($data['data'][$i]) {
                    if(!is_array($admin) || !isset($admin['sp']) || (empty($admin['name']) || empty($admin['id']))) {
                        if(!checkPlatformStore()){
                            if($data['data'][$i]['gc_isopen'] == 1){
                                $ret[] = $data['data'][$i];
                            }
                        }else{
                            $ret[] = $data['data'][$i];
	...
	/************修改注释掉原有的分类关键词调用方法 2016-08-24 杜**************/
    /**
     * 取得分类关键词，方便SEO
     */
...
//    }
/************修改成分类关键词和描述只掉用当前分类下的不调用上级分类的了 2016-08-24 杜**************/
    public function getKeyWords($gc_id = null){
        if (empty($gc_id)) return false;
        $keywrods = rkcache('goods_class_seo',true);
        $seo_title = $keywrods[$gc_id]['title'];
        ...
        return array(1=>$seo_title,2=>$seo_key,3=>$seo_desc);
    }
/************修改结束 2016-08-24 杜**************/
    /**
     * 获得商品分类缓存
     ...
     * 递归模式获取指定分类的上级所有分类
     */
    public function getCateArr($cate_id){
        global $cid;
        $dataArr = $this->table('goods_class')->field("gc_id,gc_name,gc_parent_id")->where(array("gc_id"=>$cate_id))->select();
        $cid[] = $dataArr[0]['gc_id'];
        if($dataArr[0]['gc_parent_id']){
            $this->getCateArr($dataArr[0]['gc_parent_id']);
        }
        return $cid;
    }
     ...
     * 通过一级分类获取二级全部分类
     */
    public function getCateDownArr($cate_id){
        $dataArr = $this->table('goods_class')->field("gc_id")->where(array("gc_parent_id"=>$cate_id))->select();
        return $dataArr;
    }
     ...
     *
     */
    public function getGoodsClassYiJi($id = 0) {
        if (intval($id)> 0) {
            $gc_line = array();
            $class = $this->getGoodsClassInfoById(intval($id));
            $gc_line['gc_id'] = $class['gc_id'];
