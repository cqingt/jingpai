...
class voucherModel extends Model {
	...
	public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0) {
	   ...
	   $condition['voucher_state'] = 1;
        ...
        /*2015-11-10 Save in name LT */
        $voucher_list = $this->table('voucher,voucher_template')->field('voucher.*,voucher_template.voucher_t_cate_rule,voucher_template.voucher_t_store_cates,voucher_template.voucher_t_diff_goods')->join('left')->on('voucher.voucher_t_id=voucher_template.voucher_t_id')->where($condition)->key('voucher_t_id')->select();
        /*End*/
    ...
    /*2015-11-10 Add is name LT 判断可用代金卷*/
    public function is_not_dai($goods_array,$dai_array){
        // var_dump($dai_array);
        // var_dump($goods_array);
        $money_array = array();
        foreach($dai_array as $k => &$v){//店铺代金卷
            foreach($v as $dk => $dv){//当前代金卷
                $money = '';
                foreach($goods_array[$k] as $goodsk => $goodsv){//店铺产品
                    $goods_info = Model('goods')->getGoodsInfoByID($goodsv['goods_id']);
                    $money += $this->is_money($dv,$goods_info,$goodsv['goods_price'])*$goodsv['goods_num'];
                }
                if($money < $dv['voucher_limit']){
                    unset($v[$dk]);
                }else{
                    $dv['desc'] = sprintf('面额%s元 有效期至 %s',$voucher['voucher_price'],date('Y-m-d',$voucher['voucher_end_date']));
                }
            }
        }
        return $dai_array;
    ...
    /*2015-11-10 Add is name LT 判断规则增加金钱*/
    private function is_money($daiinfo,$goodsinfo,$money){
        // var_dump($daiinfo);
        // var_dump($goodsinfo);
        $daiclass = explode(',',$daiinfo['voucher_t_store_cates']);//指定分类
        $daigoods = explode(',',$daiinfo['voucher_t_diff_goods']);//被排除的商品ID
        $goodsclass = explode(',',$goodsinfo['goods_stcids']);//商品分类
        // var_dump($goods_class);
        $goodsid = $goodsinfo['goods_id'];
        if($daiinfo['voucher_t_cate_rule']==1){//全部分类,排除单个商品
            if(in_array($goodsid, $daigoods)){
                return '';
            }else{
                return $money;
            }
        }elseif($daiinfo['voucher_t_cate_rule'] == 2){//单独分类，另排商品
            $is_money = false;
            foreach($goodsclass as $k => $v){//检测分类
                if(in_array($v,$daiclass) && !in_array($goodsid, $daigoods)){
                    $is_money = true;
                    break;
                }
            }
            return $is_money?$money:'';
        }elseif($daiinfo['voucher_t_cate_rule'] == 3){//指定产品
            if(in_array($goodsid, $daigoods)){
                return $money;
            }else{
                return '';
    ...
    //     // var_dump($goodsinfo);
    //     $daiclass = explode(',',$daiinfo['voucher_t_store_cates']);//指定分类
    //     $goodsclass = explode(',',$daiinfo['goods_commend']);//指定分类
    //     $daigoodsclass = explode(',',$daiinfo['voucher_t_diff_goods']);//被排除的商品ID
    //     // var_dump($goods_class);
    //     $goodsid = $goodsinfo['goods_id'];
    //     if($daiinfo['voucher_t_cate_rule']==1){//全部分类,排除单个商品
    //         if(in_array($goodsid, $daiclass)){
    //             return '';
    //         }else{
    //             return $money;
    //         }
    ...
    //             if(!in_array($v,$daiclass) && !in_array($goodsid, $daiclass)){
    //                 return $money;
    //             }else{
    //                 return ''; 
    //             }
	...
	public function getCanChangeTemplateInfo($vid,$member_id,$store_id = 0){
        ...
        /* Add is name lt 2016-04-27 如有参数则不需要积分 */

        if(empty($_SESSION['shoudong_voucher'])){
        ...
        $_SESSION['shoudong_voucher'] = false;
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null) {
		...
		$field = 'voucher_id,voucher_code,voucher_title,voucher_desc,voucher_start_date,voucher_end_date,voucher_price,voucher_limit,voucher_state,voucher_order_id,voucher_store_id,store_name,store_id,store_domain,voucher_t_customimg,voucher_t_cate_rule,voucher_t_store_cates,voucher_t_diff_goods';
     ...
     * 积分兑换代金券
     */
    public function exchangeVoucher($template_info, $member_id, $member_name = '',$delPoints=''){
    ...
    /**
     * 手动添加代金卷、不能重复领取
     * 2015-11-26 Add is name lt 
     * $uid     用户ID
     * $vid     代金卷模板ID
     */
    public function addDaiJingJuan($uid,$vid,$type=false){
        if($type == false){
        $uservoucher = $this->table('voucher')->where(array('voucher_owner_id'=>$uid,'voucher_t_id'=>$vid))->find();
        }
        if(empty($uservoucher)){
            ...
            /*添加代金卷*/
            $result = $this->exchangeVoucher($data['0'],$userinfo['member_id'],$userinfo['member_name'],true);
            return $result;
        }else{
            $result['state']=false;
            $result['msg']='已兑换过';
