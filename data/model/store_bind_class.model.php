    ...
    public function getStoreGcidCommisRateList($goods_list) {
        ...
        foreach ($goods_list as $goods) {
            ...
            if (!in_array($goods['gc_id'],(array)$store_gc_id_list[$goods['store_id']])) {
				...
				//zmrcui>>>>>
				$store_gc_id_list[$goods['store_id']]['gc_id_3'][] = $goods['gc_id'];
				$goods_gc_id = $goods_model->getGoodsInfoByID($goods['goods_id'] , 'gc_id_1,gc_id_2');
				$store_gc_id_list[$goods['store_id']]['gc_id_2'][] = $goods_gc_id['gc_id_2'];
				$store_gc_id_list[$goods['store_id']]['gc_id_1'][] = $goods_gc_id['gc_id_1'];
				//zmrcui<<<<<
            }
        }
        ...
        foreach ($store_gc_id_list as $store_id => $gc_id_list) {
            ...
            $condition['class_1|class_2|class_3'] = array('in',$gc_id_list['gc_id_3']);
			...
			if(!$bind_list)
			{
				...
				$condition['store_id'] = $store_id;
				//底级分类查不到查二级
				$condition['class_1|class_2'] = array('in',$gc_id_list['gc_id_2']);
				$bind_list = $this->getStoreBindClassList($condition);
				if(!$bind_list)
				{
					$condition = array();
					$condition['store_id'] = $store_id;
					//二级分类查不到查一级
					$condition['class_1'] = array('in',$gc_id_list['gc_id_1']);
					$bind_list = $this->getStoreBindClassList($condition);
				}
			}
			//zmr>>>
			...
			if(!$bind_list)
			{
				if (in_array($store_id, $own_shop_ids)) {
					$bind_list = array();
					foreach ($gc_id_list['gc_id_3'] as $k=>$gc_id) {
						$goods_class = Model('goods_class')->getGoodsClassInfoById($gc_id);
						$bind_list[$k]['store_id'] = $store_id;
						$bind_list[$k]['commis_rate'] = $goods_class['commis_rate'];
						$bind_list[$k]['class_1'] = 0;
						$bind_list[$k]['class_2'] = 0;
						$bind_list[$k]['class_3'] = 0;
						$bind_list[$k]['state'] = 1;
					}
                }else{
					$condition = array();
            ...
            if (!empty($bind_list) && is_array($bind_list)) {
                foreach ($bind_list as $bind_info) {
					...
					if($is_all)
					{
						$bind_class =$gc_id_list['gc_id_3'];
					}
                    foreach ($gc_id_list['gc_id_3'] as $gc_id) {
