...
class activity_detailModel{
	...
	public function getGoodsList($condition,$page=''){
		...
		$param['table']	= 'activity_detail,goods';
		$param['join_type']	= 'inner join';
		$param['field']	= 'activity_detail.activity_detail_sort,activity_detail.cat_name,goods.goods_id,goods.store_id,goods.goods_name,goods.goods_price,goods.goods_marketprice,goods.goods_promotion_price,goods.goods_image,goods.goods_click,goods.is_appoint,goods.is_presell';
		$param['join_on']	= array('activity_detail.item_id=goods.goods_id');
		$param['where']	= $this->getCondition($condition);
		$param['order']	= $condition['order'];
		return Db::select($param,$page);
