...
class memberModel extends Model {
    ...
    public function getMemberInfoByID($member_id, $fields = '*') {
        ...
        if (empty($member_info)) {
            ...
            /*2015-11-09 Add is name Lt 个人手机号显示解密*/
            if($member_info['member_mobile'] != ''){
                $member_info['member_mobile'] = JieMiMobile($member_info['member_mobile']);
            }
            /*End*/
    ...
    public function editMember($condition, $data) {
    	...
    	/*2015-11-09 Add is name Lt 手机号插入时加密*/
    	if($data['member_mobile']){
    		$data['member_mobile'] = JiaMiMobile($data['member_mobile']);
    	}
    	/*End*/
    ...
    public function createSession($member_info = array(),$reg = false) {
		...
		if($member_info['openid']){
			/*2015-11-25 Add is name Lt 加入openid字段*/
			$_SESSION['openid'] 	= $member_info['openid'];
			/*End*/
		}

        //add 增加会员等级 xin 20151201
        $member_gradeinfo = $this->getOneMemberGrade(intval($member_info['member_exppoints']));
        $_SESSION['level'] 	= $member_gradeinfo['level'];
     ...
     * 根据openid查找推荐人id
     */
	public function getUpdateTuiJianRen($condition,$condition1){
		$field='member_id';
		$tjr_info = $this->table('member')->field($field)->where($condition)->find();
        if(!empty($tjr_info)){
            $this->table('member')->where($condition1)->update(array('inviter_id'=>$tjr_info['member_id']));
        }
		return true;
	}
    ...
    public function register($register_info) {
		...
		array("input"=>$register_info["username"],		"require"=>"true",	"validator"=>"username","message"=>'用户名仅限中文、英文字母、数字组成！'),
        ...
        */
        ...
        /*验证openid是否重复*/
        if($register_info['openid']){
            $check_member_openid = $this->getMemberInfo(array('openid'=>$register_info['openid']));
            if(is_array($check_member_openid) and count($check_member_openid)>0) {
                $this->editMember(array('openid'=>$register_info['openid']),array('openid'=>''));
            }
        }
        /*End*/
        ...
        // 增加用户注册来源 xin 20151119
        $member_info['member_from']	= $register_info['member_from'];
		...
		if($zmr>0)
		{
			$pinfo=$this->getMemberInfoByID($zmr,'member_id');
			if(empty($pinfo))
			{
				$zmr=0;
			}
		...
		if($insert_id) {
			if($register_info['iscangdou'] != 'no'){
				//注册成功赠送上级藏豆
				Model('pushuser_gift')->setUserPoints($insert_id);
			}
            ...
            $member_info = $this->getMemberInfo(array('member_id'=>$insert_id));
	...
	public function addMember($param) {
		...
		try {
            ...
            $member_info['member_mobile']		= JiaMiMobile($param['member_mobile']);
            ...
            $member_info['member_from']	= $param['member_from'];
			...
			//记录推广参数
			if($_SESSION['tg_from']){
				$member_info['tg_from']	= $_SESSION['tg_from'];
			}
