...
class orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        ...
        $order_info['money_paid'] = Model('crm')->getPaidMoneyByOrderId($order_info['order_id']);//获取订单已付款字段 add xin 20151117
        ...
        if (in_array('order_common',$extend)) {
            ...
            $order_info['extend_order_common_ser'] = $order_info['extend_order_common'];

			$str = iconv("UTF-8","GB2312",$order_info['extend_order_common']['reciver_info']); //收货人信息反序列化
			$str = @preg_replace('!s:(\d+):"(.*?)";!se',"'s:'.strlen('$2').':\"$2\";'",$str);
			$reciver_info = unserialize($str);
			if(!$reciver_info){
				$str = $order_info['extend_order_common']['reciver_info']; //收货人信息反序列化
				$str = @preg_replace('!s:(\d+):"(.*?)";!se',"'s:'.strlen('$2').':\"$2\";'",$str);
			}

            $order_info['extend_order_common']['reciver_info'] = unserialize($str);

			//盛威添加 2015-11-11
			$order_info['extend_order_common']['reciver_info']['mob_phone'] = JieMiMobile($order_info['extend_order_common']['reciver_info']['mob_phone']);
			$order_info['extend_order_common']['reciver_info']['phone'] = JieMiMobile($order_info['extend_order_common']['reciver_info']['phone']);
     ...
     *
     */
    public function getOrderSnList($condition, $filed = 'order_sn') {
        $OrderSn = $this->table('order')->field($filed)->where($condition)->select();
		$order_sn = array();
		if($OrderSn){
			foreach($OrderSn as $k=>$v){
				$order_sn[] = $v['order_sn'];
			}
			$strordersn = implode(',',$order_sn);
		}
		return $strordersn;
    }
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(), $master = false){
        ...
        foreach ($list as $order) {
            ...
            $order['money_paid'] = Model('crm')->getPaidMoneyByOrderId($order['order_id']);//获取订单已付款字段 add xin 20151117
        ...
        if (in_array('order_common',$extend)) {
            ...
            foreach ($order_common_list as $value) {
				...
				$str = iconv("UTF-8","GB2312",$value['reciver_info']); //收货人信息反序列化
				$str = @preg_replace('!s:(\d+):"(.*?)";!se',"'s:'.strlen('$2').':\"$2\";'",$str);
				$reciver_info = unserialize($str);
				if(!$reciver_info){
					$str = $value['reciver_info']; //收货人信息反序列化
					$str = @preg_replace('!s:(\d+):"(.*?)";!se',"'s:'.strlen('$2').':\"$2\";'",$str);
				}else{
					$dataArr[$k]['address'] = iconv("UTF-8","GB2312",$reciver_info['address']);
				}

				$order_list[$value['order_id']]['extend_order_common']['reciver_info'] = @unserialize($str);
				$order_list[$value['order_id']]['extend_order_common']['invoice_info'] = @unserialize($str);
				$express = Model('express')->getExpressInfo($value['shipping_express_id']);
				$order_list[$value['order_id']]['extend_order_common']['e_name'] = $express['e_name'];
	...
	public function addOrderLog($data) {
		...
		if(intval($data['order_id']) > 0){
			//查询订单操作状态 并记录
			$yw_info = $this->table('yw_info')->field('order_status,shipping_status,pay_status')->where(array('orderid'=> $data['order_id']))->find();
            if(!empty($yw_info)){
                $data['order_status'] = $yw_info['order_status'];
                $data['shipping_status'] = $yw_info['shipping_status'];
                $data['pay_status'] = $yw_info['pay_status'];
            }
		}
    ...
    public function getOrderOperateState($operate,$order_info){
        ...
        switch ($operate) {
           ...
           case 'system_receive_pay':
               $state = ($order_info['order_state'] == ORDER_STATE_NEW && $order_info['payment_code'] == 'online') || ($order_info['order_state'] == ORDER_STATE_NEW && $order_info['payment_code'] == 'bank');
               break;
    ...
    public function getOrderAndOrderGoodsList($condition, $field = '*', $page = 0, $order = 'rec_id desc') {
        ...
        if(array_key_exists('order.zrm_id',$condition)){
            if(empty($condition['order.zrm_id'])){
                unset($condition['order.zrm_id']);
            }
        }

        return $this->table('order_goods,order')->join('left')->on('order_goods.order_id=order.order_id')->where($condition)->field($field)->page($page)->order($order)->select();
    }
     ...
     * 插入业务订单表
     */
    public function ywInfoInsert($data){
        return $this->table('yw_info')->insert($data);
    }
     ...
     *
     */
    public function ywInfoUpdate($data,$condition) {
        return $this->table('yw_info')->where($condition)->update($data);
    }
    //add end
	
	/**
	 *add 2016-10-21
     *du add根据产品id获取产品销售数量
     */
    public function getOrderGoodsSum($goods_id) {
        return $this->table('order_goods')->where(array("goods_id"=>$goods_id))->sum("goods_num");
    }
}
