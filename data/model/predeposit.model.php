    ...
    public function changePd($change_type,$data = array()) {
        ...
        switch ($change_type){
            ...
            case 'cash_del':
				...
				case 'sys_add_money':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【增加】，备注：'.$memo.'，充值单号: '.$data['pdr_sn'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
				...
				case 'sys_del_money':
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【减少】，备注：'.$memo.'，充值单号: '.$data['pdr_sn'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                ...
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_freeze_amount'] = $data['amount'];
				$data_log['lg_desc'] = '管理员调节预存款【冻结】，备注：'.$memo.'，充值单号: '.$data['pdr_sn'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit+'.$data['amount']);
                ...
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【解冻】，备注：'.$memo.'，充值单号: '.$data['pdr_sn'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
        ...
        /* Add is name lt 2016-04-07 发送模板消息给用户 */

        $member_info = Model('member')->getMemberInfoByID($data['member_id']);
            $dataArr['first'] = $member_info['member_name'].'您好,您的账户余额发生变动,信息如下';
            $dataArr['keyword1'] = '无';
            $dataArr['keyword2'] = '无';
            $dataArr['keyword3'] = $data_log['lg_desc'];
            $dataArr['keyword4'] = $data_log['lg_av_amount'];
            $dataArr['keyword5'] = $member_info['available_predeposit'];
            $dataArr['remark'] = '';
