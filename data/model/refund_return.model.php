...
class refund_returnModel extends Model{
	...
	public function editOrderRefund($refund) {
		...
		if ($refund_id > 0) {
    	    ...
    	    try {
				...
				$ShengYuAmount = $predeposit_amount-$refund['refund_amount'];
				//2016-06-07 修改部分退款不修改订单状态未取消状态
				if($ShengYuAmount > 0 && $state && $order_state >= $order_paid){ //部分款
					$log_msg = '部分产品退款完成';
				}else{
					if ($state && $order_state == 20) { //只有代收货订单全部退款完成时取消订单
				...
				if ($state && $order['order_state'] == 40) {
					//减去用户藏豆
						Model('pushuser_gift')->setUserJianQuPoints($order['buyer_id'],$order_id,$refund['refund_amount']);
					//减去积分
					if (C('points_isuse') == 1){
						Model('points')->savePointsLog('return_order',array('pl_memberid'=>$order['buyer_id'],'pl_membername'=>$order['buyer_name'],'orderprice'=>$refund['refund_amount'],'order_sn'=>$order['order_sn'],'order_id'=>$order['order_id']),true);
						
					}
					//减去会员经验值
						Model('exppoints')->saveExppointsLog('return_order',array('exp_memberid'=>$order['buyer_id'],'exp_membername'=>$order['buyer_name'],'orderprice'=>$refund['refund_amount'],'order_sn'=>$order['order_sn'],'order_id'=>$order['order_id']),true);
					//检查是否评价评价删除商品评价信息
						if(intval($refund['order_goods_id']) <= 0){
							Model('evaluate_goods')->delEvaluateGoods(array('geval_orderid'=>$order['order_id'],'geval_frommemberid'=>$refund['buyer_id']));
						}else{
							Model('evaluate_goods')->delEvaluateGoods(array('geval_orderid'=>$order['order_id'],'geval_ordergoodsid'=>$refund['order_goods_id'],'geval_frommemberid'=>$refund['buyer_id']));
							dcache($refund['order_goods_id'], 'goods_evaluation');
						}
				}
	 ...
	 *
	 */
	public function getmobileRefundStateArray($type = 'all') {
		Language::read('refund');
		$state_array = array(
			'1' => "待审核",
			'2' => "同意",
	 ...
	 * @return array
	 */
	public function getRefund_ReturnList($condition = array(), $page = '') {
	    //$condition['refund_type'] = '2';//类型:1为退款,2为退货
		$result = $this->getRefundReturnList($condition, $page);
		return $result;
	}
	 ...
	 * @return int
	 */
	public function addReturnLog($log_info,$U_UserName,$RID,$type) {
	    $Arr=array(
				'R_ID'=>$RID,
				'content'=>$log_info,
		...
		$refund_id = $this->table('refund_return_log')->insert($Arr);
		return $refund_id;
	}
}
