...
class pointsModel {
	 ...
	 * @param  string $stage 操作阶段 regist(注册),login(登录),comments(评论),order(下单),system(系统),zhuanti(活动),other(其他),pointorder(积分礼品兑换),app(同步积分兑换)
	...
	function savePointsLog($stage,$insertarr,$if_repeat = true){
		...
		switch ($stage){
			...
			case 'order':
				...
				break;
			case 'return_order': //订单退货减去积分操作
				if (!$insertarr['pl_desc']){
					$insertarr['pl_desc'] = '订单'.$insertarr['order_sn'].'商品退款';
				}
				$insertarr['pl_points'] = 0;
				if ($insertarr['orderprice']){
					$insertarr['pl_points'] = '-'.@intval($insertarr['orderprice']/C('points_orderrate'));
		...
		if ($insertarr['pl_lepai_type']){
			$value_array['pl_lepai_type'] = $insertarr['pl_lepai_type'];
		}

		if ($insertarr['pl_lepai_goodsid']){
			$value_array['pl_lepai_goodsid'] = $insertarr['pl_lepai_goodsid'];
		}
		...
		if ($result){
			...
			/*Add is name lt 2016-04-07 积分模板通知*/
			$member_info = $obj_member->getMemberInfoByID($insertarr['pl_memberid']);
			$dataArr['first'] = '您的积分账户变更如下';
			$dataArr['FieldName'] = '用户名';
			$dataArr['Account'] = $member_info['member_name'];
			$dataArr['change'] = intval(substr($insertarr['pl_points'],0,1))?'增加':'减少';
			$dataArr['CreditChange'] = $insertarr['pl_points'];
			$dataArr['CreditTotal'] = $member_info['member_points'];
			// $dataArr['Remark'] = '';
