...
class addressModel extends Model {
    ...
    public function getDefaultAddressInfo($condition = array(), $order = 'is_default desc,dlyp_id asc,address_id desc') {
        $dataArr = $this->getAddressInfo($condition, $order);
		return $dataArr;
    }
    ...
    public function getAddressInfo($condition, $order = '') {
        ...
        if($addr_info['mob_phone']){
            $addr_info['mob_phone'] = JieMiMobile($addr_info['mob_phone']);
        }
        if($addr_info['tel_phone']){
            $addr_info['tel_phone'] = JieMiMobile($addr_info['tel_phone']);
        }
	...
	public function getAddressList($condition, $order='address_id desc'){
        ...
        if (C('delivery_isuse')) {
            ...
            foreach ($address_list as $k => $v) {
				//盛威添加 2015-11-11
				$address_list[$k]['mob_phone'] = JieMiMobile($address_list[$k]['mob_phone']);
                $address_list[$k]['tel_phone'] = JieMiMobile($address_list[$k]['tel_phone']);
                if ($v['dlyp_id']) {
                    $dlyp_ids[] = $v['dlyp_id'];
                }
            }
            ...
            if (!empty($dlyp_new_list)) {
                foreach ($address_list as $k => $v) {
                    ...
                    $address_list[$k]['address'] = '（'.$dlyp_info['dlyp_address_name'].') '.$dlyp_info['dlyp_address']
                        . '，电话：'.JiaMiMobile($dlyp_info['dlyp_mobile'].'，'.$dlyp_info['dlyp_telephony'],'，');
                }
            }
        }else{
            foreach ($address_list as $k => $v) {
	...
	public function addAddress($param){
        if($param['mob_phone']){
            $param['mob_phone'] = JiaMiMobile($param['mob_phone']);
        }
        if($param['tel_phone']){
            $param['tel_phone'] = JiaMiMobile($param['tel_phone']);
        }
	...
	public function getOneAddress($id){
		if (intval($id) > 0){
            ...
            if($result['mob_phone']){
                $result['mob_phone'] = JieMiMobile($result['mob_phone']);
            }
            if($result['tel_phone']){
                $result['tel_phone'] = JieMiMobile($result['tel_phone']);
            }
			return $result;
		}else {
			return false;
	...
	public function editAddress($update, $condition){
        if($update['mob_phone']){
            $update['mob_phone'] = JiaMiMobile($update['mob_phone']);
        }
        if($update['tel_phone']){
            $update['tel_phone'] = JiaMiMobile($update['tel_phone']);
        }
