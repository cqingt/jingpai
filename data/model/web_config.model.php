...
class web_configModel extends Model{
		...
		if(!empty($code_list) && is_array($code_list)) {
    		...
    		switch ($web_id) {
        	    ...
        	    case 121:
        	    	...
        	    	break;
				case 122:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'canyu.php';
        	    	break;
				case 211:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'artist_config'.DS.'focus.php';
        	    	break;
				case 213:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'artist_config'.DS.'artist_index.php';
        	    	break;
				case 214:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'artist_config'.DS.'artist_index.php';
        	    	break;
				case 215:
        	    	...
        	    	break;
				case 216:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'artist_config'.DS.'sale_goods.php';
        	    	break;
				case 217:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'artist_config'.DS.'yishutuijian.php';
        	    	break;
        	    default:
	...
	public function getGoodsList($condition = array(),$order = 'goods_id desc',$page = ''){
	    ...
	    $field = 'goods_id,goods_commonid,goods_name,artist_id,goods_image,goods_price,goods_marketprice';
	    ...
	    if (!empty($goods_list) && is_array($goods_list)) {
	        ...
	        foreach ($goods_list as $key => $value) {
	            ...
	            if(!empty($value['artist_id'])){
	            	$conditionArtist['A_Id'] = $value['artist_id'];
	            	$field = 'A_Name,A_ZhiCheng';
	            	$artist_info = Model('artist_new')->getArtistList($conditionArtist,$field,'','',1);
	            	$value['goods_artist_name'] = $artist_info[0]['A_Name'];
	            	$artist_zhicheng = explode('|',$artist_info[0]['A_ZhiCheng']);
	            	$value['goods_artist_zhiwei'] = $artist_zhicheng[0];
	            }
