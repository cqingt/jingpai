...
<li class="course-nav-item  <?php if($_GET['gc_parent_id'] == $v['gc_id']){echo ' on';}?>" id="attr_id_sel_<?php echo $v['gc_id'];?>"  onmouseover="selAttr(<?php echo $v['gc_id'];?>);">
    <a href="<?php echo urlArtist('artist_new','searchShuHua',array('gc_parent_id'=>$v['gc_id'],'cate_id'=>$v['gc_id']));?>"><?php echo $v['gc_name'];?></a>
</li>
<?php }?>
...
<?php foreach ($xia_v['xiaji_class'] as $k => $v) {?>
    ...
    <li class="course-nav-item <?php if($_GET['cate_id'] == $v['gc_id']){echo ' on';}?>">
        <a href="<?php echo urlArtist('artist_new','searchShuHua',array('gc_parent_id'=>$v['gc_parent_id'],'cate_id'=>$v['gc_id'],'a_id'=>$_GET['a_id']));?>"><?php echo $v['gc_name'];?></a>
    </li>
...
<?php if(!empty($output['checked_attr'][$key])){?>
	...
	<li class="course-nav-item ">
		...
		<a class="category" href="<?php echo urlArtist('artist_new','searchShuHua',array('gc_parent_id'=>$_GET['gc_parent_id'],'cate_id'=>$_GET['cate_id'],'a_id'=>$a_id,'order_key'=>$_GET['order_key'],'is_shop'=>$_GET['is_shop'],'key_type'=>$_GET['key_type'],'keyword'=>$_GET['keyword']));?>"><i class="icon-close"></i><?php echo $output['checked_attr'][$key]['attr_value_name'];?></a>
...
<?php }else{?>
			            ...
			            <li class="course-nav-item ">
			                <a href="<?php $_GET['a_id']?$attr_value_id = $_GET['a_id'].'_'.$v['attr_value_id']:$attr_value_id = $v['attr_value_id'];echo urlArtist('artist_new','searchShuHua',array('gc_parent_id'=>$_GET['gc_parent_id'],'cate_id'=>$_GET['cate_id'],'a_id'=>$attr_value_id));?>"><?php echo $v['attr_value_name'];?></a>
			            </li>
						<?php }?>
...
<!-- 搜索 -->
	...
	<form action="<?php echo urlArtist('artist_new','searchShuHua');?>" method="get">
...
<div class="course-nav-hd wrapper mtb-course">
	<h2>默认排序</h2>
	<div class="sortbox m-l">
	   <a <?php if($_GET['order_key'] == '1' && $_GET['order'] == '2'){?>class="selected"<?php }?> href="<?php echo ($_GET['order'] == '2' && $_GET['order_key'] == '1') ? replaceParam(array('order_key' => '1', 'order' => '1')) : replaceParam(array('order_key' => '1', 'order' => '2')); ?>">销量<i class="icon-arrows"></i></a>

	   <a <?php if($_GET['order_key'] == '2' && $_GET['order'] == '2'){?>class="selected"<?php }?> href="<?php echo ($_GET['order'] == '2' && $_GET['order_key'] == '2') ? replaceParam(array('order_key' => '2', 'order' => '1')) : replaceParam(array('order_key' => '2', 'order' => '2')); ?>">人气<i class="icon-arrows"></i></a>

	   <a <?php if($_GET['order_key'] == '3' && $_GET['order'] == '2'){?>class="selected"<?php }?> href="<?php echo ($_GET['order'] == '2' && $_GET['order_key'] == '3') ? replaceParam(array('order_key' => '3', 'order' => '1')) : replaceParam(array('order_key' => '3', 'order' => '2')); ?>">价格<i class="icon-arrows"></i></a>
...
<div class="wrapper">
	...
	<?php if(!empty($output['goods_list'])){?>
		...
		<div class="product-display">
		  <?php if ($v['goods_state'] == 0 || $v['goods_storage'] <= 0 || ($v['is_virtual'] == 1 && $v['virtual_indate'] < TIMESTAMP)) {?>
...
<li><div class="tb-pic tb-s40"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$v['goods_id']));?>" target="_blank"><img src="<?php echo cthumb($v['goods_image'],60)?>" mid="<?php echo cthumb($v['goods_image'],360)?>"></a></div></li>
			...
			<a href="javascript:;" onclick="NTKF.im_openInPageChat('sc_1022_9999')"><i class="btn icon-chat"></i></a>
...
<script>
	...
	$(function(){
		...
		$("#ptai").click(function(){
			...
			if(che === true){
				window.location.href="<?php echo urlArtist('artist_new','searchShuHua',array('gc_parent_id'=>$_GET['gc_parent_id'],'cate_id'=>$_GET['cate_id'],'a_id'=>$_GET['a_id'],'order_key'=>$_GET['order_key'],'is_shop'=>1,'key_type'=>$_GET['key_type'],'keyword'=>$_GET['keyword']));?>";
			}else{
				window.location.href="<?php echo urlArtist('artist_new','searchShuHua',array('gc_parent_id'=>$_GET['gc_parent_id'],'cate_id'=>$_GET['cate_id'],'a_id'=>$_GET['a_id'],'order_key'=>$_GET['order_key'],'is_shop'=>0,'key_type'=>$_GET['key_type'],'keyword'=>$_GET['keyword']));?>";
			}
