...
class indexControl extends BaseCircleControl{
	...
	public function indexOp(){
		...
		//近期热点
		$ReDian_list = $model->table('circle_theme')->field('*')->where(array('is_closed'=>0, 'is_recommend'=>1))->order('theme_id desc')->limit(5)->select();

		Tpl::output('ReDian_list', $ReDian_list);
		
		//资讯快递
		$ZhiXun_list = $model->table('circle_theme')->field('*')->where(array('is_closed'=>0, 'is_recommend'=>1))->order('theme_id ASC')->limit(6)->select();
		...
		$circle_list = $model->table('circle')->field('*, is_hot*rand() as rand')->where(array('circle_status'=>1, 'is_hot'=>1))->order('circle_mcount desc')->limit(3)->select();
