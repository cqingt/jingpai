...
class indexControl extends HomeControl{
    ...
    public function __construct() {
        ...
        /*SEO*/
        Tpl::output('html_title','拍卖惠'.' - '.C('site_name'));
	...
	public function indexOp(){
        ...
        $theme = $h_model->getThemeInfo(array('T_Jtime'=>array('gt',TIMESTAMP),'T_Iswin'=>'1'));

        $theme_count = count($theme);

/*        if(!is_array($theme) || empty($theme)){
            showMessage('目前没有拍卖惠产品！','','html','error');
        }*/
        ...
        $kaishi_arr = $h_model->getGoodsInfoKai(array('lepai_admin_goods.G_Atype'=>'3','lepai_admin_theme.T_Ktime'=>array('lt',TIMESTAMP),'lepai_admin_goods.G_EndTime'=>array('gt',TIMESTAMP)),'lepai_admin_theme.T_Id =lepai_admin_goods.G_Tid');
        //print_r($kaishi_arr);
        $weikaishi_arr = $h_model->getGoodsInfoKai(array('lepai_admin_goods.G_Atype'=>'3','lepai_admin_theme.T_Ktime'=>array('gt',TIMESTAMP),'lepai_admin_goods.G_EndTime'=>array('gt',TIMESTAMP)),'lepai_admin_theme.T_Id =lepai_admin_goods.G_Tid');
        /*
        foreach ($theme as $k => $v) {
            $auction_info = $h_model->getGoodsInfo(array('G_Tid'=>$v['T_Id'],'G_Atype'=>array('egt','3')));

            foreach ($auction_info as $key => $val) {
                if($v['T_Ktime'] < TIMESTAMP && $val['G_EndTime'] > TIMESTAMP){
                    $kaishi_arr[] = $val;//加入正在拍卖
                }elseif($v['T_Ktime'] > TIMESTAMP){
                    $val['T_Ktime'] = $v['T_Ktime'];
                    $weikaishi_arr[] = $val;//加入即将开始
                }
                $theme[$k]['chujia_count'] += $val['pai_count'];//专场出价总次数
            }
        }
        */

        /*
        $remen_arr = $kaishi_arr;
        if(is_array($remen_arr) && !empty($remen_arr)){
            foreach ($remen_arr as $array){
                if(is_array($array)){
                    $key_arrays[] = $array['pai_count'];
                }
            }
            array_multisort($key_arrays,SORT_DESC,SORT_STRING,$remen_arr);
        }
        */
        $remen_arr = $h_model->getGoodsInfoLimit(array('G_EndTime'=>array('gt',TIMESTAMP),'G_Atype'=>'3'),$theme_count*2);

        Tpl::output('theme',$theme);
        Tpl::output('weikaishi_arr',$weikaishi_arr);
        Tpl::output('kaishi_arr',$kaishi_arr);
        Tpl::output('remen_arr',$remen_arr);
    ...
    public function themeOp(){
        ...
        Tpl::output('html_title',$theme['T_Title'].' - 拍卖惠'.' - '.C('site_name'));
        Tpl::output('nav_link_list',$nav_link);
        Tpl::showpage('theme');
    }
    ...
    public function auctionOp(){
        ...
        Tpl::output('html_title',$goods['G_Name'].' - 拍卖惠'.' - '.C('site_name'));
        Tpl::output('nav_link_list',$nav_link);
    ...
    public function moreOp(){
        ...
        Tpl::output('html_title','历史拍品 - 拍卖惠'.' - '.C('site_name'));
        Tpl::output('nav_link_list',$nav_link);
        Tpl::showpage('more');
    }
    ...
    public function auction_moreOp(){
        ...
        Tpl::output('html_title','正在竞拍的拍品 - 拍卖惠'.' - '.C('site_name'));
        Tpl::output('nav_link_list',$nav_link);
        Tpl::showpage('more');
    }
    ...
    private function makeLepaiSn($id){
        return (date('y',time()) % 9+1) . sprintf('%013d', $id) . sprintf('%02d', 1);
    }
