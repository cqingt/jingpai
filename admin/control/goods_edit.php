...
class goods_editControl extends SystemControl {
    ...
    public function edit_goodsOp() {
        ...
        $store_id = $goodscommon_info['store_id'];
        Tpl::output('store_id', $store_id);
        $where = array('goods_commonid' => $common_id, 'store_id' => $store_id);
        ...
        // 获取类型相关数据
        $typeinfo = $model_type->getAttr($goods_class['type_id'], $store_id, $goodscommon_info['gc_id']);
        ...
        // 实例化店铺商品分类模型
        $store_goods_class = Model('store_goods_class')->getClassTree(array('store_id' => $store_id, 'stc_state' => '1'));
        ...
        // 关联版式
        $plate_list = Model('store_plate')->getStorePlateList(array('store_id' => $store_id), 'plate_id,plate_name,plate_position');
    ...
    public function edit_save_goodsOp() {
        ...
        $store_info = Model('store')->getStoreInfoByID($store_id);
        $update_common['goods_state']        = ($store_info['store_state'] != 1) ? 0 : intval($_POST['g_state']);            // 店铺关闭时，商品下架
        ...
        $update_common['is_own_shop']        = in_array($store_id, model('store')->getOwnShopIds()) ? 1 : 0;
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $update['store_name']        = $store_info['store_name'];
		    ...
		    // 生成商品二维码
                    ...
                    $insert['store_name']        = $store_info['store_name'];
            ...
            $goods_info = $model_goods->getGoodsInfo(array('goods_spec' => serialize(null), 'goods_commonid' => $common_id, 'store_id' => $store_id), 'goods_id');
            if (!empty($goods_info)) {
                $goods_id = $goods_info['goods_id'];
                $update = array ();
                $update['goods_commonid']    = $common_id;
        ...
        $return = $model_goods->editGoodsCommon($update_common, array('goods_commonid' => $common_id, 'store_id' => $store_id));
        if ($return) {
            //提交事务
            Model()->commit();
            showDialog(L('nc_common_op_succ'), urlAdmin('goods', 'goods'), 'succ');
        } else {
            //回滚事务
            Model()->rollback();
            showDialog(L('store_goods_index_goods_edit_fail'), urlAdmin('goods', 'goods'));
        }
    }
    ...
    public function edit_imageOp() {
        ...
        if ($common_list['store_id'] != $store_id || $common_list['goods_lock'] == 1) {
            showMessage(L('wrong_argument'), urlShop('seller_center'), 'html', 'error');
        }
        ...
        $value_array = $model_spec->getSpecValueList(array('sp_value_id' => array('in', $colorid_array), 'store_id' => $store_id), 'sp_value_id,sp_value_name');
    ...
    public function edit_save_imageOp() {
        if (chksubmit()) {
            ...
            $this->recordSellerLog('编辑商品，平台货号：'.$common_id,'1',$store_id);
                ...
                showDialog(L('nc_common_save_fail'), urlAdmin('goods', 'goods'));
            }
        }
    }
    ...
    public function edit_classOp() {
        ...
        // 商品分类
        $store_id = intval($_GET['store_id']);
        $goods_class = $model_goodsclass->getGoodsClass($store_id);

        // 常用商品分类
        $model_staple = Model('goods_class_staple');
        $param_array = array();
        $seller_info = Model('seller')->getSellerInfo(array('store_id'=>$store_id));
        $param_array['member_id'] = $seller_info['member_id'];
    ...
    public function drop_goodsOp() {
        ...
        if ($return) {
            // 添加操作日志
            $this->recordSellerLog('删除商品，平台货号：'.$common_id,'1',$store_id);
    ...
    public function goods_unshowOp() {
        ...
        if ($return) {
            ...
            // 添加操作日志
            $this->recordSellerLog('商品下架，平台货号：'.$common_id,'1',$store_id);
    ...
    public function edit_jingleOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                // 添加操作日志
                $this->recordSellerLog('设置广告词，平台货号：'.$common_id,'1',$store_id);
    ...
    public function edit_plateOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                // 添加操作日志
                $this->recordSellerLog('设置关联版式，平台货号：'.$common_id,'1',$store_id);
                showDialog(L('nc_common_op_succ'), 'reload', 'succ');
            } else {
                showDialog(L('nc_common_op_fail'), 'reload');
            }
    ...
    public function download_f_code_excelOp() {
        ...
        if (empty($common_info) || $common_info['store_id'] != $store_id) {
            showMessage(L('wrong_argument'), '', '', 'error');
        }
    ...
    public function ajax_goods_classOp() {
        ...
        $model_goodsclass = Model('goods_class');
        $list = $model_goodsclass->getGoodsClass($store_id, $gc_id, $deep);
        if (empty($list)) {
            exit();
        }
        /**
     ...
     * @param $state 1成功 0失败
     */
    protected function recordSellerLog($content = '', $state = 1,$store_id){
        $sInfo = Model('seller')->getSellerInfo(array('store_id'=>$store_id));
        $seller_info = array();
        $seller_info['log_content'] = '管理员'.$this->admin_info['name'].$content;
        $seller_info['log_time'] = TIMESTAMP;
        $seller_info['log_seller_id'] = $sInfo['seller_id'];
        $seller_info['log_seller_name'] = $sInfo['seller_name'];
    ...
    public function image_uploadOp() {
        ...
        $data = array ();
        $data ['thumb_name'] = cthumb($upload->getSysSetPath() . $upload->thumb_image, 240, $store_id);
        $data ['name']      = $img_path;
