...
class storeControl extends SystemControl{
	...
	public function storeOp(){
		...
		if(intval($_GET['store_is_shuhua_']) == 1){
			$condition['store_is_shuhua_'] = 1;	
		}

		if(intval($_GET['store_is_shuhua_']) == 2){
			$condition['store_is_shuhua_'] = 0;	
		}
		if($choose_gcid > 0){
		
			$param['class_'.($gccache_arr['showclass'][$choose_gcid]['depth'])] = $choose_gcid;
			
			$model_store_bind_class = Model('store_bind_class');
			$StoreList = $model_store_bind_class->getStoreBindClassList($param,'','','store_id');
			if($StoreList){
				$store = array();
				...
				$condition['store_id'] = array('in',0);
			}

			Tpl::output('goods_class',explode(',', $_GET['goods_class']));
		}
		...
		if($_GET['yunying_name']){
			$condition['yunying_name'] = $_GET['yunying_name'];
		}

		if($_GET['zhaoshang_name']){
			$condition['zhaoshang_name'] = $_GET['zhaoshang_name'];
		}
		...
		//店铺列表
		$store_list = $model_store->getStoreList($condition, 10,'store_sort asc,store_id desc');
	...
	public function ajaxOp(){
		switch ($_GET['branch']){
			case 'store_sort':
			$id = intval($_GET['id']);
			if($id <= 0){ echo 'false'; exit;}
			$update_array['store_sort'] = intval($_GET['value']); //排序
			$result = Model('store')->editStore($update_array, array('store_id' => $id));
			echo 'true';exit;
			break;
		}
	...
	public function store_editOp(){
		...
		if (chksubmit()){
			...
			$update_array['store_sort'] = intval($_POST['store_sort']); //排序
			...
			/****************************招商运营人员信息**********************************/
			$update_array['zhaoshang_name'] = trim($_POST['zhaoshang_name']);//招商人姓名
			$update_array['yunying_name'] = trim($_POST['yunying_name']);//运营人员姓名
			$update_array['yunying_mobile'] = trim($_POST['yunying_mobile']);//运营人员手机号
			$update_array['yunying_qq'] = trim($_POST['yunying_qq']);//运营人员QQ
			/****************************招商运营人员信息 END******************************/
			
			$update_array['store_yijia'] = intval($_POST['store_yijia']);//议价开关
            $update_array['store_is_shuhua_'] = intval(trim($_POST['store_is_shuhua_']))?intval(trim($_POST['store_is_shuhua_'])):0;//是否书画代运营
            $update_array['store_is_number_'] = intval(trim($_POST['store_is_number_']))?intval(trim($_POST['store_is_number_'])):0;//店铺编号

			$update_array['least_shuhua'] = intval($_POST['least_shuhua']);
			//zmr>v95
    ...
    public function edit_save_joininOp() {
        if (chksubmit()) {
			...
			if ($_FILES['business_legal_person_img']['name'] != '') {
	...
	public function store_joininOp(){
        ...
        /* 2016-06-23 Add is name lt 时间检索功能*/
        ...
        if($sdate && $edate){
            ...
            $condition['add_time'] = array('egt',$sdate);

        }elseif(empty($sdate) && $edate){

            $condition['add_time'] = array('elt',$edate);
		...
		$store_list = $model_store_joinin->getList($condition, 10, 'joinin_state ASC,add_time desc');
		Tpl::output('store_list', $store_list);
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
			...
			$shop_array['grade_id']		= intval($joinin_detail['sg_id']);
			$shop_array['store_name']	= $joinin_detail['store_name'];
			$shop_array['sc_id']		= intval($joinin_detail['sc_id']);
            ...
            if($store_id) {
                ...
                $seller_array['seller_quicklink'] = 'store_goods_add,store_plate,store_order,store_evaluate,store_decoration,store_consult,store_complain,store_bill';
			...
			if($state) {
                ...
                //插入店铺绑定分类表
                /*  隐藏绑定分类，分类审核成功后，管理员添加 xin 20151102
    ...
    public function newshop_addOp()
    {
        ...
        if (chksubmit())
        {
            ...
            model('seller')->addSeller(array(
                ...
                'seller_group_id' => 0,
                'is_admin' => 1,
                'seller_quicklink'=>'store_goods_add,store_plate,store_order,store_evaluate,store_decoration,store_consult,store_complain,store_bill',
            ));
    ...
    public function save_appkey_typeOp(){
        $storeid = intval($_GET['storeid']);
        $type = trim($_GET['type']);

        if($storeid && $type){
            if($type == 'off'){
                Model('store')->editStore(array('store_is_appkey'=>22),array('store_id'=>$storeid));
            }else{
                Model('store')->editStore(array('store_is_appkey'=>11),array('store_id'=>$storeid));
            }
