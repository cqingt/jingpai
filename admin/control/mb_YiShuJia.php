...
class mb_YiShuJiaControl extends SystemControl{
    ...
    public function index_editOp() {
        ...
        Tpl::showpage('mb_Yishu_item.list');
    }
    ...
    public function special_editOp() {
        $model_mb_special = Model('mb_special');
		$special_detail = $model_mb_special->getMbSpecialList(array('special_id'=>$_GET['special_id']));
		Tpl::output('special_detail', $special_detail[0]);
     ...
     * 保存背景颜色设置
     */
    public function BaoCunBeiJinOp() {
		$model_mb_special = Model('mb_special');
		$update = array();
		$special_background = $this->mb_special_image_upload('special_background');
        if(!empty($special_background)) {
            $update['special_background'] = $special_background;
            if(!empty($_POST['old_special_background'])) {
                $this->mb_special_image_drop($_POST['old_special_background']);
            }
        }
		$update['special_background_color'] = empty($_POST['special_background_color'])?'#FFFFFF':$_POST['special_background_color'];
		$update['special_repeat'] = empty($_POST['special_repeat'])?'no-repeat':$_POST['special_repeat'];
		$model_mb_special->editMbSpecial($update,intval($_POST['special_id']));
		showMessage("保存成功",urlAdmin('mb_special', 'special_edit',array('special_id'=>$_POST['special_id'])));
	}
	
	/**
     * 上传图片
     */
    private function mb_special_image_upload($image) {
     ...
     * 图片删除
     */
    private function mb_special_image_drop($image) {
    ...
    public function goods_listOp() {
        ...
        $goods_list = $model_goods->getGoodsOnlineList($condition, 'goods_id,goods_name,goods_price,goods_promotion_price,goods_image', 10);
