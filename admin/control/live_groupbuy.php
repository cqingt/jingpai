...
class live_groupbuyControl extends SystemControl{
	...
	public function del_groupbuyOp(){
		if(isset($_POST)&&!empty($_POST)){
			...
			if($res){
				showMessage('删除团购成功','index.php?act=live_groupbuy','','succ');
			}else{
				showMessage('删除团购失败','index.php?act=live_groupbuy','','error');
			}
		}
	}
	...
	public function auditOp(){
		...
		if($res){
			$this->log('审核团购成功[ID:'.intval($_GET['groupbuy_id']).']',1);
	...
	public function cancelOp(){
		$model_live_groupbuy = Model('live_groupbuy');
		$res = $model_live_groupbuy->edit(array('groupbuy_id'=>intval($_GET['groupbuy_id'])),array('is_open'=>2));//取消团购

		if($res){
			$this->log('取消团购[ID:'.intval($_GET['groupbuy_id']).']',1);
	...
	public function groupbuyvoucherOp(){
		...
		if(empty($groupbuy)){
			showMessage('团购不存在','index.php?act=live_groupbuy','','error');
		}
