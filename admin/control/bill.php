...
class billControl extends SystemControl{
	...
	public function show_billOp(){
		...
		if ($_GET['query_type'] == 'refund') {
		...
		}elseif($_GET['query_type'] == 'lepaiorder'){

            $model_lepai = Model('lepai_home');
            $lepai_condition = array();
            $lepai_condition['order_state'] = ORDER_STATE_SUCCESS;
            $store_info = $model_lepai->table('store,lepai_audit')->field('store.member_id,lepai_audit.commis_rate')->join('inner')->on('store.member_id=lepai_audit.member_id')->where(array('store.store_id'=>$bill_info['ob_store_id']))->find();

            $lepai_condition['store_member_id'] = $store_info['member_id'];
            if ($if_start_date || $if_end_date) {
                $lepai_condition['finnshed_time'] = array('between',"{$start_unixtime},{$end_unixtime}");
            } else {
                $lepai_condition['finnshed_time'] = array('between',"{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}");
            }


            $lepaiorder_list = $model_lepai->getLepaiOrder($lepai_condition,'20');


            Tpl::output('store_info',$store_info);
            Tpl::output('order_list',$lepaiorder_list);
            Tpl::output('show_page',$model_lepai->showpage());
            $sub_tpl_name = 'bill_order_bill.show.lepaiorder_list';

        }elseif($_GET['query_type'] == 'lepaicancelorder'){
            $model_lepai = Model('lepai_home');
            $lepai_condition = array();
            $lepai_condition['order_state'] = 0;
            $store_info = $model_lepai->table('store,lepai_audit')->field('store.member_id,lepai_audit.commis_rate')->join('inner')->on('store.member_id=lepai_audit.member_id')->where(array('store.store_id'=>$bill_info['ob_store_id']))->find();

            $lepai_condition['store_member_id'] = $store_info['member_id'];
            if ($if_start_date || $if_end_date) {
                $lepai_condition['cancel_time'] = array('between',"{$start_unixtime},{$end_unixtime}");
            } else {
                $lepai_condition['cancel_time'] = array('between',"{$bill_info['ob_start_date']},{$bill_info['ob_end_date']}");
            }
            $lepaiorder_list = $model_lepai->getLepaiOrder($lepai_condition,'20',"add_time desc","order_id,order_sn,pd_amount,(select T_Title from shop_lepai_admin_theme where T_Id = lepai_theme_id) as teme_name,(select G_Name from shop_lepai_admin_goods where G_Id = lepai_goods_id) as goods_name,(select G_EndTime from shop_lepai_admin_goods where G_Id = lepai_goods_id) as end_time");
            Tpl::output('store_info',$store_info);
            Tpl::output('order_list',$lepaiorder_list);
            Tpl::output('show_page',$model_lepai->showpage());
            $sub_tpl_name = 'bill_order_bill.show.lepaicancelorder_list';
		}else {
			...
			if($store_info['store_is_shuhua_'] == 1){
				$field = 'SUM(IF(ROUND(goods_pay_price*(1-commis_rate/100),2)<(select goods_costprice from shop_goods_common where goods_commonid=(select goods_commonid from shop_goods where `shop_goods`.goods_id = `shop_order_goods`.goods_id))*goods_num,goods_pay_price-(select goods_costprice from shop_goods_common where goods_commonid=(select goods_commonid from shop_goods where `shop_goods`.goods_id = `shop_order_goods`.goods_id))*goods_num,ROUND(goods_pay_price*commis_rate/100,2))) as commis_amount,order_id';
			}else{
		...
		$store_array = Model('store')->getStoreInfoByID($bill_info['ob_store_id']);
	...
	public function export_orderOp(){
		...
		if($_GET['query_type'] == 'lepaiorder'){
            ...
            $data = $model_lepai->getLepaiOrder($lepai_condition,'');
		...
		$export_data = array();
		$export_data[0] = array('订单编号','订单金额','运费','佣金','下单日期','付款时间','付款方式','流水号','成交日期','商家','商家编号','买家','买家编号','商品');
		$order_totals = 0;
		$shipping_totals = 0;
		$commis_totals = 0;
		$k = 0;
		foreach ($data as $v) {
			...
			if($_GET['query_type'] == 'lepaiorder'){
			 $commis_totals += $export_data[$k+1][] = ncPriceFormat($store_info['commis_rate'] * $v['order_amount']/100);
			}else{
		    ...
		    $export_data[$k+1][] = date('Y-m-d',$v['payment_time']);
		    $export_data[$k+1][] = orderPaymentName($v['payment_code']);
		    $export_data[$k+1][] = 'SC'.$v['trade_no'];
