...
class goodsControl extends SystemControl{
    ...
    public function goodsOp() {
        ...
        foreach ($goods_list as $k => &$v) {
            $condition_store['store_id'] = $v['store_id'];
            $store_state = Model()->table('store')->field('store_state')->where($condition_store)->find();
            if(!empty($store_state)){
                $v['store_state'] = reset($store_state);
            }
        }
     ...
     * 类目调整
     */
    public function goods_leimuOp() {
		/**
         * 处理商品分类
         */
        $choose_classid = ($t = intval($_REQUEST['choose_classid']))>0?$t:0;
        $gccache_arr = Model('goods_class')->getGoodsclassCache($choose_classid,3);
        if (chksubmit()) {
			//处理商品转移
            $commonids = $_POST['commonids'];
            $commonid_array = explode(',', $commonids);
			...
			$update = array();
			$update2 = array();
            $update2['gc_id_1'] = intval($_POST['search_class'][0]);
			$update2['gc_id_2'] = intval($_POST['search_class'][1]);
			$update2['gc_id_3'] = intval($_POST['search_class'][2]);
			$update2['gc_id'] = $choose_classid;

			$update['gc_id_1'] = intval($_POST['search_class'][0]);
			$update['gc_id_2'] = intval($_POST['search_class'][1]);
			$update['gc_id_3'] = intval($_POST['search_class'][2]);
			$update['gc_id'] = $choose_classid;
			$gc_name = Model('goods_class')->getGoodsClassLineForTag($choose_classid);
			$update['gc_name'] = $gc_name['gc_tag_name'];

			//书画 >国画 >动物
			//file_put_contents('aaa.txt',print_r($gc_name['gc_tag_name'],true).'=='.print_r($update,true));
			//exit;
			$where = array();
            $where['goods_commonid'] = array('in', $commonid_array);
            Model('goods')->editProducesGoodsClassUp($update,$update2, $where); //批量修改产品分类
			Model('goods_class')->dropCache();//更新删除分类缓存
            showDialog(L('nc_common_op_succ'), 'reload', 'succ');
		...
		Tpl::output('gc_choose_json',json_encode($gccache_arr['choose_gcid']));
        Tpl::output('commonids', $_GET['id']);
        Tpl::showpage('goods.leimu', 'null_layout');
    }
    ...
    /*AJAX提交精品推荐*/
    public function ajax_is_pushOp(){
        $id = trim($_GET['id']);
        $type = trim($_GET['type']);
        if($type == 'add'){
            $model = Model('goods_common');
            $data['is_push'] = '1';
            if($model->where(array('goods_commonid'=>$id))->update($data)){
                echo $id;
            }
        }elseif($type == 'del'){
            $model = Model('goods_common');
