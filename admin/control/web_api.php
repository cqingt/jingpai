...
class web_apiControl extends SystemControl {
	...
	public function code_updateOp() {
		...
		if (!empty($code)) {
			...
			$state = $model_web_config->updateCode(array('code_id'=> $code_id),array('code_info'=> $code_info,'code_html'=>$_POST['article_content']));
		}
		if($state) {
			echo '1';exit;
		} else {
	...
	public function recommend_picOp() {
		...
		if (!empty($code) && $key_id > 0 && $pic_id > 1) {
			...
			//盛威添加 2015-9-19 功能，增加值得参与保存刷新
			if($_GET['sw']){
				$a = 1;
			}
			Tpl::showpage('web_recommend_pic'.$a,'null_layout');
		}
	}
	...
	private function _upload_pic($file_name) {
	    ...
	    file_put_contents('admin_images_.txt',print_r($pic_name,true),FILE_APPEND);
