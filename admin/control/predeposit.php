...
class predepositControl extends SystemControl{
	...
	public function pd_cash_payOp(){
        ...
        try {
			...
			$dataArr['first'] = $member_info['member_name'].',您好。您的提现申请已处理。';
			$dataArr['keyword1'] = $info['pdc_amount'];
			$dataArr['keyword2'] = '普通提现';
			$dataArr['keyword3'] = date("Y-m-d",$info['pdc_add_time']);
			$dataArr['keyword4'] = '审核通过';
			$dataArr['keyword5'] = date("Y-m-d",time());
			$dataArr['remark'] = '如果您有任何疑问，可咨询在线客服或致电客户服热线400-81-96567，我们将竭诚为您服务。';
	...
	public function add_money(){
			...
			if($memo){
				//Du 2016-11-30新加流水单号检测
				@preg_match_all('/\d+/',$memo,$arr);
				if($arr[0]){
					$pos = array_search(max($arr[0]), $arr[0]);
					$trade_no = $arr[0][$pos];
					$orderContNo = Model('order')->table('order')->field('order_sn,order_amount')->where(array("trade_no"=>$trade_no))->find();
					if($orderContNo){
						showMessage('该流水单号是已支付订单【'.$orderContNo['order_sn'].'】支付金额【'.$orderContNo['order_amount'].'元】','','','error');
					}
				}
			}
		    if ($money <= 0) {
