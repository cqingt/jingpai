	...
	public function memberOp(){
		...
		if ($_GET['search_field_value'] != '') {
    		switch ($_GET['search_field_name']){
				...
				//zmr>v30
				case 'member_mobile':
    				$condition['member_mobile'] = array('like', '%' . JiaMiMobile(trim($_GET['search_field_value'])) . '%');
    				break;
        ...
        //add 注册来源  xin 20151130
        if (is_array($_GET['search_field_regfrom']) && !empty($_GET['search_field_regfrom'])) {
            $search_field_regfrom = '';
            foreach($_GET['search_field_regfrom'] as $v){
                $search_field_regfrom .= $v.',';
            }
            $search_field_regfrom = substr($search_field_regfrom,0,-1);
            $condition['member_from'] = array('in',$search_field_regfrom);
        }
		...
		//会员等级
		$search_grade = $_GET['search_grade'] == '' ? -1 : intval($_GET['search_grade']);
		if ($search_grade >= 0 && $member_grade){
			if($member_grade[$search_grade+1]['exppoints']){
				...
				$condition['member_exppoints'] = array(array('egt',$member_grade[$search_grade]['exppoints']));
			}
		...
		if($_GET['search_begin_time'] && !$_GET['search_end_time']){
			$condition['member_time'] = array('gt',strtotime($_GET['search_begin_time']));
		}elseif(!$_GET['search_begin_time'] && $_GET['search_end_time']){
			$condition['member_time'] = array('lt',strtotime($_GET['search_end_time']));
		}elseif($_GET['search_begin_time'] && $_GET['search_end_time']){
			$condition['member_time'] = array(array('gt',strtotime($_GET['search_begin_time'])),array('lt',strtotime($_GET['search_end_time'])),'and');
		}
		...
		Tpl::output('search_field_regfrom',$_GET['search_field_regfrom']);
		Tpl::output('member_list',$member_list);
		Tpl::output('member_from',$model_member->field('member_from')->where("member_from <> ''")->group('member_from')->select());
		Tpl::output('page',$model_member->showpage());
		Tpl::output('pageNumber',$model_member->getTotalNum());
