...
class activityControl extends SystemControl{
	...
	public function newOp(){
		...
		$validate_arr[] = array('input'=>$_FILES['activity_banner_m']['name'],'require'=>'true','message'=>Language::get('activity_new_banner_null'));
		...
		$upload->upfile('activity_banner');
		$images_pc = $upload->file_name;

		$upload	= new UploadFile();
		$upload->set('default_dir',ATTACH_ACTIVITY);
		$upload->upfile('activity_banner_m');
		$images_m = $upload->file_name;

		if(empty($images_pc) || empty($images_m)){
		...
		$input['activity_banner']	= $images_pc;
		$input['activity_banner_m']	= $images_m;
		...
		if($result){
			...
			@unlink(BASE_UPLOAD_PATH.DS.ATTACH_ACTIVITY.DS.$images_pc);
			@unlink(BASE_UPLOAD_PATH.DS.ATTACH_ACTIVITY.DS.$images_m);
	...
	public function editOp(){
		...
		if($_FILES['activity_banner_m']['name']!=''){
			$upload	= new UploadFile();
			$upload->set('default_dir',ATTACH_ACTIVITY);
			$result	= $upload->upfile('activity_banner_m');
			if(!$result){
				showMessage($upload->error);
			}
			$input['activity_banner_m']	= $upload->file_name;
		}
     ...
     * 修改分类名称
     */
    public function update_catOp(){
        if(empty($_REQUEST['activity_detail_id'])){
            showMessage('请选择要改下的选项');
        }
        ...
        $activity_detail	= Model('activity_detail');
        $Res = $activity_detail->update(array('cat_name'=>$cat_name),$id);
        if($Res){
            showMessage('修改成功');
        }else{
            showMessage('修改失败');
