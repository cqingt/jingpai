...
class typeControl extends SystemControl {
	...
	public function typeOp(){
		...
		if(!empty($goods_name)){
			...
			$condition_goods['goods_name'] = array(array('like',"%{$goods_name}%"));
			$goods_info = $model_goods->getGoodsInfo($condition_goods,'gc_id,gc_id_1,gc_id_2,gc_id_3');

			$condition['class_id'] = $goods_info['gc_id'];
		}

		if(!empty($choose_gcid)){
			$class_info = Model('goods_class')->getChildClass($choose_gcid);
			$gc_id = array();
			if($class_info){
			...
			$condition['class_id'] = array('in',$gc_id);
		}


		$type_list = $model_type->table('type')->where($condition)->order('type_sort asc')->page(10)->select();
		...
		Tpl::output('page',$model_type->showpage(2));
