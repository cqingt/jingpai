	...
	public function adv_addOp(){
		if(!chksubmit()){
			...
			if ($error != ''){
				...
				$insert_array['adv_start_date']  = strtotime($_POST['adv_start_time']);//$this->getunixtime($_POST['adv_start_time']);
				$insert_array['adv_end_date']    = strtotime($_POST['adv_end_time']);//$this->getunixtime($_POST['adv_end_time']);
    ...
    public function adv_editOp(){
    	if($_POST['form_submit'] != 'ok'){
			...
			if ($error != ''){
                ...
                $param['adv_start_date'] = strtotime($_POST['adv_start_date']);//$this->getunixtime(trim($_POST['adv_start_date']));
                $param['adv_end_date']   = strtotime($_POST['adv_end_date']);//$this->getunixtime(trim($_POST['adv_end_date']));
				/**
				  * 建立图片广告信息的入库数组
				  */
				if($_POST['mark'] == '0'){
