...
class cms_specialControl extends SystemControl{
     ...
     * cms专题开启OR关闭
     **/
    public function cms_special_onOp() {
        $special_id = intval($_GET['special_id']);
        $on_num = intval($_GET['on'])?intval($_GET['on']):'0';


        $dataArr['special_id'] = $special_id;
        $uploadArr['special_on'] = $on_num;

        if(Model()->table('cms_special')->where($dataArr)->update($uploadArr)){
            redirect('index.php?act=cms_special&op=cms_special_list&curpage='.$_GET['curpage']);
        }else{
            redirect('index.php?act=cms_special&op=cms_special_list&curpage='.$_GET['curpage']);
        }
    ...
    public function cms_special_saveOp() {
		...
		$param['special_keywords'] = $_POST['special_keywords'];
		$param['special_description'] = $_POST['special_description'];
