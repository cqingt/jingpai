...
class goods_classControl extends SystemControl{
	...
	public function goods_class_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$insert_array['is_artist']		= trim($_POST['is_artist']);
				$insert_array['is_two_class']	= trim($_POST['is_two_class']);
                ...
                $insert_array['gc_isopen']     = ($_POST['gc_isopen'] == 'on' || $_POST['gc_isopen'] == 1)?'1':'0';//add 是否开放此分类给第三方店铺展示 xin 20151104
	...
	public function goods_class_editOp(){
		...
		if (chksubmit()){
			...
			$update_array['is_artist']		= trim($_POST['is_artist']);
			$update_array['is_two_class']	= trim($_POST['is_two_class']);
            ...
            $update_array['gc_isopen']     = ($_POST['gc_isopen'] == 'on' || $_POST['gc_isopen'] == 1)?'1':'0';//add 是否开放此分类给第三方店铺展示 xin 20151104
			...
			if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
				...
				while (!feof($fp)) {
					...
					if (!empty($data)){
						...
						//盛威添加 删除数组末尾为空的元素
						$tmp_array = delArrayLastNull($tmp_array);
