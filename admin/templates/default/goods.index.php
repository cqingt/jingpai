  ...
  <form method="get" name="formSearch" id="formSearch">
    ...
    <table class="tb-type1 noborder search">
      <tbody>
		...
		<th><label for="search_commonid">店铺类型</label></th>
        <td>
			<select name="type_id" id="type_id"  class="querySelect"><option value="0">请选择...</option><option value="1" <?php if($output['search']['type_id'] == 1){ ?>selected<?php }?>>自运营店铺</option><option value="2" <?php if($output['search']['type_id'] == 2){ ?>selected<?php }?>>代运营店铺</option><option value="3" <?php if($output['search']['type_id'] == 3){ ?>selected<?php }?>>商家店铺</option></select>
		</td>
        </tr>
        <tr>
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="align-center">
            ...
            <?php if($v['goods_lock'] != 1){ ?>
            <!--<a href="<?php echo urlAdmin('goods_edit', 'edit_goods', array('commonid' => $v['goods_commonid']));?>"><?php echo $lang['nc_edit'];?></a>-->
            <?php }else{echo '锁定';}?>
            &nbsp;|&nbsp;<a href="<?php if($v['store_state'] == 1){echo urlShop('goods', 'index', array('goods_id' => $output['storage_array'][$v['goods_commonid']]['goods_id']));}else{echo "index.php?act=goods&op=goods&search_store_name=$v[store_name]";}?>" target="_blank"><?php echo $lang['nc_view'];?></a>
      ...
      <tfoot>
        <tr class="tfoot">
          ...
          <td colspan="16"><label for="checkallBottom"><?php echo $lang['nc_select_all']; ?></label>
			...
			<a href="JavaScript:void(0);" class="btn" nctype="leimu_tiaozeng"><span>类目调整</span></a>
...
$(function(){
	...
	// 类目调整
    $('a[nctype="leimu_tiaozeng"]').click(function(){
        str = getId();
        if (str) {
            goods_leimu(str);
        }
...
// 类目调整
function goods_leimu(ids) {
    _uri = "<?php echo ADMIN_SITE_URL;?>/index.php?act=goods&op=goods_leimu&id=" + ids;
    CUR_DIALOG = ajax_form('goods_leimu', '类目调整', _uri, 450);
}
// 获得选中ID
function getId() {
    var str = '';
...
function is_push(id,type){
  var text = $("#ispush_"+id).text();
  if(text == '精品推荐'){
    $.ajax({
        type: "GET",
        cache: false,
        url : "index.php?act=goods&op=ajax_is_push",
        data: 'id=' + id + '&type=add',
        success : function(html){
          if(html){
            $("#ispush_"+id).text('取消推荐');
          }
        }
    });
  }else if(text == '取消推荐'){
