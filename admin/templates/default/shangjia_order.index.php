...
<div class="page">
  ...
  <form method="get" action="index.php" name="formSearch" id="formSearch">
    <input type="hidden" name="act" value="shangjia_order" />
         ...
         <th><label><?php echo $lang['order_state'];?></label></th>
          <td colspan="4"><select name="order_state" class="querySelect">
			...
			<select name="is_short">
				<option value="0" <?php if(intval($_GET['is_short']) == 0){ ?> selected <?php }?>>全部订单店铺</option>
				<option value="1" <?php if(intval($_GET['is_short']) == 1){ ?> selected <?php }?>>自营店铺</option>
				<option value="2" <?php if(intval($_GET['is_short']) == 2){ ?> selected <?php }?>>第三方店铺</option>
			</select>
			</td>
			-->
            <th><label>流水单号</label></th>
            <td><input class="txt2" type="text" name="trade_no" value="<?php echo $_GET['trade_no'];?>" /></td>
        
        </tr>
        <tr>
          <th><label for="query_start_time">支付时间</label></th>
  ...
  <table class="table tb-type2 nobdb">
    ...
    <tbody>
      ...
      <tr class="hover">
        ...
        <td class="nowrap align-center"><?php if($order['payment_time']){ echo date('Y-m-d H:i:s',$order['payment_time']); } ?></td>
		...
		<td class="align-center"><?php echo $order['trade_no'];?></td>
