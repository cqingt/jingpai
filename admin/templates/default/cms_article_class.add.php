  ...
  <form id="add_form" method="post" action="index.php?act=cms_article_class&op=cms_article_class_save">
    <table class="table tb-type2">
      <tbody>
        ...
        <!-- 09-10新加三级分类添加 -->
        <tr class="noborder">
          <td colspan="2" class="required"><label class="validation" for="class_name"><?php echo '请选择分类'.$lang['nc_colon'];?></label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform">
            <select name="class_parent_id" id="class_parent_id" class="valid">
              <option value="0">请选择...</option>
<?php foreach($output['list'] as $v){ ?>
  <?php if($v['class_parent_id'] == '0'){?>
    <option value="<?php echo $v['class_id']; ?>">&nbsp;&nbsp;<?php echo $v['class_name']; ?></option>
        <?php foreach($output['list'] as $vv){ ?>
          <?php if($vv['class_parent_id'] == $v['class_id']){?>
            <option value="<?php echo $vv['class_id']; ?>">&nbsp;&nbsp;<?php echo '|--'.$vv['class_name']; ?></option>
                <?php foreach($output['list'] as $vvv){ ?>
                  <?php if($vvv['class_parent_id'] == $vv['class_id']){?>
                    <option value="<?php echo $vvv['class_id']; ?>">&nbsp;&nbsp;<?php echo '|--|--'.$vvv['class_name']; ?></option>
                  <?php };?>
                <?php };?>
          <?php };?>
        <?php };?>
