...
class circle_member_snshomeControl extends BaseSNSControl {
	...
	public function indexOp(){
		...
		if (!empty($storelist)){
			...
			$goodslist = $goods_model->getGoodsOnlineList(array('store_id'=> array('in', $storeid_arr)), 'goods_id,goods_name,goods_image,store_id,goods_price');
			if (!empty($goodslist)){
				foreach ($goodslist as $k=>$v){
	...
	public function shareglistOp(){
		...
		unset($condition['share_islike']);

		$condition['share_isshare'] = "1";//分享的商品
		$condition['order'] = " share_addtime desc";

		$sharegoods_model = Model('sns_sharegoods');
		$goodssharelist = $sharegoods_model->getSharegoodsList($condition,$page,'*','detail');

		if(!empty($goodslist) || !empty($goodssharelist)){
			$shareid_array = array();
			$likeid_array = array();
			foreach($goodssharelist as $val){
				$shareid_array[]	= $val['share_id'];
			}
			foreach($goodslist as $val){
				$likeid_array[]	= $val['like_id'];
			}
			...
			$like_array = Model()->table('sns_albumpic')->field('count(item_id) as count,item_id,ap_cover')->where(array('ap_type'=>1, 'item_id'=>array('in', $likeid_array)))->group('item_id')->select();
			if(!empty($pic_array) || !empty($like_array)){
				$pic_list = array();
				$like_list = array();
				...
				foreach ($like_array as $val){
					$val['ap_cover'] = UPLOAD_SITE_URL.'/'.ATTACH_MALBUM.'/'.$this->master_id.'/'.str_ireplace('.', '_1024.', $val['ap_cover']);
					$like_list[$val['item_id']]	= $val;
				}
				Tpl::output('pic_list', $pic_list);
				Tpl::output('pic_list', $like_list);
...
//		print_r($goodslist);
//		echo '-->';
		Tpl::output('goodssharelist',$goodssharelist);

		Tpl::output('show_page',$page->show());
		Tpl::output('menu_sign','sharegoods');
		Tpl::showpage('circle/baoBei');
	}
	...
	public function storelistOp(){
		...
		if (!empty($storelist)){
			...
			//查询店铺推荐商品
			$goods_model = Model('goods');
			$goodslist = $goods_model->getGoodsOnlineList(array('store_id'=> array('in', $storeid_arr), 'goods_commend' => 1), 'goods_id,store_id,goods_name,goods_image,goods_price');
			if (!empty($goodslist)){
				foreach ($goodslist as $k=>$v){
					$v['goodsurl'] = urlShop('goods', 'index', array('goods_id'=>$v['goods_id']));
					$storelist_new[$v['store_id']]['goods'][] = $v;
	...
	public function traceOp(){
		...
		$lazy_arr = lazypage(15,$delaypage,$count,true,$page->getNowPage(),$page->getEachNum(),$page->getLimitStart());
		//动态列表
...
//		echo '-->';
		...
		//取得分享商品或分享店铺数据  add mk time 2016-11-30
//		$trace = array();
//		foreach ($tracelist as $key => $value) {
//			if($value['trace_title'] === "分享了商品")
//			{
//				$trace['分享了商品'][] = $value['trace_content'];
//			}
//			elseif ($value['trace_title'] === "分享了店铺") {
//				$trace['分享了店铺'][] = $value['trace_content'];
//			}
//		}
//
////		$trace = preg_replace("/[\t\n\r]+/","",$trace);
//	//	分享商品
//		$pre_goods = '#<div class="fd-media"><div class="goodsimg"><a target="_blank" href="(.*)"><img src="(.*)" onload=".*" alt="(.*)"></a></div><div class="goodsinfo"><dl><dt><a target="_blank" href="(.*)">(.*)</a></dt><dd>价&nbsp;&nbsp;格：&yen;(.*)</dd><dd>运&nbsp;&nbsp;费：&yen;20.0</dd><dd nctype=".*"><a href="javascript:void(0);" onclick=".*">收藏该宝贝</a></dd></dl></div></div>#isu';
//
//		//分享店铺
//		$pre_store = '<div class="fd-media">
//					<div class="goodsimg"><a target="_blank" href="http://www.96567.com/shop-24.html"><img src="http://images.96567.com/upload/shop/common/default_store_avatar.gif" onload="javascript:DrawImage(this,120,120);" alt="藏金阁"></a></div>
//					<div class="goodsinfo">
//						<dl>
//							<dt><a target="_blank" href="http://www.96567.com/shop-24.html">藏金阁</a></dt>
//	                  		<dd nctype="storecollectbtn_24"><a href="javascript:void(0);" onclick="javascript:collect_store(\'24\',\'succ\',\'storecollectbtn_24\');">收藏该店铺</a></dd>
//	                  	</dl>
//	                  </div>
//	             </div>';
//
//		preg_match_all($pre_store, $trace['分享了店铺'][0], $arr_store);
//
//		preg_match_all($pre_goods, $trace['分享了商品'][0], $arr_goods);
//		echo '<!--';
//		print_r($arr_goods);
//		echo '-->';
		//输出会员id
		$member_id = $_GET['mid'];
		Tpl::output('mid',$member_id);
		...
		Tpl::output('nchash',substr(md5(SHOP_SITE_URL.$_GET['act'].$_GET['op']),0,8));
		Tpl::output('menu_sign', 'snstrace');
		Tpl::showpage('circle/xinXianShi');
	}
	...
	public function trace_delOp(){
		...
		$model = Model();
		$trace_id = $_GET['id'];
		$result = $model->table('sns_tracelog')->delete($trace_id);
		if($result){
			showMessage('删除成功');
		}
			showMessage('参数错误');
