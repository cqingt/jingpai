...
class circle_sns_albumControl extends BaseSNSControl {
...
//		Tpl::output('menu_sign', 'snsalbum');
	...
	public function album_cateOp(){
		...
		Tpl::output('ac_list_member_id',$ac_list[0]['member_id']);
		self::profile_menu('album','album');
		Tpl::showpage('circle/xiangCe');
	}
	/*
	 * 上传图片
	 * add name mk
	 * add time 2019-11-17
	 * */
	public function photo_uploadOp(){
		...
		Tpl::output('count', $count);
		Tpl::output('ac_list',$ac_list);
		Tpl::showpage('circle/XiangCeShangChuan');
	}
	/**
	 * 相册分类添加
	 *
...
//		Tpl::showpage('sns_album_class_add','null_layout');
		Tpl::showpage('circle/xinJianXiangCe');
	}
	/**
	 * 相册保存
	 *
...
public function album_add_saveOp(){
	...
	if ($return) {
		showMessage('新建成功','index.php?act=circle_sns_album');
	}
}
...
//	}
	...
	public function album_pic_delOp(){
		...
		foreach($id as $v){
			...
			$del_status = $model->table('sns_albumpic')->delete($ap_info['ap_id']);
		}
