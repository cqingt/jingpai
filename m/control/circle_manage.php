...
class circle_manageControl extends BaseCircleManageControl{
	...
	public function circle_picOp(){
		$model = Model();
			$update = array();
			$update['circle_id']	= $_GET['c_id'];
			// 上传图片
...
//			showDialog(L('nc_common_op_succ'), 'reload', 'succ');
		...
		//查询圈子图片
		$circle_img = $model->table('circle')->where(array('circle_id'=>$this->c_id))->field('circle_img')->find();
		Tpl::output('circle_img', $circle_img);
//		echo '<!--';
//			print_r($circle_img);
//		echo '-->';
		$this->sidebar_menu('index');
		Tpl::showpage('circle/ziXunHuoDong4');
	}
	/**
	 * 成员管理
	 */
...
//		$this->circleMemberInfo();
		...
		Tpl::showpage('circle/ziXunHuoDong4-2');
	}
	/**
...
//		echo '-->';
		...
		Tpl::showpage('circle/ziXunHuoDong4-3');
	}
	/**
	 * 申请中成员操作
	 */
...
//		$this->circleMemberInfo();
		...
		Tpl::showpage('circle/ziXunHuoDong4-4');
	}
	/**
	 * 分类添加
	 */
	...
	public function class_delOp(){
		...
		$status = Model()->table('circle_thclass')->where($where)->delete();
		if($status){
			showMessage('删除成功');
		}
