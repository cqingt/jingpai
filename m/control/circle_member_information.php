...
class circle_member_informationControl extends BaseMemberControl {
	/**
	 * 用户中心
	 *
	 * @param
	...
	public function memberOp() {
		...
		Tpl::output('html_title','账户信息'.' - '.C('site_name'));
		Tpl::showpage('circle/geRenZiLiao');
	}
	 ...
	 * add time   2016-11-10
	 * */
	public function updaOp(){

		$model_member = Model();
		$member_array['member_name']	= $_POST['uname'];
		$member_array['member_truename']	= $_POST['truename'];
		$member_array['member_sex']	= $_POST['gender'];
		$member_array['member_birthday']	= $_POST['appDate'];
		$member_array['member_qq']	= $_POST['qq'];
		$member_array['member_ww']	= $_POST['ww'];
		$update = $model_member->table('member')->where(array('member_id'=>$_SESSION['member_id']))->update($member_array);
...
//			$update = $model_member->editMember(array('member_id'=>$_SESSION['member_id']),$member_array);
			...
			if($update = $model_member->editMember(array('member_id'=>161813),$member_array)){
					echo 1;
			}else{
					echo 2;
			}
...
//		if (!chksubmit()){
		...
		Tpl::output('menu_sign','profile');
		Tpl::output('menu_sign_url','index.php?act=circle_member_information&op=member');
...
//		Language::read('member_home_member,cut');
		$member_info = Model('member')->getMemberInfoByID(161813,'member_avatar');
