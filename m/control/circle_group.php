...
class circle_groupControl extends BaseCircleThemeControl{
	...
	public function indexOp(){
		...
		if($_GET['order'] && $_GET['desc']){
			$orderby = $_GET['order'].' '.$_GET['desc'];
		}else{
			$orderby = 'is_stick desc,lastspeak_time desc';
		}
		$theme_list = $model->table('circle_theme')->where($where)->order($orderby)->page(20)->select();
		...
		$this->breadcrumd();
		Tpl::showpage('circle/ziXunHuoDong');
	}
	/*
	 * 申请加入
	 * */
	public function applyviewOp(){
		...
		Tpl::showpage('circle/shenQingJiaRu2');
	}
...
//				showDialog($error);
				...
				if(intval($this->circle_info['circle_joinaudit']) == 0){
					...
					showMessage('申请成功',"index.php?act=circle_group&c_id=$this->c_id");
				}else{
	...
	public function group_memberOp(){
		...
		Tpl::showpage('circle/ziXunHuoDong2');
	}
	/**
	 * 圈子成员编辑
	 */
	...
	public function group_goodsOp(){
		...
		Tpl::showpage('circle/ziXunHuoDong3');
	}
	 ...
	 * add time 2016-11-16
	 * */
	public function manage_applyviewOp(){
		Tpl::showpage('circle/C-qq');
	}

	public function manage_applyinfoOp(){
		$model = Model();
		// Circle information
		$this->circleInfo();
		$update = array();
		...
		showMessage('申请成功，请等待审核通过');
	}


	/**
	 * Applied to be an administrator申请管理员
	 */
