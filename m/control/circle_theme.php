...
class circle_themeControl extends BaseCircleThemeControl{
...
//			$insert['thclass_name'] = $thclass_name;
			...
			$themeid = $model->table('circle_theme')->insert($insert);//true
			...
			if ($themeid) {
				...
				if ($_GET['sp'] == 1) {
					...
					$insert['poll_multiple'] = intval($_POST['multiple']);//单多选
					...
					if (!empty($_POST['polloption'])) {
						...
						$insert_array = array();
						$array_polloption = explode('-', $_POST['polloption']);

						foreach ($array_polloption as $val) {
							if ($val == '') continue;
							$option = array();
...
//			$insert['thclass_name'] = $thclass_name;
	...
	*投票
	...
	public function poll_detailOp(){
		...
		$this->breadcrumd();
		Tpl::showpage('circle/xinTouPiao2');
...
//		$goods_array = tidyThemeGoods($goods_array, 'reply_id', 2);
		...
		$member_list = array_under_reset($member_list, 'member_id');
		Tpl::output('member_list', $member_list);
		$new_member = reset($member_list);
		Tpl::output('new_member', $new_member);

		//判断用户是否加入圈子
		$c_id = $_GET['c_id'];
		$is_identity = $model->table('circle_member')->where(array('circle_id'=>$c_id,'member_id' => $_SESSION['member_id']))->count();
		echo'<!--';
			print_r($is_identity);
		echo'-->';
		Tpl::output('is_identity', $is_identity);
		...
		$this->breadcrumd();
		Tpl::showpage('circle/xinHuaTiXiangQingYe');
	}
	...
	public function new_themeOp(){
		...
		if($_GET['sp'] == 1){
			...
			Tpl::showpage('circle/xinTouPiao');
		}else{
			// breadcrumb navigation
			$this->breadcrumd(L('circle_new_theme'));
			Tpl::showpage('circle/xinHuaTi');
		}
	}
	/**
	 * 编辑话题
 ...
 * add time 2016.11.26
 * */
	public function reply_viewOp(){
		$c_id = $_GET['c_id'];
		$t_id = $_GET['t_id'];
		Tpl::output('c_id', $c_id);
		Tpl::output('t_id', $t_id);
		Tpl::showpage('circle/reply');
	}
	/**
	 * 话题回复保存
	 */
	public function save_replyOp(){
		...
		if($_POST){
			...
			if($error != ''){
				...
				$insert['member_name']	= $_SESSION['member_name'];
				//circleCenterCensor
				$insert['reply_content']= $_POST['replycontent'];
				...
				if($reply_id){
					showMessage('发表成功',"index.php?act=circle_theme&op=theme_detail&c_id=$this->c_id&t_id=$this->t_id");
//					if($_GET['type'] == 'adv'){
//						// 插入话题商品
//						if(!empty($_POST['goods'])){
//							$goods_insert = array();
