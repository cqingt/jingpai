...
class circle_member_securityControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        Tpl::output('html_title','账户安全'.' - '.C('site_name'));

		Tpl::showpage('member_security.index');
    }

    /*绑定微信临时二级码、AJAX获取*/
    public function ajaxGetWxewmOp(){
        // $token = file_get_contents('http://m.96567.com/weixin/get_token.php');

        $weixinInfo = new weixinSDK;//微信基类

        $wxewm = new wxewm($weixinInfo->token,$_SESSION['member_id']);

        $ewmImages = $wxewm->getEwm();
    ...
    public function send_bind_emailOp() {
	    ...
	    showDialog('验证邮件已经发送至您的邮箱，请于24小时内登录邮箱并完成验证！','index.php?act=circle_member_information&op=member','succ','',5);
    ...
    public function authOp() {
        ...
        if (chksubmit(false,true)) {
			...
			//写入扩展验证数据 2015-11-10 盛威添加
			$model_member->addMemberCommon(array('member_id'=>$_SESSION['member_id']));
    ...
    public function modify_pwdOp() {
        ...
        //add 修改密码 增加同步中心 xin
        $update	= $model_member->editMember(array('member_id'=>$_SESSION['member_id']),array('member_passwd'=>md5($_POST['password']),'ec_salt'=>''));
        if($update){
            $ToApi = new ToApi();
            $ToApi->update_password($_SESSION['member_name'],md5($_POST['password']));
        }
        //add end
    ...
    public function modify_mobileOp() {
        ...
        if (chksubmit()) {
            ...
            /*2015-11-16 Add is name lt 首次绑定手机送代金卷*/
            $model = Model('voucher');
            $voucherresult = $model->addDaiJingJuan($_SESSION['member_id'],36);
            $vouchersucc = '';
            ...
            // 绑定手机送上级藏豆 20160406 xin
            if($member_info['member_mobile_bind'] != 1){ //如果原来未绑定，为首次绑定
                Model('pushuser_gift')->setUserPoints($_SESSION['member_id']);
            }

            
            showDialog('手机号绑定成功'.$vouchersucc,'index.php?act=member_security&op=index','succ');
        }
    }
