...
class circle_searchControl extends BaseCircleControl{
	...
	public function groupOp(){
		...
		Tpl::showpage('circle/circle_search');
	}

	/*
	 * 圈子 更多
	 * */
	public function circlr_moreOp(){

		$model = Model('circle');
		$count = $model->select();
		$circle_list = $model->page(20,$count)->select();
		if($circle_list){
			foreach($circle_list as $k=>$v){
				$identity = 0;
				if($_SESSION['is_login']){
		...
		$this->circleSEO(L('search_circle'));
		Tpl::showpage('circle/quanZiFenLei');
	}
}
