...
class hudongControl extends mobileHomeControl {
	private $ZtModel;
	...
	public function get_weixin_infoOp(){
		...
		if($weixin_result['subscribe'] == '1'){
			...
			$url = urldecode($_GET['zan_url'])?urldecode($_GET['zan_url']):urlWap('hudong','ad_20160719');
			header("location:$url");
		}else{
			//授权获取微信用户数据
			...
			$url =  urlWap('hudong','ad_20160719_get_user_info');
	...
	public function ad_20160719_get_user_infoOp(){
		...
		$url = urldecode($_SESSION['zan_url'])?urldecode($_SESSION['zan_url']):urlWap('hudong','ad_20160719');
		$_SESSION['zan_url'] = '';
		header("location:$url");
	}
	...
	public function register_showOp(){
		...
		if(chksubmit(true)){
            ...
            $register_info['member_from'] = '点赞送银条(hu_'.($_SESSION['zan_ua']?$_SESSION['zan_ua']:'weixin').')';
