...
class member_returnControl extends mobileMemberControl {
	...
	public function shipOp(){
		...
		if (chksubmit()) {
			...
			if ($state) {
				showWapMessage("保存成功",'index.php?act=member_return&op=index','succ');
			} else {
	...
	public function delayOp(){
		...
		if (chksubmit()) {
			if ($return['seller_state'] != '2' || $return['goods_state'] != '3') {//检查状态,防止页面刷新不及时造成数据错误
				showWapMessage("参数错误",'reload','error','CUR_DIALOG.close();');
			}
			$refund_array = array();
			$refund_array['delay_time'] = time();
			$refund_array['goods_state'] = '2';
			$state = $model_refund->editRefundReturn($condition, $refund_array);
			if ($state) {
				showWapMessage("保存成功",'reload','succ','CUR_DIALOG.close();');
			} else {
				showWapMessage("保存失败",'reload','error','CUR_DIALOG.close();');
			}
		}
		$model_trade = Model('trade');
		$return_delay = $model_trade->getMaxDay('return_delay');//发货默认5天后才能选择没收到
