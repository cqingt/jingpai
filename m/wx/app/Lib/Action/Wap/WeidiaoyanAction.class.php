<?php
class WeidiaoyanAction extends BaseAction{
	public $token;
	public $wecha_id;
	...
	public function index(){
		...
		if (IS_POST){
			...
			$row['time']=time();
			$this->weidiaoyan_value_model->add($row);
			$this->redirect(U('Weidiaoyan/index',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'id'=>$thisForm['id'],'success'=>1)));
		}else {
			//判断是否提交过信息了
			$submitInfo=$this->weidiaoyan_value_model->where(array('wecha_id'=>$this->wecha_id,'formid'=>$thisForm['id']))->find();
			if ($submitInfo){
				...
				$submitted=1;
				//二维码图片
				$imgSrc=generateQRfromGoogle(C('site_url').'/index.php?g=Wap&m=Weidiaoyan&a=submitInfo&token='.$this->token.'&wecha_id='.$this->wecha_id.'&id='.$thisForm['id']);
