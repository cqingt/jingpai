...
class ProductAction extends BaseAction{
	...
	public function orderCart(){
		...
		//支付宝配置
        $alipay_m_config = M('alipay_m_config')->where(array(
            'token' => $this->token
        ))->find();
        $this->assign('alipay_m_config', $alipay_m_config);
		//原官方版支付宝配置
...
// 结束
			...
			if ($alipay_m_config['open']){
				$this->redirect(U('Alipay_m_/pay',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'success'=>1,'price'=>$totalFee,'orderName'=>$orderName,'orderid'=>$orderid)));
			}else {
