...
<div class="content" style="width:920px; background:none; margin-left:275px; border:none; margin-bottom:30px;" >
          ...
          <form method="post"   action="<?php echo U('Ewm/doUpContent');?>"  enctype="multipart/form-data" >
          <div class="msgWrap form">

<input type="hidden" name="C_Id" value="<?php echo ($result['C_Id']); ?>">
<input type="hidden" name="C_Eid" value="<?php echo ($result['C_Eid']); ?>">
...
<TR>
  ...
  <TD id="selectClass">
      ...
      <option value="1" <?php if(!$result['C_Gid']): ?>selected="selected"<?php endif; ?>>自定义回复</option>
      <option value="99" <?php if($result['C_Gid']): ?>selected="selected"<?php endif; ?>>关键词回复</option>
    </select>

    <select onchange="selectGuanjiancifunction()"  name="guanjianci" id="guanjianci">
    <option value="">请选择</option>
    <?php if(is_array($ci)): foreach($ci as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if($result['C_Gid'] == $v['id']): ?>selected="selected"<?php endif; ?>><?php echo ($v["keyword"]); ?></option><?php endforeach; endif; ?>
               ...
               <TR>
                  ...
                  <TD><input required="required" type="input" class="px" id="C_Name" value="<?php echo ($result['C_Name']); ?>"    name="C_Name" style="width:580px;"> </TD>
                </TR>
                <TR>
                  <TH valign="top"><label for="text">简介：</label></TH>
                  <TD><textarea  class="px" id="Hfcontent"  name="C_Content" style="width:580px;  height:100px"><?php echo ($result['C_Content']); ?></textarea><br />限制200字内
</TD>
                </TR>

				<TR>
                  <TH valign="top"><label for="pic">封面图片地址：</label></TH>
                  <TD><input class="px" id="pic"  name="pic" value="<?php echo ($result['C_ImgUrl']); ?>" style="width:580px;"  />  <script src="<?php echo C('site_url');?>/tpl/static/upyun.js"></script><a href="###" onclick="upyunPicUpload('pic',700,420,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('pic')">预览</a>
</TD>
                </TR>
                 ...
                 <TR>
                  ...
                  <TD><input type="input" class="px" id="url" value="<?php echo ($result['C_Url']); ?>" name="C_Url" id="C_Url" style="width:580px;"><br /><span class="red">如需跳转到其他网址，请在这里填写网址(例如：http://baidu.com，记住必须有http://)</span></TD>
                </TR>
...
<script>
  function selClass(){
    ...
    if(v == 99){
      var g = "<?php echo ($result['C_Gid']); ?>";
      selectGuanjiancifunction(g);
      $("#guanjianci").val(g);
    }else if(v == 1){
      $("#guanjianci").val('');
    }
  ...
  function selectGuanjiancifunction(id){
    if(id){
      var v = id;
    }else{
