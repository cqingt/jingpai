...
<h3 class="clearfix">收货人信息 <span class="btn-s btn-prink-s fright buys1-edit-address buys1-edit-btn" id="edit_reciver">修改</span></h3>
...
<ul class="buys-ycnt buys1-hide-detail">
		...
		<li class="clearfix">
			...
			<div class="value fleft" id="address"><?php echo intval($output['address_info']['dlyp_id']) ? '[自提服务站] ' : '';?><?php echo $output['address_info']['area_info'],$output['address_info']['address'];?></div>
		</li>
		<li class="clearfix">
			<span class="key fleft">联系电话：</span>
			<div class="value fleft" id="mob_phone"><?php echo $output['address_info']['mob_phone'] ? $output['address_info']['mob_phone'] : $output['address_info']['tel_phone'];?></div>
		</li>
   </ul>
...
function hideAddrList(addr_id,true_name,address,phone) {
	...
	$("#addr_list").html('<ul class="buys-ycnt buys1-hide-detail"><li class="clearfix"><span class="key fleft">姓名：</span><div class="value fleft" id="true_name">'+true_name+'</div></li><li class="clearfix"><span class="key fleft">详细地址：</span><div class="value fleft" id="address">'+address+'</div></li><li class="clearfix"><span class="key fleft">联系电话：</span><div class="value fleft" id="mob_phone">'+phone+'</div></li></ul>');
...
$('#edit_reciver').on('click',function(){
    ...
    $('#addr_list').load(SITEURL+'/index.php?act=member_buy&op=load_addr');
});
//异步显示每个店铺运费 city_id计算运费area_id计算是否支持货到付款
function showShippingPrice(city_id,area_id) {
	$('#buy_city_id').val('');
    $.post(SITEURL + '/index.php?act=member_buy&op=change_addr', {'freight_hash':'<?php echo $output['freight_hash'];?>',city_id:city_id,'area_id':area_id}, function(data){
