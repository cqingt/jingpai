...
<script src="<?php echo MOBILE_TEMPLATES_URL;?>/js/swipe.js"></script>
<script>
...
<div class="m_w">
	 ...
	 $(function(){
		$(window).scroll(function(){
			if($(window).scrollTop()>10){
				$('.m_header0505').css('background','#e04346');
			}else{
				$('.m_header0505').css('background','none');
			}
		})
	 })
  ...
  <div class="m_w clearfix">
    <div style="-webkit-transform:translate3d(0,0,0);">
      <div id="banner_box" class="box_swipe">
       <ul>
        <?php foreach($output['adv_list'] as $k=>$v){ ?>
          <li>
             <a onClick="return false;" href="<?php echo $v['data'];?>"><img src="<?php echo $v['image'];?>" alt=""></a>
          </li>
        <?php } ?>
       </ul>
        <ol>
...
.box_swipe>ol{height:24px;position: relative;z-index:10;margin-top:-25px;text-align:center;padding-right:20px;background-color:rgba(0,0,0,0.5);}
...
<?php } ?>
	...
	<?php $getadvImg = getadvImg(1080);?>
    <div class="floor">
		<a href="<?php echo $getadvImg['Href']?>">
    	 	<img class="index-ad" src="<?php echo $getadvImg['Img']?>" alt="">
    	</a>
    </div>
				...
				<% for (var i in item) { %>
                  <div class="goods-item">
                       <a href="index.php?act=goods&op=index&goods_id=<%= item[i].goods_id %>">
                          ...
                          <div class="goods-item-price"><% if(item[i].goods_price  >= 1){ %>￥<%= item[i].goods_price %> <% }else{ %> 咨询客服 <% } %></div>
                       </a>
                  </div>
