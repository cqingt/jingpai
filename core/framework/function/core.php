 ...
 */
function showEditor($id, $value='', $width='700px', $height='300px', $style='visibility:hidden;',$upload_state="true", $media_open=false, $type='all',$upload_url=''){
	//是否开启多媒体
	$media = '';
	if ($media_open){
		$media = ", 'flash', 'media'";
    ...
    /* Add is name lt 2016-05-09 单独添加一个上传文件地址、解决跨域问题*/
...
function ncPriceFormatForList($price) {
    if ($price >= 10000) {
       return L('currency').number_format(floor($price/100)/100,2,'.','').L('ten_thousand');
    } else {
 ...
 *
 */
function getadvImg($ap_id = 0){
	$ap_id = intval($ap_id);
	$ap_info = Model('adv')->getApById($ap_id);
	if (!$ap_info)return;
	$num = count($ap_info['adv_list']);
	$key = mt_rand(0,$num-1);
	$pic_content = unserialize($ap_info['adv_list'][$key]['adv_content']);
	$content['Img'] = UPLOAD_SITE_URL."/".ATTACH_ADV."/".$pic_content['adv_pic'];
	$content['Href'] = "http://".$pic_content['adv_pic_url']."";
    $content['is_use'] = $ap_info['is_use'];
	return $content;
}
...
function url($act = '', $op = '', $args = array(), $model = false, $site_url = ''){
	$noModelArr = array('member_article','publish','member_picture');
    //伪静态文件扩展名
    $ext = '.html';
    //入口文件名
    $file = 'index.php';
//    $site_url = empty($site_url) ? SHOP_SITE_URL : $site_url;
    ...
    if ($model && !in_array($act,$noModelArr)) {
        //伪静态模式
        $url_perfix = "{$act}-{$op}";
        if (!empty($args)){
            $url_perfix .= '-';
    ...
    $reg_match_from = array(
    	'/^article-article-ac_id-([0-9]+)-curpage-([0-9]+)\.html$/',
        '/^category-index\.html$/',
        ...
        '/^article_cate-ac_id-([0-9]+)-curpage-([0-9]+).html$/',
		...
		'/^index-auction-id-(\d+)\.html$/',
		'/^index-theme-tid-(\d+).html$/',
		'/^artist-index-artist_id-(\d+).html$/',
		'/^index-more.html$/',
        '/^index-auction_more.html$/',
        

        );
    $reg_match_to = array(
    	'article_cate-\\1-\\2.html',
        'category.html',
        'goods-\\1.html',
        'shop-\\1.html',
        ...
        'article_cate-\\1-\\2.html',
        'document-\\1.html',
        'category-\\1-\\2-\\3-\\4-\\5-\\6-\\7-\\8-\\9.html',
        ...
        'comments-\\1-\\2-\\3.html',
		'auction-\\1.html',
		'auction-theme-tid-\\1.html',
		'artist-\\1.html',
		'auction-more.html',
        'auction-auction_more.html',
    ...
    if(URL_MODEL && in_array($act.':'.$op, $rewrite_item)) {
        ...
        switch ($act.':'.$op) {
            ...
            case 'article:article':
                $tpl_args['ac_id'] = empty($args['ac_id']) ? 1 : $args['ac_id'];
...
function urlCMS($act = '', $op = '', $args = array()){
    return url($act, $op, $args, URL_MODEL, CMS_SITE_URL);
}
/**
...
function urlCircle($act = '', $op = '', $args = array()){
    return url($act, $op, $args, URL_MODEL, CIRCLE_SITE_URL);
}
/**
...
function urlMicroshop($act = '', $op = '', $args = array()){
    return url($act, $op, $args, URL_MODEL, MICROSHOP_SITE_URL);
}
/**
 * 验证是否为平台店铺
 *
...
/**
 * 递归模式删除导入分类中最后为空值的数组元素
 */
function delArrayLastNull($arr){
	if(!is_array($arr)){ return $arr; }
	if($arr[count($arr)-1]==''){
		unset($arr[count($arr)-1]);
		$arr = delArrayLastNull($arr);
	}
	return $arr;
}
 ...
 * 获取文章列表页URL
 */
function getCMSListsUrl($class_id) {
	//盛威修改 2015-11-9
	if(URL_MODEL){
		return CMS_SITE_URL.DS.'tzxy_cat-'.$class_id.".html";
	}else{
		return CMS_SITE_URL.DS.'index.php?act=article&op=article_list&class_id='.$class_id;
	}
	//原始代码：return CMS_SITE_URL.DS.'index.php?act=article&op=article_list&class_id='.$class_id;
}

/**
 * 乐拍使用的URL链接函数，强制使用动态传参数模式
 *
 * @param string $act control文件名
 * @param string $op op方法名
 * @param array $args URL其它参数
 * @return string
 */
function urlLepai($act = '', $op = '', $args = array()){
    //return url($act, $op, $args, URL_MODEL, BASE_SITE_URL);
	return url($act, $op, $args, false, LEPAI_SITE_URL);
	//return url($act, $op, $args, false, 'http://localhost/paimai/lepai/');
}


function pmUrl($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, LEPAI_BASE_URL);
}

/**
 * 清除html标签，清除空格，制表符等符号
 * 截取字符串
 */

function html_substr_word($value,$len=30){
...
function JiaMiMobile($str){
    ...
    $url = "http://123.57.49.61:8888/sms/code.php?m=".$str;
 ...
 * @ 手机号解密
 */
function JieMiMobile($str){

	$http = strtolower($_SERVER['HTTP_HOST']);
	$act = strtolower('system.96567.com');
    ...
    $context = stream_context_create($opts);
    $url = "http://123.57.49.61:8888/sms/code.php?status=1&m=".$str;
    $output = file_get_contents($url, false, $context);
    return $output;
}
...
function accessLog(){
	$path = BASE_PATH . '/../';
	$logStr = date('Y-m-d H:i:s').' '.getIp().' '.$_SERVER['REQUEST_URI']."\n";
	$filename = 'log/access/'.date('Y-m-d').'.log';
	file_put_contents($path.$filename,$logStr,FILE_APPEND);
}
...
function pushBaiduUrl($urls){
$api = 'http://data.zz.baidu.com/urls?site=www.96567.com&token=ri1hqMPwDd4Fbji7';
$ch = curl_init();
$options =  array(
    CURLOPT_URL => $api,
    CURLOPT_POST => true,
