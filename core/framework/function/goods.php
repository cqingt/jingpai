...
function thumb($goods = array(), $type = ''){
    ...
    /*2015-09-08 修改添加 以前没有的代码*/
    $img = strstr($goods['goods_image'],'images');
    $img_array = explode('/',$img);
    if($img_array['0'] == 'images'){
        $thumb_host = UPLOAD_SITE_URL.'/'.ATTACH_GOODS;
        $store_id = $goods['store_id'];

        // var_dump(file_exists('http://image.96567.com/shop/store/goods/3/images/201105/goods_img/64_G_1304625687738.jpg'));

        // if (file_exists($thumb_host.'/'.$store_id.'/'.$goods['goods_image'])){
        //     return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
        // }
        // $type?$res_file = str_replace('.','_'.$type.'.', $goods['goods_image']):$res_file = $goods['goods_image'];
        /*如果没有该缩略图则显示原图*/
        // if(file_exists($thumb_host.'/'.$store_id.'/'.$res_file)){
        // return $thumb_host.'/'.$store_id.'/'.$res_file;
        // }else{
        $imgUrl = $thumb_host.'/'.$store_id.'/'.$goods['goods_image'];
...
function cthumb($file, $type = '', $store_id = false) {
    ...
    if($img_array['0'] == 'images'){
        ...
        //仅限于原EC产品图片
        //找出产品ID
        // $store_array = explode('_',end($img_array));
        // $store_id = $store_array['0'];
        return $thumb_host.'/3/'.$file;
    }
 ...
 * @return string
 */
function pthumb($image_name = '', $type = ''){
    if (!in_array($type, array('small','mid','max'))) $type = 'small';
    if (empty($image_name)){
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage('240');
    }
    list($base_name, $ext) = explode('.', $image_name);
    list($store_id) = explode('_', $base_name);
    $file_path = ATTACH_PAIMAI.DS.$store_id.DS.$base_name.'_'.$type.'.'.$ext;
    if(!file_exists(BASE_UPLOAD_PATH.DS.$file_path)) {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage('240');
    }
    return UPLOAD_SITE_URL.DS.$file_path;
...
function orderPaymentName($payment_code) {
    return str_replace(
            array('offline','online','alipay','tenpay','chinabank','predeposit','unionpay','bank','wxpay','CDduihuan'),
            array('货到付款','在线付款','支付宝','财付通','网银在线','站内余额支付','银联在线支付','银行转账','微信支付','藏豆兑换礼品'),
            $payment_code);
}
    ...
    return str_replace(
        ...
        */
        // add 秒杀模块 xin
            ...
            array('','抢购','限时折扣','优惠套装','赠品','秒杀','会员特价','推荐优惠'),
        // add end
            $goods_type);
}
