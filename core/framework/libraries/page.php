...
class Page{
	...
	public function show($style = null){
		...
		switch ($this->style) {
			...
			case '6':
				if ($this->getNowPage() <= 1){
					$html_page .= '<span class="s2">'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</span>';
				}else {
					$html_page .= '<span class="s2"><a class="demo" href="'. $this->page_url . ($this->getNowPage()-1) .'">'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</a></span>';
				}
				if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
					$html_page .= '<span class="s2">'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</span>';
				}else {
					$html_page .= '<span class="s2"><a class="demo" href="'. $this->page_url . ($this->getNowPage()+1) .'">'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</a></span>';
				}
				break;
            case '7':
                $cms_page_url = CMS_SITE_URL.DS.'tzxy_cat-'.intval($_GET['class_id'])."-";
                $html_page .= '<ul>';
                if ($this->getNowPage() <= 1){
                    $html_page .= '<li>'.$this->left_inside_a_html.$this->pre_home.$this->right_inside_a_html.'</li>';
                    $html_page .= '<li>'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</li>';
                }else {
                    $html_page .= '<li><a class="demo" href="'. $cms_page_url .'1.html">'.$this->left_inside_a_html.$this->pre_home.$this->right_inside_a_html.'</a></li>';
                    $html_page .= '<li><a class="demo" href="'. $cms_page_url . ($this->getNowPage()-1) .'.html">'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</a></li>';
                }
                //$html_page .= $this->getNowBar();
                if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
                    $html_page .= '<li>'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</li>';
                    ...
                    $html_page .= '<li><a class="demo" href="'. $cms_page_url . ($this->getNowPage()+1) .'.html">'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</a></li>';
                    $html_page .= '<li><a class="demo" href="'. $cms_page_url . $this->getTotalPage() .'.html">'.$this->left_inside_a_html.$this->pre_last.$this->right_inside_a_html.'</a></li>';
                }
                $html_page .= '</ul>';
                break;

            case '88':
				$html_page .= '<div class="pagination mt10">';

				if ($this->getNowPage() <= 1){
					$html_page .= "<a  has_more='false' class='next-page disabled'>上一页</a>";
				}else {
					$html_page .= "<a href='".$this->page_url.($this->now_page-1)."' has_more='false' class='next-page '>上一页</a>";
				}

				$html_page .= '<div class="new-tbl-cell new-p-re">';

				$html_page .= '<div class="new-a-page">';

				$html_page .= "<span class='new-open'>".$this->getNowPage()."/".$this->getTotalPage()."</span>";

				$html_page .= '</div>';

				$html_page .= '<select class="new-select" onchange="window.location.href=this.value;">';
				...
				if($i==$this->now_page){
					...
					$html_page .= "<option value='".$this->page_url.$i."' selected='selected'>".$i."</option>";

				}else{

					$html_page .= "<option value='".$this->page_url.$i."' >".$i."</option>";
				...
				if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
					$html_page .= "<a has_more='false' class='next-page disabled'>下一页</a>";
				}else {
					$html_page .= "<a href='".$this->page_url.($this->now_page+1)."' has_more='false' class='next-page '>下一页</a>";
				}
