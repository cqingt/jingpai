...
class UploadFile{
	...
	public function upfile($field){
		...
		if ($this->thumb_width && $this->thumb_height && $this->thumb_ext){
			...
			// ($this->thumb_width > $this->thumb_height)?$this->thumb_height = $this->thumb_width:$this->thumb_width = $this->thumb_height;
...
//				}
		...
		if(@move_uploaded_file($this->upload_file['tmp_name'],BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name)){
			...
			if ($ifresize){
				...
				for ($i=0;$i<count($thumb_width);$i++){
					$resizeImage->newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling,$this->zhiding);
					if ($i==0) {
			...
			//如果图片存在则存入阿里云
			if(file_exists(BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name)){
				$oss = new aili_yun_oss;//实例化云
				//把双杠替换成单杠
				$DBASE_UPLOAD_PATH = str_replace('//','/',BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name);

				$aliyun_state = $oss->yun_upload_img($DBASE_UPLOAD_PATH);

				file_put_contents('aliyun_state.txt',print_r($aliyun_state,true),FILE_APPEND);
			}
			return true;
		}else {
