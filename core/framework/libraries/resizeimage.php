...
class ResizeImage{
	 ...
	 */
	public function newImg($img, $wid, $hei,$scale,$small_ext="_small.",$dst_img='',$filling = true,$zhiding = false){
...
// 		$this->config['thumb_quality'] = (C('thumb_quality') ? C('thumb_quality') : 100);
		...
		if ($scale != 360){
			...
			file_put_contents('filling.txt',print_r($scale.'--',true),FILE_APPEND);

			file_put_contents('filling.txt',print_r($zhiding.'++',true),FILE_APPEND);


			if($scale > 240 && $zhiding == true){
...
//			imageCopyMerge($newimg, $snewimg,$dst_x,$dst_y,0,0,$this->width*$wh/$this->height,$wh,100);
			...
			//如果缩略图片存在则存入阿里云
			if(is_file($this->dstimg)){
				$oss = new aili_yun_oss;//实例化云
				$oss->yun_upload_img($this->dstimg);
			}
