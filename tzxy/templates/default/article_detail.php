    ...
    <article class="article-detail-content">
      ...
      <section class="article-body" id="text"><?php echo $output['article_detail']['article_content'];?>
        ...
        <div class="article-tag"><?php echo $lang['cms_keyword'];?><?php echo $lang['nc_colon'];?>
          ...
          <?php if($output['article_detail']['article_class_id']==69) { ?>
            <a href="<?php echo CMS_SITE_URL.DS;?>index.php?act=article&op=article_list&class_id=69&keywords=<?php echo $value;?>" target="_blank"><?php echo $value;?></a>
          <?php } else { ?>
      ...
      <div class="cp-recommend">
          <h2>
             <strong>藏品推荐</strong>
          </h2>
          <ul>
             <?php foreach($output['tuijian'] as $val) { ?>
             <li>
               <a href="<?php echo urlShop('goods','index',array('goods_id'=> $val['goods_id']));?>" target="_blank">
                  <div class="cp-recimg">
                    <img src="<?php echo $val['goods_pic'];?>" alt="">
                  </div>
                  <h4><?php echo $val['goods_name']; ?></h4>
                  <p><?php if($val['promotion_type']){ ?><em>活动价：</em><?php }else{ ?><em>商城价：</em><?php } ?><strong>
				  <?php if($val['promotion_price']>0){ ?>
						<?php echo ncPriceFormatForList($val['promotion_price']);?>
				  <?php }else{ ?>
					  <?php if($val['goods_price']>10000) { ?>
					  <?php echo ncPriceFormatForList($val['goods_price']);?>万元
					  <?php } elseif($val['goods_price']==0) {?>
					  咨询客服
					  <?php } else {?>
      ...
      <div class="article-below-ad">
           <?php echo loadadv(1079);?>
      </div>
...
<script>
    (function(){
        var src = document.location.protocol +'//js.passport.qihucdn.com/11.0.1.js?cef17df3d89c9cf4082a82f10ae02307';
