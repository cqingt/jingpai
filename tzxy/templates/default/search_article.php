  ...
  <div class="mainbox">
    ...
    <ul class="article-search-list">
	  ...
	  <!--根据修改文档隐藏封面图
		...
		--->
        <h3 class="article-title" style="float: left;"> <a href="<?php echo $article_url;?>" target="_blank"> <?php echo $value['article_title'];?> </a> </h3>
        <div class="article-sub" style="float: left;"><span class="author"><?php echo $lang['cms_text_publisher'];?><?php echo $lang['nc_colon'];?><?php echo empty($value['article_author'])?$lang['cms_text_guest']:$value['article_author'];?></span><span class="source"><?php echo $lang['cms_text_origin'];?><?php echo $lang['nc_colon'];?> <a href="<?php echo empty($value['article_origin_address'])?CMS_SITE_URL:$value['article_origin_address'];?>" target="_blank"> <?php echo empty($value['article_origin'])?C('site_name'):$value['article_origin'];?> </a></span><span class="date"><?php echo date('Y-m-d',$value['article_publish_time']);?></span></div>
        <div class="article-tag" style="float: left;"><?php echo $lang['cms_keyword'];?><?php echo $lang['nc_colon'];?>
