...
class chinabank{
	 ...
	 * @var string
	 */
	private $gateway   = 'https://tmapi.jdpay.com/PayGate';
	/**
	 * 支付接口标识
	 *
	 * @var string
	...
	public function submit(){
        ...
        $notify_url = "[url:=".SHOP_SITE_URL."/api/payment/chinabank/notify_url.php]";	// 请填写返回url,通知url
    ...
    $v_md5info = strtoupper(md5($text));                            //md5函数加密并转化成大写字母
		...
		'remark2'    	  => $notify_url,                          									// 备注字段2
		);

        if($this->bank_code){
            $parameter['pmode_id'] = $this->bank_code; // 银行代码，支持直接跳转到网银
        }

		$html = '<html><head></head><body>';
		$html .= '<form method="post" name="E_FORM" action="https://tmapi.jdpay.com/PayGate">';
