		...
		if (!empty($favorites_list) && is_array($favorites_list)){
			...
			if (!empty($goods_list) && is_array($goods_list)){
				...
				foreach ($goods_list as $key=>$fav){
					$fav_id = $fav['goods_id'];
					//获取促销信息
					$goods_info = Model('goods')->getChuXiao($fav_id);
					if($goods_info['promotion_type'] && $goods_info['promotion_price']){
						$fav['promotion_type'] = $goods_info['promotion_type'];
						$fav['promotion_price'] = $goods_info['promotion_price'];
					}
...
Tpl::output('html_title','我的收藏'.' - '.C('site_name'));
	...
	private function profile_menu($menu_type,$menu_key='') {
		$menu_array = array(
			...
			3=>array('menu_key'=>'memberShoucang','menu_name'=>'艺术家收藏', 'menu_url'=>'index.php?act=member_favorites&op=memberShoucang')
		);
	...
	public function memberShoucangOp(){
		...
		self::profile_menu('memberShoucang','memberShoucang');
    ...
    public function delShoucangOp(){
        ...
        if($model_artist->delArtistShoucang($condition)){
			showDialog("操作成功！",'index.php?act=member_favorites&op=memberShoucang','succ');
        }else{
