...
class store_orderControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        foreach ($order_list as $key => $order_info) {
			...
			//盛威添加 2015-11-11
			$order_list[$key]['extend_order_common']['reciver_info']['phone'] = JieMiMobile($order_info['extend_order_common']['reciver_info']['phone']);
			$order_list[$key]['extend_order_common']['reciver_info']['mob_phone'] = JieMiMobile($order_info['extend_order_common']['reciver_info']['mob_phone']);
        }
    ...
    public function show_orderOp() {
		...
		//$order_info['extend_order_common']['reciver_info']['mob_phone']
        if (empty($order_info)) {
...
foreach ($order_info['goods_list'] as &$goods) {
    ...
    $goods_gc_id = $goods_model->getGoodsInfoByID($goods['goods_id'] , 'gc_id_1,gc_id_2,gc_id_3');
...
/* End */
    ...
    private function _order_spay_price($order_info, $post) {
        ...
        if(!chksubmit()) {
			...
			$goods_pay_price = $post['goods_pay_price'];
			$goods_amount = 0;
			if($goods_pay_price){
				foreach($goods_pay_price as $k=>$v){
					Model('order_goods')->where(array('rec_id'=>$k,'order_id'=>$order_info['order_id']))->update(array('goods_pay_price'=>$v));
					$goods_amount += $v;
					
				}
				return $logic_order->changeOrderSpayPrice($order_info,'seller',$_SESSION['member_name'],$goods_amount);
			}else{
