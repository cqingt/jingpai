...
class connectControl extends BaseHomeControl{
	public function __construct(){
		...
		//盛威添加 2015-11-25
		$arr = array();
		parse_str($_COOKIE['qqjs_'.$_COOKIE['cid']],$arr);
		$_SESSION['openid'] = $arr['openid'];
		$_SESSION["appid"] = $arr['appid'];
		$_SESSION["appkey"] = $arr['appkey'];
	...
	public function registerOp(){
		...
		if (chksubmit()){
			...
			showMessage(Language::get('nc_common_save_succ'),BASE_SITE_URL);
		}else {
			//检查登录状态
			...
			if($result) {
				...
				showMessage(Language::get('login_usersave_regist_fail'),BASE_SITE_URL.'/index.php?act=login&op=register','html','error');//"会员注册失败"
			}
		}
	}
	/**
	...
	public function autologin(){
		...
		if (is_array($member_info) && count($member_info)>0){
			...
			showMessage($success_message,BASE_SITE_URL);
		}
	}
	/**
	 * 更换绑定QQ号码
