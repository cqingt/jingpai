...
class store_promotion_bundlingControl extends BaseSellerControl {
    ...
    public function bundling_addOp() {
        ...
        if (chksubmit()) {
            ...
            $data['bl_state'] = $bl_state = intval($_POST['state']);
            ...
            if (!empty($_POST['goods']) && is_array($_POST['goods'])) {
                foreach ($_POST['goods'] as $key => $val) {
                    ...
                    //检查商品是否已经参加同时段活动,$bl_state为活动开启状态 add xin 20151107
                    if($goods_info['sku_lock'] == 1 && $bl_state == 1) {
                        if($_POST['bundling_id'] < 1){ //小于1为新增，增加失败删除刚刚添加的套装
                            Model('p_bundling')->delBundling($return, $_SESSION['store_id']);
                        }
                        showDialog($goods_info['goods_name'].'已经参加其他未结束活动！', '', '', 'error');
                    }
                    //add end
            ...
            // 插入数据
            $return = $model_bundling->addBundlingGoodsAll($data_goods,$bl_state);//add xin 20151107 增加参数$bl_state判定优惠套装状态
