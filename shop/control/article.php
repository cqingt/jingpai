...
class articleControl extends BaseHomeControl {
	...
	public function articleOp(){
		...
		Tpl::output('show_page',$page->show(5));
		/**
		 * 最新文章列表
		 */
	...
	public function showOp(){
		...
		if(empty($article) || !is_array($article) || $article['article_show']=='0'){
			showMessage($lang['article_show_not_exists'],'http://www.96567.com/','html','error');//'该文章并不存在'
		}
		...
		if(empty($article_class) || !is_array($article_class)){
			showMessage($lang['article_show_delete'],'http://www.96567.com/','html','error');//'该文章已随所属类别被删除'
		}
		$default_count	= 5;//定义最新文章列表显示文章的数量
		/**
		 * 分类导航
