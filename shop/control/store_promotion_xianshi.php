...
class store_promotion_xianshiControl extends BaseSellerControl {
    ...
    public function xianshi_quota_add_saveOp() {
        ...
        $this->recordSellerLog('购买'.$xianshi_quota_quantity.'份限时折扣套餐，单价'.$current_price.'元');
    ...
    public function xianshi_goods_addOp() {
        ...
        $xianshi_id = intval($_POST['xianshi_id']);
        $xianshi_price = intval($_POST['xianshi_price']);

        if($_POST['xianshi_price'] < 1) {
            $data['result'] = false;
            $data['message'] = '折扣价格必须整数且不能小于1元';
    ...
    public function xianshi_goods_price_editOp() {
        ...
        $xianshi_price = $_POST['xianshi_price'];
