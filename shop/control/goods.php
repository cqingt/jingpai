...
class goodsControl extends BaseGoodsControl {
    ...
    public function indexOp() {
		...
		// by abc.com
//		$rs = $model_goods->getGoodsList(array('goods_commonid'=>$goods_info['goods_commonid']),'*,(SELECT SUM(og.goods_num) FROM shop_order_goods as og WHERE og.goods_id=`shop_goods`.goods_id) as goods_salenum');
//		$count = 0;
//		foreach($rs as $v){
//			$count += $v['goods_salenum'];
//		}
		$goods_info['goods_salenum'] = @intval(Model('order')->getOrderGoodsSum($goods_info['goods_id']));
        //存入商品购买总数
        //$model_goods->editGoodsById(array('goods_salenum'=>$count),$goods_info['goods_id']);
		//  添加 end
        ...
        $consult_count = count($consult_list);
        ...
        Tpl::output('_360so', true);

		if($goods_info['gc_id_1'] == 79 && $goods_info['store_id'] == 22){
			$_GET['op'] = 'ShGoods';
			Tpl::output('YiShu', true);
			//获取艺术家个人信息
			$getOne =  Model('artist')->getOneArtist($goods_info['artist_id']);
			Tpl::output('getOne', $getOne);
			//获取当前艺术的产品数量以及销量
			$OneGoods =  Model('artist')->getOneGoods($goods_info['artist_id']);
			Tpl::output('OneGoods', $OneGoods);
			$storeInfo = Model('store')->getStoreInfoByID($goods_info['store_id']); //获取店铺信息
			Tpl::output('store_yijia',$storeInfo['store_yijia']);
			//获取登陆用户手机号
			...
			$this->get_new_miaoshao_listOp("ShuHua");
     ...
     * 立即分享
     */
    public function share_noticeOp() {
        if (!$_SESSION['is_login'] ){
            showMessage('请登录后在分享', '', '', 'error');
        }
        Tpl::showpage('share_notice.submit', 'null_layout');
    }
    ...
    public function arrival_notice_submitOp() {
        ...
        if(!preg_match("/1[34578]{1}\d{9}$/",$_POST['mobile'])){
            showDialog('手机号无效', 'reload');
        }
        // 验证商品数是否充足
        $goods_info = Model('goods')->getGoodsInfoByID($goods_id, 'goods_id,goods_name,store_id,goods_storage,goods_state');
	...
	private function get_new_miaoshao_listOp($type=''){
			...
			$condition = array();
			$condition['state'] = '20';
			$condition['start_time'] = array(array('egt', strtotime(date('Y-m-d',time()).' '.$YH.':0:0')),array('lt', strtotime(date('Y-m-d')) + 86400),'and');
			if($type == 'ShuHua'){
				$goods_list = Model()->table('goods_common')->where(array("gc_id_1"=>'79'))->limit(999999999)->select();
				if($goods_list){
					$goods_idArr = array();
			...
			foreach($miaosha_info as $k=>$v){
				if($v['start_time'] > strtotime(date('Y-m-d')) && $v['start_time'] < (strtotime(date('Y-m-d')) + 86400)){
				   ...
				   if(date('H',time()) >= $start_hour && date('H',time()) < $end_hour && $miaosha_list[$start_hour][$v['miaosha_id']]['shengyukucun'] > 0){
						...
						unset($miaosha_list[$start_hour][$v['miaosha_id']]);
				   } else {
		 ...
		 Tpl::output('miaosha',$miaosha_list[$qv]);
