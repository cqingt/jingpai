...
class ltControl extends BaseHomeControl {
    ...
    public function jd_shOp(){

        $sql = 'SELECT B_ID FROM sw_customer WHERE B_LaiYuan IN(22,33) AND B_Time <= 1480521599';
        $crm_result = CrmDb::getAll($sql);

        $crm_user_id = '';
        foreach ($crm_result as $key => $value) {
            $crm_user_id .= $value['B_ID'].',';
        }

        $crm_user_id = trim($crm_user_id,',');


        $condition['MemberID'] = array('in',$crm_user_id);

        $yw_order = Model()->table('yw_info')->field('order_sn,orderid')->where($condition)->select();

        $crm_order_id = '';
        foreach ($yw_order as $key => $value) {
            $crm_order_id .= $value['orderid'].',';
        }
        $crm_order_id = trim($crm_order_id,',');


        $condition_list['order_goods.order_id'] = array('in',$crm_order_id);
        $condition_list['order.order_state'] = array('gt',10);
        $condition_list['goods.gc_id_1'] = 79;

        $order_list = Model()->table('order_goods,order,goods')
        ->join('left')
        ->on('order_goods.order_id=order.order_id,order_goods.goods_id=goods.goods_id')
        ->where($condition_list)
        ->field('order_goods.*,order.order_sn')
        // ->group('goods.gc_id_2')
        ->page($page)
        ->order($order)
        ->select();
    ...
    public function jd_Op(){

        $sql = 'SELECT order_sn,S_ClerkName,S_OrderAmount,S_PaymentTime FROM sw_order_settlement WHERE S_BID IN(SELECT B_ID FROM sw_customer WHERE B_LaiYuan = 33) AND S_PaymentTime <= 1480521599 ORDER BY S_OrderAmount DESC';
        $crm_result = CrmDb::getAll($sql);
        ...
        $shop_result = Model()->table('order')->field('order_id,order_sn,add_time,order_crm_add,goods_amount,order_amount,pd_amount,shipping_fee')->where($condition)->select();



        foreach ($crm_result as $k => &$v) {
            foreach ($shop_result as $sk => $sv) {
                if($v['order_sn'] == $sv['order_sn']){
                    $v['add_time'] = $sv['add_time'];
                    $v['goods_amount'] = $sv['goods_amount'];
                    $v['order_amount'] = $sv['order_amount'];
                    $v['pd_amount'] = $sv['pd_amount'];
                    $v['shipping_fee'] = $sv['shipping_fee'];
                }
            }
        }
        ...
        $str .= '<tr><td>订单号</td><td>下单时间</td><td>业务员</td><td>回款金额</td><td>商品金额</td><td>订单金额</td><td>余额支付</td><td>运费</td></tr>';
        if($crm_result){
            foreach($crm_result as $k=>$v){
                $str .= '<tr>
                <td>&nbsp;'.$v['order_sn'].'</td>
