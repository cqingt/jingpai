...
class BaseHomeControl extends Control {
    ...
    public function __construct(){
        ...
        // 首面轮播广告
        $ap_info = loadadv(1094,'list');
        $ap_info = $ap_info ? $ap_info : [];
        foreach ($ap_info as $k => &$v) {
            $v['adv_content'] = unserialize($v['adv_content']);
        }
    ...
    private function _getMenuList() {
        $menu_list = array(
            ...
            'promotion' => array('name' => '促销', 'child' => array(
                array('name' => '藏品惠管理', 'act'=>'store_groupbuy', 'op'=>'index'),
                array('name' => '秒杀管理', 'act'=>'store_miaosha', 'op'=>'miaosha_list'), //add 秒杀模块 xin
                // array('name' => '拍卖管理', 'act'=>'store_paimai', 'op'=>'index'),
                ...
                array('name' => '优惠券管理', 'act'=>'store_voucher', 'op'=>'templatelist'),
                array('name' => '活动管理', 'act'=>'store_activity', 'op'=>'store_activity'),
                //array('name' => '会员特价管理', 'act'=>'store_vipsale', 'op'=>'vipsale_list'),//add 会员特价 xin 20151127
                //array('name' => '一元夺宝管理', 'act'=>'store_duobao', 'op'=>'duobao_list'), //add 一元夺宝 xin
            )),
            ...
            'account' => array('name' => '账号', 'child' => array(
                ...
                array('name' => 'API申请', 'act'=>'store_api', 'op'=>'index'),
            ))
        );
        return $menu_list;
    }
...
class BasePointShopControl extends Control {
    ...
    /**
     * 获得积分中心会员信息包括会员名、ID、会员头像、会员等级、经验值、等级进度、积分、已领优惠券、已兑换礼品、礼品购物车
     */
...
class ServiceControl extends Control {
    ...
    public function __construct(){
        ...
        Tpl::setDir('servicecenter');

        Tpl::setLayout('service_center_layout');
        ...
        $nav = array(array('ac_name'=>'购物指南'),array('ac_name'=>'售后服务'),array('ac_name'=>'特色服务'),array('ac_name'=>'客服中心'),array('ac_name'=>'企业介绍'));
        //导航
        $model = Model('servicecenter');
        foreach($nav as $key => $val) {
            $condition = array();
            $condition['where'] = "ac_name = '" .$val['ac_name'] ."'";
            $condition['field'] = 'ac_id';
            $nav[$key]['ac_id'] = $model -> getArticleClassOne($condition);
        }
        Tpl::output( 'nav' , $nav );
    }
}
...
class QuestionControl extends Control {
    ...
    public function __construct(){
        ...
        Tpl::setLayout('question_header_layout');
        Tpl::setDir('question');
