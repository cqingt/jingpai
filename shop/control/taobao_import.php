...
class taobao_importControl extends BaseSellerControl {
	...
	public function indexOp(){
		...
		if(!$_POST){
			 ...
			 * 获取商品分类
			 */
			$store_bind_class_list = Model('store_bind_class')->getStoreBindClassList(array('store_id'=>$_SESSION['store_id'],'state'=>array('in',array(1,2))), null);
			$class_id = array();
			if($store_bind_class_list){
				foreach($store_bind_class_list as $k=>$v){
					$class_id[] = $v['class_1'];
				}
			}

			$gc	= Model('goods_class');
			$gc_list	= $gc->getGoodsClassList(array('gc_parent_id'=>'0','gc_id'=>array('in',$class_id)));
		   ...
		   if (strtoupper(CHARSET) == 'GBK'){
			...
			require_once(BASE_ROOT_PATH.'/core/framework/libraries/oss/aili_yun_oss.class.php');
			...
			if(is_array($records) and count($records) > 0){
				foreach($records as $k=>$record){
					...
					//file_put_contents('image.txt',print_r($pic_array,true),FILE_APPEND);
					if(empty($record['goods_name']))continue;

                    if(!empty($record['goods_body'])){
                        $record['goods_body'] = stripslashes($record['goods_body']);
                        preg_match_all('/<img.*?src=["|\'](.*?)["|\'].*?>/is',$record['goods_body'],$match_res);
                        if(!empty($match_res[1]) && is_array($match_res[1])){
                            foreach($match_res[1] as $v){
								$new_img = BASE_UPLOAD_PATH.DS.'shop'.DS.'contents'.DS.basename($v);
								//抓取淘宝商品详情页图片到本地服务器 2016-06-20 修改
								$img = @file_get_contents($v); 
								@file_put_contents($new_img,$img);
								$oss->yun_upload_img($new_img);
                                $base_path = 'http://images.96567.com/upload/shop/contents/';
                                $base_img = $base_path.basename($v);
                                $record['goods_body'] = str_replace($v,$base_img,$record['goods_body']);
                            }
                        }
					...
					//zmr>v30生成二维码
					$PhpQRCode->set('date',M_SITE_URL . '/index.php?act=goods&op=index&goods_id='.$goods_id1);
		...
		if(!empty($pic_array) && is_array($pic_array)){
			...
			foreach($pic_array as $v){
				if($v != ''){
                    ...
                    $line[0] = $_SESSION['store_id'].'_'.$line[0];
                    if(strpos($line[0],'.jpg') === false){
                        $line[0] = $line[0].'.jpg';
                    }
					$goods_image[] = $line[0];
				}
