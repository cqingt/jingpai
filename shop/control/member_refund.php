...
class member_refundControl extends BaseMemberControl {
	...
	public function add_refundOp(){
		...
		$ZiYingStore = Model('store')->getOwnShopIds();//获取自营店铺的id
		$ZiYingStore[]='22'; //id 中金店铺id
		
		if(in_array($order['store_id'],$ZiYingStore)){
			Tpl::output('IsZiYing',true); //检查是是否是自营店铺 自营店铺则没仅退款选项
			showDialog('该店铺暂不支持自主退款/退货申请，请联系客服处理，请见谅！','','notice','','5');
		}
...
//    			}
	...
	public function add_refund_allOp(){
        ...
        if($store_info['is_own_shop']){
            showDialog('退款/退货请致电客服热线 400-81-96567 或直接联系您的专属客服','index.php?act=member_order&op=index','error','','5');
        }
