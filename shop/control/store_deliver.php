...
class store_deliverControl extends BaseSellerControl {
	...
	public function sendOp(){
		...
		//如果是自提订单，只保留自提快递公司
		if (isset($order_info['extend_order_common']['reciver_info']['dlyp']) && !empty($order_info['extend_order_common']['reciver_info']['dlyp'])) {
	...
	public function buyer_address_editOp() {
		...
		$str = iconv("UTF-8","GB2312",$order_common_info['reciver_info']); //收货人信息反序列化
		$str = preg_replace('!s:(\d+):"(.*?)";!se',"'s:'.strlen('$2').':\"$2\";'",$str);
		$reciver_info = unserialize($str);
		if(!$reciver_info){
			$str = $order_common_info['reciver_info']; //收货人信息反序列化
			$str = preg_replace('!s:(\d+):"(.*?)";!se',"'s:'.strlen('$2').':\"$2\";'",$str);
		}

		$order_common_info['reciver_info'] = unserialize($str);

		
		//盛威添加 2015-11-11
		$order_common_info['reciver_info']['mob_phone'] = JieMiMobile($order_common_info['reciver_info']['mob_phone']);
		$order_common_info['reciver_info']['phone'] = JieMiMobile($order_common_info['reciver_info']['phone']);
		$order_common_info['reciver_info']['tel_phone'] = JieMiMobile($order_common_info['reciver_info']['phone']);
    ...
    private function _get_reciver_info() {
        $reciver_info = array(
            ...
            'phone' => JiaMiMobile(trim($_POST['reciver_mob_phone'])) . ',' . JiaMiMobile($_POST['reciver_tel_phone']),
            'area' => $_POST['reciver_area'],
            'street' => $_POST['reciver_street'],
            'mob_phone' => JiaMiMobile($_POST['reciver_mob_phone']),
            'tel_phone' => JiaMiMobile($_POST['reciver_tel_phone']),
