...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function groupbuy_quota_add_saveOp() {
        ...
        //获取当前价格
        $current_price = C('groupbuy_price');
        ...
        //记录店铺费用
        $this->recordStoreCost($current_price * $groupbuy_quota_quantity, '购买藏品惠');

        $this->recordSellerLog('购买'.$groupbuy_quota_quantity.'份藏品惠套餐，单价'.$current_price.L('nc_yuan'));
    ...
    public function groupbuy_upOp() {
        ...
        if(empty($groupbuy_id)){
            showMessage('当前没有可用藏品惠，请先添加',urlShop('store_groupbuy', 'index'),'','error');
        }
        ...
        if($_POST['submit_form'] == 'ok'){
            ...
            showMessage('操作成功',urlShop('store_groupbuy', 'groupbuy_up',array('groupbuy_id'=>$groupbuy_id)));
    ...
    public function groupbuy_saveOp() {
        ...
        $goods_info = $model_goods->getGoodsInfoByID($goods_id, 'goods_id,goods_commonid,goods_name,goods_price,store_id,virtual_limit,goods_image');
        ...
        $param['groupbuy_price'] = $_POST['groupbuy_price'];
        ...
        if ($_GET['vr']) {
            if ($param['upper_limit'] > 0 && $goods_info['virtual_limit'] > 0
                ...
                showDialog(sprintf('虚拟藏品惠活动的限购数量(%d)不能大于虚拟商品本身的限购数量(%d)',
                    $param['upper_limit'], $goods_info['virtual_limit']
        ...
        if($result) {
            ...
            $this->recordSellerLog('发布藏品惠活动，藏品惠名称：'.$param['groupbuy_name'].'，商品名称：'.$param['goods_name']);
    ...
    private function profile_menu($menu_key='') {
        ...
        case 'groupbuy_add_vr':
            $menu_array[] = array('menu_key'=>'groupbuy_add_vr','menu_name'=>'新增虚拟藏品惠','menu_url'=>'index.php?act=store_groupbuy&op=groupbuy_add_vr');
            break;
