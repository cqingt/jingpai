...
class member_orderControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        foreach ($order_list as $order_id => $order) {
            ...
            if(is_array($order['extend_order_goods']) && !empty($order['extend_order_goods'])){
            ...
            if ($order['order_state'] == ORDER_STATE_NEW) {
                $order_group_list[$order['pay_sn']]['pay_amount'] += $order['order_amount']-$order['pd_amount']-$order['rcb_amount']-$order['money_paid'];
            }
...
Tpl::output('html_title','实物交易订单'.' - '.C('site_name'));
    ...
    public function show_orderOp() {
        ...
        if ($order_info['order_state'] == ORDER_STATE_SEND) {
           //$order_info['order_confirm_day'] = $order_info['delay_time'] + ORDER_AUTO_RECEIVE_DAY * 24 * 3600;
			//by abc.com
			$order_info['order_confirm_day'] = $order_info['delay_time'] + ORDER_AUTO_RECEIVE_DAY + 20 * 24 * 3600;
        }
	...
	public function change_stateOp() {
		...
		if($_GET['state_type'] == 'order_cancel') {
			...
			if ($_GET['state_type'] == 'order_delete') {
				showDialog('无权操作','','error');
				exit();
			}
