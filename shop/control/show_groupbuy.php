...
class show_groupbuyControl extends BaseHomeControl {
    ...
    public function indexOp()
    {
        ...
        // 线上藏品惠 推荐'recommended' => 1,
        $groupbuy = $model_groupbuy->getGroupbuyOnlineList(array(
            'is_vr' => 0,
        ), null , 'groupbuy_xu ASC , start_time ASC ,  state asc');
    ...
    private function _show_vr_groupbuy_list($function_name)
    {
        ...
        Tpl::output('html_title', Language::get('text_groupbuy_list').' - '.C('site_name'));
    ...
    private function _show_groupbuy_list($function_name) {
        ...
        /*2016-01-22 Add is name lt 新增排序*/
        if($order){
            $order = "groupbuy_xu ASC , ".$order;
        }else{
            $order = "groupbuy_xu ASC ";
        }
        /*End*/
    ...
    public function groupbuy_detailOp() {
        ...
        if ($groupbuy_info['is_vr']) {
            ...
            $goods_common_info = Model('goods')->getGoodeCommonInfoByID($groupbuy_info['goods_commonid']);
            Tpl::output('goods_body', $goods_common_info['goods_body']);
