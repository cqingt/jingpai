...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
		...
		//推荐商品
		if (intval($_GET['store_goods_commend']) == 1) {
			$where['goods_commend'] = 1;
		}
        $goods_list = $model_goods->getGoodsCommonOnlineList($where);
        Tpl::output('show_page', $model_goods->showpage());
        Tpl::output('goods_list', $goods_list);
		(isset($_GET['store_goods_commend'])) && Tpl::output('store_goods_commend', $_GET['store_goods_commend']);//判断是否搜索推荐商品
    ...
    /*2015-11-20 Add is name Lt 预约预售查看*/
    public function arrival_noticeOp(){
        $model = Model('arrival_notice');
		...
		$where['store_id'] = $_SESSION['store_id'];
        $result = $model->getArrNoticeList($where,10);
        Tpl::output('result',$result);
        Tpl::output('pageNumber',$model->getTotalNum());
        Tpl::output('page',$model->showpage());
        $this->profile_menu('goods_list', 'arrival_notice');
        Tpl::showpage('store_goods_list.arrival_notice');
    }
    /*End*/
    ...
    public function edit_goodsOp() {
        ...
        /*查出所有艺术家*/
        $artist_model = Model();
        $result_artist = $artist_model->table('artist')->field('A_Id,A_Name')->where("A_Name<>''")->order('A_Id desc')->select();
        Tpl::output('result_artist', $result_artist);
        /*根据一级分类ID查出是否关联艺术家*/
        $goods_class = Model('goods_class');
        $one_classid = $goodscommon_info['gc_id_1'];
        $is_artist = $goods_class->where(array('gc_id'=>$one_classid))->find();
        Tpl::output('is_artist', $is_artist['is_artist']);

        /* End */


        /*2015-11-27 Add is name lt 查出商品关键词*/
        $goods_key = $model_goods->getGoodsInfo(array('goods_commonid'=>$goodscommon_info['goods_commonid']),'goods_id,goods_keywords,goods_description');
        Tpl::output('goods_key', $goods_key);
        /*End*/
		/*2015-11-30 查询关联商品*/
		$GetGoodsLink = $model_goods->GetGoodsLink(array('goods_id'=>$goods_key['goods_id']));
		Tpl::output('goods_id', $goods_key['goods_id']); //当前的产品id
    ...
    public function edit_save_goodsOp() {
		...
		if ($_POST['g_serial'])
		{
			$SerNum = $model_goods->is_g_serial($_POST['g_serial'],$common_id);
			if($SerNum > 0){
				showDialog('商家货号以存在请检查', urlShop('store_goods_online', 'edit_goods', array('commonid' => $common_id)));
			}
		}
		...
		if ((empty($_POST['g_serial']) && intval($_SESSION['is_own_shop']) == 1) || intval($_SESSION['store_id']) == 22)
		{
			if($_POST['g_serial']){
				...
				$g_serial   = $model_goods->generate_g_serial($common_id); //生成商品货号
			}
		}
		else
		{
			...
			if(intval($_SESSION['is_own_shop']) == 0 && substr($g_serial,0,1) == '9' && intval($_SESSION['store_id']) != 22){
				showDialog('编号违规请检查', urlShop('store_goods_online', 'edit_goods', array('commonid' => $common_id)));
			}
		}
		...
		if (!empty($_POST['goods']) && is_array($_POST['goods'])) {
			foreach ($_POST['goods'] as $key => $val) {
				...
				if (empty($goods_info) || $goods_info['store_id'] != $_SESSION['store_id']) {
					showDialog('关联商品必须是本店商品', urlShop('store_goods_online', 'edit_goods', array('commonid' => $common_id)));
				}
		...
		$getGoodsInfo = $model_goods->getGoodsInfo(array('goods_commonid'=>$common_id),'goods_id,goods_storage');
		$GetGoodsLink = $model_goods->GetGoodsLink(array('goods_id'=>$getGoodsInfo['goods_id']),'link_goods_id');
		if($GetGoodsLink){
			//删除跟当前商品有关的所有关联商品
			if($GetGoodsLink){foreach($GetGoodsLink as $glk=>$glv){$model_goods->deleteGoodsLink(array('goods_id'=>array('in',$glv['link_goods_id']))); }}
		}
        ...
        $update_common['goods_serial']       = $g_serial;
        ...
        //add 商品是否支持货到付款 xin 20151114
        if(!$update_common['is_own_shop']){
            if(!$this->store_info['store_huodaofk']){ //非自营判断是否开启货到付款，未开启默认不支持
                //$update_common['goods_daofu'] = 0;
            }else{
                $update_common['goods_daofu'] = intval($_POST['goods_daofu']);
            }
        }else{
            $update_common['goods_daofu'] = intval($_POST['goods_daofu']);
        }
...
/* 2015-10-08 Add LT 添加产品关联的艺术家ID */
        ...
        if($_POST['artist']){
            $update_common['artist_id'] = intval($_POST['artist']);
            $update_common['sh_width'] = $_POST['sh_width'];
            $update_common['sh_height'] = $_POST['sh_height'];
            $update_common['sh_pingchi'] = $_POST['sh_pingchi'];
        }else{
            $update_common['artist_id'] = null;
            $update_common['sh_width'] = null;
            $update_common['sh_height'] = null;
            $update_common['sh_pingchi'] = null;
        }
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    if(!empty($update_common['goods_daofu'])){
                        $update['goods_daofu'] = $update_common['goods_daofu'];//货到付款状态
                    }

                    /* 2015-10-08 Add LT 添加产品关联的艺术家ID */
            
                        if($_POST['artist']){
                            $update['artist_id'] = intval($_POST['artist']);
                            $update['sh_width'] = $_POST['sh_width'];
                            $update['sh_height'] = $_POST['sh_height'];
                            $update['sh_pingchi'] = $_POST['sh_pingchi'];
                        }else{
                            $update['artist_id'] = null;
                            $update['sh_width'] = null;
                            $update['sh_height'] = null;
                            $update['sh_pingchi'] = null;
                        }

                        /* End */

                    /*2015-11-27 Add is name lt 加入关键词和描述*/
                    $update['goods_keywords'] = $_POST['goods_keywords'];
                    $update['goods_description'] = $_POST['goods_description'];
                    /*End*/
		    ...
		    // 生成商品二维码
                    ...
                    if(!empty($update_common['goods_daofu'])){
                        $insert['goods_daofu'] = $update_common['goods_daofu'];//货到付款状态
                    }
                    /* 2015-10-08 Add LT 添加产品关联的艺术家ID */
            
                         if($_POST['artist']){
                            $insert['artist_id'] = intval($_POST['artist']);
                            $insert['sh_width'] = $_POST['sh_width'];
                            $insert['sh_height'] = $_POST['sh_height'];
                            $insert['sh_pingchi'] = $_POST['sh_pingchi'];
                        }else{
                            $insert['artist_id'] = null;
                            $insert['sh_width'] = null;
                            $insert['sh_height'] = null;
                            $insert['sh_pingchi'] = null;
                        }
                    ...
                    /*2015-11-27 Add is name lt 加入关键词和描述*/
                    $insert['goods_keywords'] = $_POST['goods_keywords'];
                    $insert['goods_description'] = $_POST['goods_description'];
                    /*End*/
		...
		/* 自营店铺 店铺更新价格 */
		if (intval($_SESSION['is_own_shop']) == 1 || intval($_SESSION['store_id']) == 22)
		{
			$shop_goods_log = array(
				'goods_id' => 	$goods_id, //产品id
				'goods_name' => $update_common['goods_name'],//产品名称
				'goods_sn' => $g_serial, //产品编号
				'update_shopprice' => $update_common['goods_price'], //销售价格
				'update_costprice' => $update_common['goods_costprice'], //成本价格
				'update_time' => TIMESTAMP, //修改时间
				'operator_name' => $_SESSION['seller_name'] //操作人名称
			);
			$model_goods->InsertSellerLog($shop_goods_log);
			if($getGoodsInfo['goods_storage'] != $update['goods_storage']){
				//记录编辑库存记录
				$shop_goods_log['storage_num'] = $getGoodsInfo['goods_storage'];//编辑前
				$shop_goods_log['storagenum_'] = $update['goods_storage'];//编辑后
				$shop_goods_log['update_log'] = "商城商家后台编辑";
				$model_goods->InsertStorageLog($shop_goods_log);
			}
		}
        ...
        if ($return) {
			...
			if ((empty($g_serial) && intval($_SESSION['is_own_shop']) == 1) || intval($_SESSION['store_id']) == 22)
			{
				//@file_get_contents('http://zyh.96567.com/api/Up_GoodsPrice.php?Money='.$update_common['goods_price'].'&PID='.$g_serial.''); //同步更新zyh商城价格
			}
            ...
            case 'goods_list':
                $menu_array = array(
                   array('menu_key' => 'goods_list',    'menu_name' => '出售中的商品', 'menu_url' => urlShop('store_goods_online', 'index')),
                    array('menu_key' => 'arrival_notice',    'menu_name' => '预约预售查看', 'menu_url' => urlShop('store_goods_online', 'arrival_notice'))
                );
                break;
            case 'edit_detail':
	...
	public function maker_qrcodeOp()
	{
		...
		$lst=$model_goods->getGoodsList($where,'goods_id','','','20000');
		if(empty($lst))
		{
			echo '未找到商品信息';
			return;
		...
		foreach($lst as $k=>$v)
		{
			$goods_id=$v['goods_id'];
			//zmr>v30生成二维码
			$qrcode_url=M_SITE_URL . '/index.php?act=goods&op=index&goods_id='.$goods_id;
	...
	//生成所有店铺二维码 杜
	public function maker_SuoYouqrcodeOp()
	{
		//zmr>v30
		header("content-type:text/html; charset=utf-8"); 
		echo '正在生成，请耐心等待...';
		...
		$eachnum = 50;
		$count = $model_goods->query("SELECT count(0) as num FROM  `shop_goods` WHERE  `goods_addtime` > 1451577600 ");
		$count_page = ceil($count[0]['num']/$eachnum);
	
		if($page > $count_page){ echo '<br/><b style="color:blue">==========全部生成完成==========</b>'; exit;}
		$limit = ($page-1)*50;
		$store_array = $model_goods->query("SELECT goods_id,store_id FROM  `shop_goods` WHERE  `goods_addtime` > 1451577600 limit ".$limit.",".$eachnum."");
		require_once(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
		$PhpQRCode = new PhpQRCode();
		foreach($store_array as $k=>$v)
		{
			$PhpQRCode->set('pngTempDir',BASE_UPLOAD_PATH.DS.ATTACH_STORE.DS.$v['store_id'].DS);
			$goods_id=$v['goods_id'];
			//zmr>v30生成二维码
			$qrcode_url=M_SITE_URL . '/index.php?act=goods&op=index&goods_id='.$goods_id;
			$PhpQRCode->set('date',$qrcode_url);
		...
		header("Location: http://www.96567.com/index.php?act=store_goods_online&op=maker_SuoYouqrcode&page=".intval($page+1));
	}
