...
class store_billControl extends BaseSellerControl {
    ...
    public function indexOp() {
        ...
        $bill_list = $model_bill->getOrderBillList($condition,'*',12,'ob_state asc,ob_no desc');
    ...
    /*
     * 拍卖惠订单列表 xin 20160226
     */
    public function show_lepaiorderOp(){
        if (!preg_match('/^20\d{5,12}$/',$_GET['ob_no'])) {
        ...
        $lepaistore_info = $model_lepai->getStoreInfo(array('member_id'=>$store_info['member_id']));
        ...
        $order_condition['store_member_id'] = $store_info['member_id'];
        ...
        $order_list = $model_lepai->getLepaiOrder($order_condition,12);

        Tpl::output('lepaistore_info',$lepaistore_info);
        Tpl::output('order_list',$order_list);
        Tpl::output('show_page',$model_lepai->showpage());
        $sub_tpl_name = 'store_bill.show.lepaiorder_list';
        $this->profile_menu('show','lepai_order_list');
     ...
     * 拍卖惠订单导出
     */
    public function export_lepai_orderOp(){
        if (!preg_match('/^20\d{5,12}$/',$_GET['ob_no'])) {
            showMessage('参数错误','','html','error');
        }
        if (substr($_GET['ob_no'],6) != $_SESSION['store_id']){
        ...
        $order_list = $model_lepai->table('lepai_order,lepai_admin_goods')->where($order_condition)->join('inner')->on('lepai_order.lepai_goods_id=lepai_admin_goods.G_Id')->field('*')->order('order_id desc')->select();

        $export_data = array();
        $export_data[0] = array('订单编号','订单金额','运费','佣金','下单日期','成交日期','买家','商品');
        $order_totals = 0;
        ...
        foreach ($order_list as $k=>$v) {
            ...
            $commis_totals += $export_data[$k+1][] = $v['order_amount'] * $lepaistore_info['commis_rate'] / 100;
            $export_data[$k+1][] = date('Y-m-d H:i',$v['add_time']);
            $export_data[$k+1][] = date('Y-m-d H:i',$v['finnshed_time']);
            $export_data[$k+1][] = $v['buyer_name'];
            $export_data[$k+1][] = $v['G_Name'];
            $k++;
        }
        ...
        $csv->filename = $csv->charset('拍卖惠订单明细-',CHARSET).$_GET['ob_no'];
     ...
     * 查看拍卖惠订单详情页
     */
    public function lepaiorder_detailOp(){
        Language::read('member_store_index');
        $order_id = intval($_GET['order_id']);
        $model_lepai = Model('lepai_home');
        $order_detail = $model_lepai->getLepaiOrderShow(array('order_id'=>$order_id));
        if(empty($order_detail)){
            showMessage('参数错误','','html','error');
        }
        $store_info = $model_lepai->getStoreInfo(array('member_id'=>$order_detail['store_member_id']));
        if(empty($store_info)){
            showMessage('参数错误','','html','error');
        }
        $express = rkcache('express',true);
        $order_detail['shipping_ename'] = $express[$order_detail['shipping_ecode']];
        Tpl::output('order_info',$order_detail);
        Tpl::output('store_info',$store_info);
        Tpl::showpage('store_bill.lepaidetail');
    }
	...
	public function show_billOp(){
		...
		/* Add is name lt 2016-11-23 店铺详细信息(提现卡号)*/
		$model_store_joinin = Model('store_joinin');
        $joinin_detail = $model_store_joinin->getOne(array('member_id'=>$_SESSION['member_id']));
        $store_joinin['bank_account_name'] = $joinin_detail['bank_account_name']?substr_replace($joinin_detail['bank_account_name'],'*',0,3):'';
		$str_len = strlen($joinin_detail['bank_account_number']);
		$str_xin = '';
		for ($i=1; $i <= $str_len-8 ; $i++) { 
			$str_xin .= '*';
		}
		$store_joinin['bank_account_number'] = $joinin_detail['bank_account_number']?substr_replace($joinin_detail['bank_account_number'],$str_xin,4,($str_len-8)):'';
		$store_joinin['bank_name'] = $joinin_detail['bank_name'];
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			...
			case 'show':
                ...
                array('menu_key'=>'lepai_order_list','menu_name'=>'拍卖惠订单列表', 'menu_url'=>'index.php?act=store_bill&op=show_lepaiorder&ob_no='.$_GET['ob_no']),
