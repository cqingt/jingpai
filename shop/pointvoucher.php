...
class pointvoucherControl extends BasePointShopControl {
	...
	public function voucherexchangeOp(){
		...
		file_put_contents('dai_money11.txt', print_r('11',true),FILE_APPEND);
		$result = true;
		file_put_contents('dai_money11.txt', print_r('1',true),FILE_APPEND);
		$message = "";
		file_put_contents('dai_money11.txt', print_r('2',true),FILE_APPEND);
		if ($vid <= 0){
			$result = false;
			L('wrong_argument');
		}

		file_put_contents('dai_money11.txt', print_r('3',true),FILE_APPEND);
...
/**
	...
	public function voucherexchange_addOp(){
		...
		$model_voucher = Model('voucher');
		//验证是否可以兑换代金券
		$data = $model_voucher->getCanChangeTemplateInfo($vid,$uid,'0');
		if ($data['state'] == false){
			showDialog($data['msg'],'','error',$js);
		}
		
		//查出会员名称
		$name = Model('member')->getMemberInfoByID($uid);		
		
		//添加代金券信息
		$data = $model_voucher->exchangeVoucher($data['info'],$uid,$name['member_name']);
		if ($data['state'] == true){
			showDialog($data['msg'],'','succ',$js);
		} else {
		    showDialog($data['msg'],'','error',$js);
