...
<div class="nch-container wrapper" >
  ...
  <?php if(!empty($output['miaosha_list']) && is_array($output['miaosha_list'])) { ?>
    ...
    <div class="goods-featured">
         <div class="feaTitle">今日秒杀</div>
         <div class="feaContent">

              <ul class="buy-off borb">
                  <?php foreach($output['miaosha_list'] as $val) { ?>
                  <li>
                    <div class="buyImg">
					<?php if($val['is_shipping'] == 1){?>
							<div class="icon-by"></div>
					<?php } ?>
                         <a href="<?php echo $val['goods_url'];?>" target="_blank">
                            <img src="<?php echo $val['goods_image'];?>" alt="<?php echo $val['goods_name'];?>">
                         </a>
                    </div>
                    <div class="buyText">
                         <a href="<?php echo $val['goods_url'];?>" target="_blank"><?php echo $val['goods_name'];?></a>
                    </div>
                    <p class="buyRmb"><?php echo ncPriceFormatForList($val['miaosha_price']);?></p>
                  </li>
                  <?php } ?>
              </ul>
      ...
      <?php } ?>
<?php if(!empty($output['dataArr2'])){?>
    <!-- 新品推荐 GJ -->
    <div class="goods-featured">
         <div class="feaTitle">新品推荐</div>
         <div class="feaContent">

              <ul class="buy-off borb">
              <?php foreach($output['dataArr2'] as $k=>$v){ ?>
                  <li>
                    <div class="buyImg">
                    ...
                    <div class="buyText">
                         <a href="<?php echo urlShop('goods','index',array('goods_id'=>$v['goods_id']));?>" target="_blank">
						 <?php echo $v['promotion_title'];?></a>
                    </div>
					 <?php if(intval($v['promotion_price']) > 0){ ?>
							<p class="buyRmb"><?php echo ncPriceFormatForList($v['promotion_price']);?></p>
					  <?php }else{ ?>
							 <p class="buyRmb"><?php if(intval($v['goods_price']) < 1){ ?>咨询客服<?php }else{ ?><?php echo ncPriceFormatForList($v['goods_price']);?><?php } ?></p>
					<?php } ?>
          ...
          <ul>
            ...
            <li class="nch-sidebar-bowers">
              ...
              <dl>
                ...
                <dd><?php if(intval($v['goods_price']) < 1){ ?>咨询客服<?php }else{ ?><?php  if(intval($v['goods_promotion_price']) > 0){ echo ncPriceFormatForList($v['goods_promotion_price']);}else{ echo ncPriceFormatForList($v['goods_price']);} ?><?php } ?></dd>
              </dl>
            </li>
            <?php } ?><?php } ?>
          </ul>
...
?>
    <div class="hot-recommend">
         <div class="hotLeft"> 
              ...
              <?php
				$i=0;
			  	foreach($output['dataArr'] as $k=>$v){
					if($i<3){
			  ?>
                 <li>
                    <div class="hot-img">
                    	<a href="<?php echo urlShop('goods','index',array('goods_id'=>$v['goods_id']));?>"><img src="<?php echo $v['goods_pic'];?>" alt="<?php echo $v['goods_name'];?>"></a>
                    </div>
                    <div class="hot-text">
                         <p class="ht1"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$v['goods_id']));?>" target="_blank"><?php echo $v['promotion_title'];?></a></p>
						 
						  <?php if(intval($v['promotion_price']) > 0){ ?>
							<p class="ht2"><?php echo ncPriceFormatForList($v['promotion_price']);?></p>
						  <?php }else{ ?>
							<p class="ht2"><?php if(intval($v['goods_price']) < 1){ ?><em>咨询客服</em><?php }else{ ?><?php echo ncPriceFormatForList($v['goods_price']);?><?php } ?></p>
						  <?php } ?>
                         <p class="ht3"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$v['goods_id']));?>" target="_blank">立即抢购</a></p>
                    </div>
                 </li>
                <?php
						unset($output['dataArr'][$i]);
					}
					$i++;
				}
				?>
         ...
         <div class="hotRight">
              ...
              <?php
			  	...
			  	foreach($output['dataArr'] as $k=>$v){
					if($i<4){
			  ?>
                 <li><a href="<?php echo urlShop('goods','index',array('goods_id'=>$v['goods_id']));?>" target="_blank"><?php echo $v['promotion_title'];?></a></li>
              <?php
					}
					$i++;
				}
...
<?php if(!empty($output['dataArr1'])){ ?>
    <!-- 商品精选 GJ-->
    <div class="goods-featured">
         <div class="feaTitle">商品精选</div>
         <div class="feaContent">

              <ul class="buy-off fl">
              <?php foreach($output['dataArr1'] as $k=>$v){ ?>
                  <li>
                    <div class="buyImg">
                         <a href="<?php echo urlShop('goods','index',array('goods_id'=>$v['goods_id']));?>" target="_blank">
                            <img src="<?php echo $v['goods_pic'];?>" alt="<?php echo $v['goods_name'];?>">
                    ...
                    <p class="buyRmb"><?php if(intval($v['goods_price']) < 1){ ?>咨询客服<?php }else{ ?><em><?php if(intval($v['promotion_price']) > 0){ ?>活动售价：</em><?php echo ncPriceFormatForList($v['promotion_price']);?><?php }else{ ?>本店售价：</em><?php echo ncPriceFormatForList($v['goods_price']);?><?php } ?><?php } ?></p>
                  </li>
               <?php }?>
              </ul>
