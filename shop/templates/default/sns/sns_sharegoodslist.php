...
<div class="wrapper dfclearfix mtb">
	<div class="picture-content dfclearfix">
	 ...
	 <ul>
      <li class="on"><a href="index.php?act=member_snshome&op=shareglist&type=share&mid=<?php echo $output['master_info']['member_id'];?>"><?php echo $lang['sns_shareofgoods'];?></a></li>
      <li class=""><a href="index.php?act=member_snshome&op=shareglist&type=like&mid=<?php echo $output['master_info']['member_id'];?>"><?php echo $lang['sns_likeofgoods'];?></a></li>
    </ul>
			<?php if($output['relation'] == '3'){?>
			<span>
			<?php if(!empty($output['goodslist'])){?>
			
				<p><strong>分享、晒单</strong>收藏和已购商品，让我的粉丝们<strong>关注</strong>一下！ </p>
			<?php }?>
				<a class="btn-sharebaby"href="javascript:void(0);" id="snssharegoods"><?php echo $lang['sns_share_treasure'];?></a>
		...
		<div class="handover_con">
			...
			<div class="sharelist-g mt30">
				<ul class=" nc-sns-pinterest">
				<?php $i=1;?>
				<?php foreach ($output['goodslist'] as $k=>$v){ ?>
					

					<li id="recordone_<?php echo $v['share_id'];?>" <?php if($i%4 == 0 || $i==1){ ?>style="margin-left: 0px;"<?php } ?>>
					<ul class="handle">
					...
					<dl>
					  <dt class="goodspic"><span class="thumb size233"><i></i><a href="index.php?act=member_snshome&op=goodsinfo&mid=<?php echo $v['share_memberid'];?>&id=<?php echo $v['share_id'];?>" title="<?php echo $v['snsgoods_goodsname']?>"> <img src="<?php echo cthumb($v['snsgoods_goodsimage'],240,$v['snsgoods_storeid']);?>"/></a>
					  </span>
					  <h2><a href="index.php?act=member_snshome&op=goodsinfo&mid=<?php echo $v['share_memberid'];?>&id=<?php echo $v['share_id'];?>" target="_blank"><?php echo $v['snsgoods_goodsname']?></a></h2>
