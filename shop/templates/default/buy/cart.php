...
<div class="ncc-main">
  ...
  <form action="<?php echo urlShop('buy','buy_step1');?>" method="POST" id="form_buy" name="form_buy">
    ...
    <table class="ncc-table-style" nc_type="table_cart">
      ...
      <tbody>
        <tr>
          <th colspan="20"><strong><p>店铺：</p><a class="shop_name" href="<?php echo urlShop('show_store','index',array('store_id'=>$store_id), $output['store_list'][$store_id]['store_domain']);?>"><?php echo $cart_list[0]['store_name']; ?></a> <a class="shop_chat" href="JavaScript:void(0);" title="在线联系" <?php if($output['store_list'][$store_id]['is_own_shop'] == 1){?> 
	...
	<?php } ?>></a></strong> <span member_id="<?php echo $output['store_list'][$store_id]['member_id'];?>"></span>
        ...
        <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" nc_group="<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo $cart_info['state'] ? '' : 'item_disabled';?>">
          ...
          <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>><dl class="ncc-goods-info">
              ...
              <dd> <span class="groupbuy">藏品惠<?php if ($cart_info['upper_limit']) {?>，最多限购<strong><?php echo $cart_info['upper_limit']; ?></strong>件<?php } ?></span></dd>
              <?php }?>
              <!-- add 秒杀模块 xin -->
              <?php if ($cart_info['ifmiaosha']) {?>
                  <dd> <span class="groupbuy">秒杀<?php if ($cart_info['upper_limit'] > 0) {?>，最多限购<strong><?php echo $cart_info['upper_limit']; ?></strong>件<?php } ?></span></dd>
              <?php }?>
              <!-- add 会员特价模块 xin 20151130 -->
              <?php if ($cart_info['ifvipsale']) {?>
                  <dd> <span class="groupbuy">会员特价<?php if ($cart_info['upper_limit'] > 0) {?>，最多限购<strong><?php echo $cart_info['upper_limit']; ?></strong>件<?php } ?></span></dd>
              <?php }?>
              <!-- add end -->
			  <!-- add 推荐优惠 -->
              <?php if ($cart_info['iftuijian']) {?>
                  <dd> <span class="groupbuy">推荐优惠<?php if ($cart_info['upper_limit'] > 0) {?>，最多限购<strong><?php echo $cart_info['upper_limit']; ?></strong>件<?php } ?></span></dd>
              <?php }?>
              <!-- add end -->
