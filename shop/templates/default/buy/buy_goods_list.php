...
<div class="ncc-receipt-info">
  ...
  <table class="ncc-table-style">
    ...
    <tbody>
      <tr>
        <th colspan="20"><strong><p>店铺：</p><a class="shop_name" href="<?php echo urlShop('show_store','index',array('store_id'=>$store_id));?>"><?php echo $cart_list[0]['store_name']; ?></a><a class="shop_chat" href="javascript:void(0);" title="在线联系" <?php if($output['store_list'][$store_id]['is_own_shop'] == 1){?> 
		onclick="NTKF.im_openInPageChat('sc_1000_9999')"
	<?php }else{ ?>onclick="NTKF.im_openInPageChat('sc_<?php echo 1000+intval($store_id);?>_9999')"
	<?php } ?>></a> </strong> <span member_id="<?php echo $output['store_list'][$store_id]['member_id'];?>"></span>
      ...
      <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo ($cart_info['state'] && $cart_info['storage_state']) ? '' : 'item_disabled';?>">
        ...
        <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>><dl class="ncc-goods-info">
            ...
            <dd> <span class="groupbuy">藏品惠</span></dd>
            <?php }?>
            <!-- add 秒杀模块 xin -->
            <?php if ($cart_info['ifmiaosha']) {?>
                <dd> <span class="groupbuy">秒杀</span></dd>
            <?php }?>
            <!-- add 会员特价模块 xin -->
            <?php if ($cart_info['ifvipsale']) {?>
                <dd> <span class="groupbuy">会员特价</span></dd>
            <?php }?>
            <!-- add end -->
			<!-- add 推荐优惠 xin -->
            <?php if ($cart_info['iftuijian']) {?>
                <dd> <span class="groupbuy">推荐优惠</span></dd>
            <?php }?>
            <!-- add end -->
            <!-- add 满即送 xin -->
            <?php if ($cart_info['manjisong']) {?>
                <dd> <span class="groupbuy">满即送活动商品</span></dd>
            <?php }?>
            <!-- add end -->
            <?php if ($cart_info['bl_id'] != '0') {?>
      ...
      <?php } ?>
      <!-- 满送商品 add xin 2160112 goods list -->
      <?php if (is_array($output['store_premiums_list'][$store_id])) {?>
          <?php foreach ($output['store_premiums_list'][$store_id] as $mansong_goods) { ?>
              <tr class="shop-list">
                  <td></td>
                  <td class="w60"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$mansong_goods['goods_id']));?>" target="_blank" class="ncc-goods-thumb"><img src="<?php echo cthumb($mansong_goods['goods_image'],60,$store_id);?>" alt="<?php echo $mansong_goods['goods_name']; ?>" /></a></td>
                  <td class="tl"><dl class="ncc-goods-info">
                          <dt><a href="<?php echo urlShop('goods','index',array('goods_id'=>$mansong_goods['goods_id']));?>" target="_blank"><?php echo $mansong_goods['goods_name']; ?></a> </dt>
                          <span class="ncc-goods-gift">满即送赠品</span>
                      </dl></td>
                  <td></td>
                  <td><em><?php echo $mansong_goods['goods_num'];?></em></td>
                  <td></td>
                  <td></td>
              </tr>
          <?php } ?>
      ...
      <tr>
        <td class="tr" colspan="20"><div class="ncc-store-account">
            ...
            <dl class="freight" style="display: none">
                <dt>货到付款手续费：</dt>
                <dd><em id="eachStorePayFee_<?php echo $store_id;?>" pay_fee="<?php echo number_format($output['pay_fee'][$store_id],2,".", "");?>">0.00</em><?php echo $lang['currency_zh'];?></dd>
            </dl>
            <dl>
              <dt>商品金额：</dt>
            ...
            <dl class="voucher">
              <dt>
                <select nctype="voucher" name="voucher[<?php echo $store_id;?>]">
                  <option value="<?php echo $voucher['voucher_t_id'];?>|<?php echo $store_id;?>|0.00">选择店铺优惠券</option>
    ...
    <!-- 平台优惠券 -->
      ...
      <tr>
          <td class="tr" colspan="20"><div class="ncc-store-account">
          <dl class="voucher">
              <dt style="width:500px"><span style="color: #666;">注：平台优惠券与店铺优惠券不可以同时使用！ </span>
              ...
              <dd><em id="PingtaiVoucher">-0.00</em><?php echo $lang['currency_zh'];?></dd>
          </dl>
              </div></td>
      </tr>
      <?php } ?>
