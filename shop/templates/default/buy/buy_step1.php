...
function calcOrder() {
    ...
    $('em[nc_type="eachStoreTotal"]').each(function(){
        ...
        if ($('#eachStorePayFee_'+store_id).length > 0) {
            eachTotal += parseFloat($('#eachStorePayFee_'+store_id).html());
        }
    ...
    if ($('#PingtaiVoucher').length > 0) {
        allTotal += parseFloat($('#PingtaiVoucher').html());
    }
...
$(function(){
    ...
    $('select[nctype="voucher"]').on('change',function(){
        ...
        $('em[nc_type="eachStoreTotal"]').each(function(){
            var s_id = $(this).attr('store_id');

            if ($('#eachStoreVoucher_'+s_id).length > 0) {
                vouchereachTotal += parseFloat($('#eachStoreVoucher_'+s_id).html());
            }
        });
        if(vouchereachTotal != 0){
            $('select[nctype="voucher_pingtai"]').attr('disabled','disabled');
        }else{
            $('select[nctype="voucher_pingtai"]').removeAttr('disabled');
        }

        calcOrder();
    });

    $('select[nctype="voucher_pingtai"]').on('change',function(){
        if($(this).attr('name') == 'voucher_pingtai'){
            var items = $(this).val().split('|');
            $('#PingtaiVoucher').html('-'+number_format(items[2],2));
            if(number_format(items[2],2) != 0){
                $('select[nctype="voucher"]').attr('disabled','disabled');
            }else{
                $('select[nctype="voucher"]').removeAttr('disabled');
            }
