...
<?php if ($output['store_count'] == 0) {?>
        <li>
            <input type="radio" value="bank" name="payment_type" id="payment_type_bank">
            <label for="payment_type_bank">银行转账</label>
        </li>
<?php } ?>
        <?php if (!$output['deny_edit_payment']) {?>
      <li>
        <input type="radio" value="offline" name="payment_type" id="payment_type_offline">
        <label for="payment_type_offline">货到付款（需额外支付货到付款商品总额 3% 的手续费)</label>
...
<div id="confirm_offpay_goods_list" style="display: none;">
  <dl class="ncc-offpay-list" data-hideshow="offline">
    ...
    <?php if(!empty($output['pay_goods_list']['online'])){ ?>
        ...
        */
        ...
        if (payment_type == 'offline'){
            ...
            $('em[nc_type="eachStoreTotal"]').each(function(){
                ...
                $('#eachStorePayFee_'+store_id).parent().parent().show();
                $('#eachStorePayFee_'+store_id).html($('#eachStorePayFee_'+store_id).attr('pay_fee'));
            });
        }else{
            $('#edit_payment').parent().after('<div class="ncc-candidate-items"><ul><li>'+content+'</li></ul></div>');
            $('#pd_panel').show();
            $('em[nc_type="eachStoreTotal"]').each(function(){
                store_id = $(this).attr('store_id');
                $('#eachStorePayFee_'+store_id).html('0.00');
                $('#eachStorePayFee_'+store_id).parent().parent().hide();
            });
        }
        calcOrder();
        ableOtherEdit();
