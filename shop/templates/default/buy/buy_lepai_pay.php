...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5>订单详情内容可通过查看<a href="index.php?act=lepai_order" target="_blank">我的拍卖惠订单</a>进行核对处理。</h5>
  </div>
  <form action="index.php?act=payment&op=lepai_order" method="POST" id="buy_form">
    <input type="hidden" name="pay_sn" value="<?php echo $output['orderInfo']['pay_sn'];?>">
    <input type="hidden" id="payment_code" name="payment_code" value="">
      <input type="hidden" id="bank_code" name="bank_code" value="">
    <div class="ncc-receipt-info">
      <div class="ncc-receipt-info-title">
        <h3>
          在线支付金额：<strong>￥<?php echo ($output['orderInfo']['order_amount'] - $output['orderInfo']['rcb_amount'] - $output['orderInfo']['pd_amount']);?></strong> <?php if($output['orderInfo']['pd_amount'] > 0){?><span style="font-size:12px">（拍卖惠保证金已抵扣在线支付金额：<?php echo $output['orderInfo']['pd_amount'];?>元）</span><?php } ?>
          </h3>
      </div>
      <table class="ncc-table-style">
        ...
        <tbody>
          <tr>
            <td></td>
            <td class="tl"><?php echo $output['orderInfo']['order_sn']; ?></td>
            <td class="tl">￥<?php echo $output['orderInfo']['order_amount'];?></td>
            <td>快递</td>
          </tr>
        </tbody>
      </table>
