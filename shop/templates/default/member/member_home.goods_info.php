      ...
      <ul id="favoritesGoodsList" class="jcarousel-skin-tango">
        ...
        <li>
          <div class="ncm-goods-thumb-120"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$favorites['goods']['goods_id']));?>" target="_blank"><img alt="" src="<?php echo thumb($favorites['goods'], 240);?>"></a>
            <div class="ncm-goods-price"><em><?php if($favorites['goods']['promotion_price'] >0 ){ ?>
				￥<?php echo ncPriceFormat($favorites['goods']['promotion_price']);?>
			<?php }else{ ?>
				<?php if($favorites['goods']['goods_price'] <= 0){ ?>
				咨询客服
				<?php }else{ ?>
				
					￥<?php echo ncPriceFormat($favorites['goods']['goods_price']);?>
				<?php } ?>
			<?php } ?>
			</em></div>
          </div>
