...
<div class="wrap">
  ...
  <div class="ncm-security-user">
    ...
    <div class="user-intro">
      ...
      <dl>
        <dt>绑定邮箱：</dt>
        <dd><?php echo $output['member_info']['member_email_bind'] == 1 ? encryptShow($output['member_info']['member_email'],4,4) : '未绑定';?></dd>
      </dl>
      <dl>
        <dt>绑定微信：</dt>
        <dd><?php echo $output['member_info']['openid'] != '' ? '已绑定' : '未绑定';?></dd>
      </dl>
      <dl>
        <dt>手机号码：</dt>
        <dd><?php echo $output['member_info']['member_mobile_bind'] == 1 ? encryptShow($output['member_info']['member_mobile'],4,4) : '未绑定';?></dd>
      </dl>
      <dl>
        <dt>上次登录：</dt>
  ...
  <div class="ncm-security-container">
    ...
    <dl id="email" class="<?php echo $output['member_info']['member_email_bind'] == 1 ? 'yes' : 'no';?>">
      ...
      <dd><span class="explain">进行邮箱验证后，可用于接收敏感操作的身份验证信息，以及订阅更优惠商品的促销邮件。
        <?php echo $output['member_info']['member_email_bind'] == 1 ? '' : '<em>限时完成<strong>邮箱验证</strong>可领取<strong>100元现金券1张</strong>！</em>';?>
      
      </span><span class="handle"><a href="<?php echo SHOP_SITE_URL;?>/index.php?act=member_security&op=auth&type=modify_email" class="ncm-btn ncm-btn-acidblue bd">绑定邮箱</a><a href="index.php?act=member_security&op=auth&type=modify_email" class="ncm-btn ncm-btn-orange jc">修改邮箱</a></span></dd>
    </dl>
    <dl id="mobile" class="<?php echo $output['member_info']['member_mobile_bind'] == 1 ? 'yes' : 'no';?>">
      ...
      <dd><span class="explain">进行手机验证后，可用于接收敏感操作的身份验证信息，以及进行积分消费的验证确认，非常有助于保护您的账号和账户财产安全。
        <?php echo $output['member_info']['member_mobile_bind'] == 1 ? '' : '<em>限时完成<strong>手机验证</strong>可领取<strong>100元现金券1张</strong>！</em>';?>
      
      </span><span class="handle"><a href="index.php?act=member_security&op=auth&type=modify_mobile" class="ncm-btn ncm-btn-acidblue bd">绑定手机</a><a href="index.php?act=member_security&op=auth&type=modify_mobile" class="ncm-btn ncm-btn-orange jc">修改手机</a></span></dd>
    </dl>
    ...
    <dl id="wechat" class="<?php echo $output['member_info']['openid'] != ''  ? 'yes' : 'no';?>">
      <dt><span class="icon"><i></i></span><span class="item">
        <h4>微信绑定</h4>
        <h6><?php echo $output['member_info']['openid'] != '' ? '已绑定' : '未绑定';?></h6>
        </span></dt>
      <dd>
        <span class="explain">进行微信验证后，可用于接收敏感操作的身份验证信息，以及订阅更优惠商品的促销信息。<?php echo $output['member_info']['openid'] != '' ? '' : '<em>限时完成<strong>微信绑定</strong>可领取<strong>105元现金券</strong>！</em>';?></span>

        <span class="handle">
          <?php if($output['member_info']['openid']){?>
          <a href="###" class="ncm-btn ncm-btn-orange jc">已绑定</a>
          <?php }else{?>
          <a href="#"  data-reveal-id="myModal" data-animation="fade" class="ncm-btn ncm-btn-acidblue bd">绑定微信</a>
          <?php }?>
        </span>
...
<!--           <div id="style1">
              <div class="dialog-wechat">
                   <img id="bangdingwxewm" src="dialog-wechat.jpg" alt="微信">
          ...
          <div id="style2">
              <div class="dialog-wechat">
                   <img id="bangdingwxewm" src="<?php echo $output['userewm'];?>" alt="微信">
                   <p><strong>扫码关注、立送￥105</strong></p>
              </div>
              <div class="dialog-welcome">
                   <p>欢迎您成为收藏天下的一员，您尚未关注官方微信，扫码有惊喜哦~ </p>
                   <p><em>时间不多，勿失良机！</em></p>
                   <a class="close-reveal-modal btn-right mtl8-2" href="<?php echo urlShop('member_security','index');?>">确认</a>
              </div>
          </div>
          <!-- 样式二 End-->
          ...
          <!-- 样式三 Start -->
          <!-- <div id="style3">
              <h1>欢迎您成为收藏天下的一员，您的账户尚未完成安全验证，现在就去验证<strong>手机、邮箱</strong>吧！完成还有奖励拿！！！</h1>
              <div class="dialog-btn">
                  <a class="btn-left" href="javascript:;">去验证</a>
                  <a class="close-reveal-modal btn-right" href="javascript:;">先逛逛</a>
              </div>
          </div> -->
          <!-- 样式三 End-->
