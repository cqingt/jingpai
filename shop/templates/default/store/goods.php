...
<div id="content" class="wrapper pr">
  ...
  <div class="ncs-detail<?php if ($output['store_info']['is_own_shop']) echo ' ownshop'; ?>">
    ...
    <!-- S 商品基本信息 -->
    <div class="ncs-goods-summary <?php if ($output['store_info']['is_own_shop']) {echo 'self-support-bj'; }?>">
      <div class="name">
        <h1><?php echo $output['goods']['goods_name']; ?></h1>
        <strong><?php echo html_entity_decode(str_replace("\n", "<br>", $output['goods']['goods_jingle']));?></strong> </div>
      <div class="ncs-meta">
        <div class="rate"> <!-- S 描述相符评分 --><p href="javascript:void(0)">商品评分：</p>
          <div class="raty" data-score="<?php echo $output['goods_evaluate_info']['star_average'];?>"></div>
          <!-- E 描述相符评分 --> 
          <p><a href="#ncGoodsRate">（<?php echo intval($output['goods_evaluate_info']['good_percent']);?>%好评）</a></p>
        </div>

          

        <!-- S 商品参考价格 -->
          <?php if(intval(C('show_goods_marketprice'))){?>
        <dl>
        ...
        <dl>
          <dt style="line-height:24px">促销信息：</dt>
      ...
      <div class="ncs-key">
		<!-- S 关联商品-->
          <div class="demo-ds">
              <span>累计销量：<strong><?php echo intval($output['goods']['goods_salenum']); ?></strong></span>
              <span>累计评价：<strong><?php echo intval($output['goods_evaluate_info']['all']);?></strong></span>
              <span>购买咨询：<strong><?php echo $output['consult_count']?></strong></span>
          </div>
      ...
      <!-- S 购买按钮 -->
        <div class="ncs-btn"><!-- S 提示已选规格及库存不足无法购买 -->
		  ...
		  <?php if ($output['goods']['promotion_type'] == 'tuijianyouhui') {?>
				...
				<a href="javascript:void(0);" class="share" title="立即分享" name="share_submit" nctype="share_submit">立即分享</a>
		  <?php }?>
      ...
      <div class="ncs-recommend">
        ...
        <div class="content">
          <ul>
            ...
            <li>
              <dl>
                <dt class="goods-name"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $goods_commend['goods_id']));?>" target="_blank" title="<?php echo $goods_commend['goods_jingle'];?>" name="goods_tuijian<?php echo $key;?>"><?php echo $goods_commend['goods_name'];?><em><?php echo $goods_commend['goods_jingle'];?></em></a></dt>
                <dd class="goods-pic"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $goods_commend['goods_id']));?>" target="_blank" title="<?php echo $goods_commend['goods_jingle'];?>" name="goods_tuijian<?php echo $key;?>"><img src="<?php echo thumb($goods_commend, 240);?>" alt="<?php echo $goods_commend['goods_name'];?>"/></a></dd>
