  ...
  <table class="search-form">
    <tbody>
      <tr>
        ...
        <td class="tc w70"><a href="index.php?act=store_goods_add&op=guanlian_add_goods" nctype="search_a" class="ncs-btn"><i class="icon-search"></i><?php echo $lang['nc_search'];?></a></td>
        <td class="w10"></td>
      </tr>
    </tbody>
  </table>
	...
	if(typeof(T) != 'undefined'){
		T.each(function(){
			if(typeof(A[$(this).attr('nctype')]) != 'undefined'){
				$(this).children(':last').html('<a href="JavaScript:void(0);" onclick="bundling_operate_delete($(\'#bundling_tr_'+$(this).attr('nctype')+'\'), '+$(this).attr('nctype')+')" class="ncsc-btn-mini ncsc-btn-orange"><i class="icon-ban-circle"></i>从关联商品移除</a>');
			}
		});
	}
}
...
function bundling_goods_add(o){
    ...
    $('<tr id="bundling_tr_' + _data.gid + '"></tr>')
        ...
        .append('<td class="w90"><input type="text" nctype="price" name="goods[g_' + _data.gid + '][title]" value="" class="text w70"></td>')
    ...
    $('li[nctype="' + _data.gid + '"]').children(':last').html('<a href="JavaScript:void(0);" class="ncsc-btn-mini ncsc-btn-orange" onclick="bundling_operate_delete($(\'#bundling_tr_' + _data.gid + '\'), ' + _data.gid + ')"><i class="icon-ban-circle"></i>从关联商品移除</a>');
