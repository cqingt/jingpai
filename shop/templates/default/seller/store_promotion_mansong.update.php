...
<div class="ncsc-form-default">
  ...
  <form id="add_form" action="index.php?act=store_promotion_mansong&op=mansong_update&mansong_id=<?php echo $output['mansong_info']['mansong_id']?>" method="post">
    <input type="hidden" name="form_submit" value="ok" />
    <input id="level2_flag" type="hidden" value="false" />
    <input id="level3_flag" type="hidden" value="false" />
      <?php if($output['mansong_info']['end_time'] <= TIMESTAMP){ ?>
          <font color="red"> 说明：此满即送活动已结束，只能查看，无法进行修改编辑</font>
      <?php } ?>
    <dl>
      <dt><i class="required">*</i><?php echo $lang['mansong_name'].$lang['nc_colon'];?></dt>
      <dd>
        <input id="mansong_name" name="mansong_name" type="text" maxlength="25" class="w400 text" value="<?php echo $output['mansong_info']['mansong_name']?>"/>
    ...
    <dl>
      ...
      <dd>
        <input id="start_time" name="start_time" type="text" class="text w130" value="<?php echo date('Y-m-d H:i',$output['mansong_info']['start_time']) ?>"/><em class="add-on"><i class="icon-calendar"></i></em>
        <span class="error-message"></span>
        <p class="hint"><?php echo sprintf($lang['mansong_add_start_time_explain'],date('Y-m-d 00:00',($output['mansong_info']['start_time'] > TIMESTAMP)?TIMESTAMP:$output['mansong_info']['start_time']));?></p>
      </dd>
    </dl>
    <dl>
      <dt><i class="required">*</i><?php echo $lang['end_time'].$lang['nc_colon'];?></dt>
      <dd>
      <input id="end_time" name="end_time" type="text" class="text w130" value="<?php echo date('Y-m-d H:i',$output['mansong_info']['end_time']) ?>"/><em class="add-on"><i class="icon-calendar"></i></em>
        <span class="error-message"></span>
        <p class="hint">
...
<?php } ?>
      ...
      <?php if ($output['isOwnShop']) { ?>
          <dl>
              <dt><i class="required">*</i>活动方式：</dt>
              <dd> <span class="mr50">
      <select name="mansong_type" class="valid" id="mansong_type">
          <option value="1" <?php echo ($output['mansong_info']['state'] == 1)?'selected':'';?>>普通满即送（可多个规则叠加）</option>
          <option value="2" <?php echo ($output['mansong_info']['state'] == 2)?'selected':'';?>>每满N元送（只能添加一个规则）</option>
      </select>
      </span> </dd>
          </dl>
      <?php } ?>
    ...
    <dl>
      ...
      <select name="cate_rule" class="valid" id="cate_rule">
          <option value="1" <?php echo ($output['mansong_info']['cate_rule'] == 1)?'selected':'';?>>全部店铺分类商品参加</option>
          <option value="2" <?php echo ($output['mansong_info']['cate_rule'] == 2)?'selected':'';?>>按店铺分类选择参加</option>
          <option value="3" <?php echo ($output['mansong_info']['cate_rule'] == 3)?'selected':'';?>>按商品选择参加</option>
      </select>
      </span> </dd>
    </dl>
      <dl id="category_tree" style="display: none">
          ...
          <dd>
              <table class="ncsc-default-table mb15">
                  ...
                  <tbody nctype="bundling_data1"  class="bd-line tip">
                  <?php if(!empty($output['store_cate_arr'])){?>
                      <?php foreach($output['store_cate_arr'] as $val){?>
                              <tr style="display: table-row;" id="cates_tr_<?php echo $val;?>">
                                  <input type="hidden" name="store_cates[]" value="<?php echo $val;?>">
                                  <td class="tl" colspan="2">
                                      <dl class="goods-name">
                                          <dt style="width: 300px;"><?php echo $output['cates_des'][$val]['desc'];?></dt>
                                      </dl>
                                  </td>
                                  <td class="nscs-table-handle w90">
                                      <span><a href="javascript:void(0);" onclick="cates_operate_delete($('#cates_tr_<?php echo $val;?>'), '<?php echo $val;?>')" class="btn-orange"><i class="icon-ban-circle"></i><p>移除</p></a></span>
                                  </td>
                              </tr>
                      <?php }?>
                  <?php }?>
      ...
      <dl id="diff_goods">
          ...
          <dd>
              <table class="ncsc-default-table mb15">
                  ...
                  <?php if(!empty($output['b_goods_list'])){?>
                      <?php foreach($output['b_goods_list'] as $val){?>
                          <tr id="bundling_tr_<?php echo $val['goods_id']?>" style="display: table-row;">
                              <input type="hidden" nctype="goods_id" name="goods_ids[]" value="<?php echo $val['goods_id']?>">
                              <td class="w50 ">
                                  <div class="pic-thumb">
                                      <img nctype="bundling_data_img" ncname="<?php echo $val['goods_image']?>" src="<?php echo cthumb($val['goods_image'], 60, $_SESSION['store_id']);?>" onload="javascript:DrawImage(this,60,60)" width="60" height="60">
                                  </div>
                              </td>
                              <td class="tl">
                                  <dl class="goods-name">
                                      <dt style="width: 300px;"><?php echo $val['goods_name']?></dt>
                                  </dl>
                              </td>
                              <td class="nscs-table-handle w90">
                                  <span>
                                      <a href="javascript:void(0);" onclick="bundling_operate_delete($('#bundling_tr_<?php echo $val['goods_id']?>'), '<?php echo $val['goods_id']?>')" class="btn-orange"><i class="icon-ban-circle"></i><p>移除</p></a>
                                  </span>
                              </td>
                          </tr>
              ...
              <a id="bundling_add_goods" href="index.php?act=store_promotion_mansong&op=mansong_add_goods" class="ncsc-btn ncsc-btn-acidblue">添加排除商品</a>
    ...
    <dl>
      ...
      <dd>
        <input type="hidden" id="mansong_rule_count" name="rule_count" value="<?php echo count($output['rule_list']);?>">
        <ul id="mansong_rule_list" class="ncsc-mansong-rule-list">
            <?php if(!empty($output['rule_list'])){?>
                <?php foreach($output['rule_list'] as $val){?>
                    ...
                    <?php if($val['goods_id'] > 0){ ?>
                        <span>送礼品<a href="<?php echo $val['goods_url'];?>" title="<?php echo $val['mansong_goods_name'];?>" class="goods-thumb" target="_blank">
        <img src="<?php echo $val['goods_image_url'];?>"></a>
                        <input nctype="mansong_goods_id" type="hidden" value="<?php echo $val['goods_id'];?>">
                        </span>
                    <?php } ?>

                        <input type="hidden" name="mansong_rule[]" value="<?php echo $val['price'].','.$val['discount'].','.$val['goods_id'];?>">
...
$(document).ready(function(){
    ...
    rule('<?php echo $output['mansong_info']['cate_rule'];?>');
