  ...
  <div class="adds">
    ...
    <form method="post" action="index.php?act=store_deliver&op=buyer_address&order_id=<?php echo $_GET['order_id'];?>" id="address_form" target="_parent">
      ...
      <dl>
        ...
        <dd>
<input type="text" class="text" name="new_area1" id="new_area1" value="<?php 
$encode = mb_detect_encoding($output['address_info']['reciver_info']['area'], array("ASCII",'UTF-8',"GB2312","GBK",'BIG5'));
if($encode != 'UTF-8'){
	echo iconv("GB2312","UTF-8//IGNORE",$output['address_info']['reciver_info']['area']);
}else{
	echo $output['address_info']['reciver_info']['area'];
}
?>"/>
        </dd>
      </dl>
      <dl>
        ...
        <dd>
          <input type="text" class="text" name="new_street1" id="new_street1" value="<?php 
			$encode = mb_detect_encoding($output['address_info']['reciver_info']['street'], array("ASCII",'UTF-8',"GB2312","GBK",'BIG5'));
			if($encode != 'UTF-8'){
				echo iconv("GB2312","UTF-8//IGNORE",$output['address_info']['reciver_info']['street']);
			}else{
				echo $output['address_info']['reciver_info']['street'];
			}
			?>"/>
        </dd>
      </dl>
      ...
      <dl>
        ...
        <dd>
          <input type="text" class="text" name="new_tel_phone1" id="new_tel_phone1" value="<?php echo $output['address_info']['reciver_info']['tel_phone'];?>"/>
        </dd>
      </dl>
...
$(document).ready(function(){
	...
	$('#submit').on('click',function(){
		if ($('#address_form').valid()) {
            ...
            var reciver_tel_phone = $('#new_tel_phone1').val();
            var reciver_area = $('#new_area1').val();
            var reciver_street = $('#new_street1').val();
