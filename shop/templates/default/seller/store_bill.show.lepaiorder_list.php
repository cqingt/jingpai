<?php defined('InShopNC') or exit('Access Invalid!');?>
  <form method="get" id="formSearch">
    <table class="search-form">
      ...
      <input type="hidden" id='op' name='op' value='show_lepaiorder' />
...
<table class="ncsc-default-table">
    ...
    <tbody>
      ...
      <tr class="bd-line">
        ...
        <td><?php echo ncPriceFormat($order_info['order_amount'] * $output['lepaistore_info']['commis_rate'] /100);?></td>
        <td>
       	<a target="_blank" href="index.php?act=store_bill&op=lepaiorder_detail&order_id=<?php echo $order_info['order_id'];?>"><?php echo $lang['nc_view'];?></a>
        </td>
      </tr>
      <?php } ?>
      <?php } else { ?>
...
$(function(){
    $('#ncexport').click(function(){
    	$('input[name="op"]').val('export_lepai_order');
    	$('#formSearch').submit();
    });
    $('#ncsubmit').click(function(){
    	$('input[name="op"]').val('show_lepaiorder');
    	$('#formSearch').submit();
    });
});
</script>