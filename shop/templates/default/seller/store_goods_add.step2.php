  ...
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        <dt>商品关键词</dt>
        <dd>
          <input name="goods_keywords" type="text" class="text w400" value="<?php echo $output['goods_key']['goods_keywords']; ?>" />
          <span></span>
          <p class="hint">商品关键词使用英文逗号隔开</p>
        </dd>
      </dl>
      ...
      <dl>
        <dt>商品描述</dt>
        <dd>
          <textarea name="goods_description" class="textarea h60 w400"><?php echo $output['goods_key']['goods_description']; ?></textarea>
          <span></span>
          <p class="hint">商品卖点最长不能超过300个汉字</p>
        </dd>
      </dl>
      <dl>
      ...
      <dl>
        ...
        <!--add 修改非自营店可以显示成本价 xin 20151019-->
        <?php if($_SESSION['is_own_shop']){ ?>
            <input name="g_costprice" value="<?php echo $output['goods']['goods_costprice']; ?>" type="hidden" />
        <?php }else{ ?>
            <dl>
      ...
      <dl id="spec_change_<?php echo $k;?>" nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        <dt>
        ...
        <dd <?php if($k == '1'){?>nctype="sp_group_val"<?php }?>>
          <ul class="spec">
            ...
            <li><span nctype="input_checkbox">
              <input type="checkbox" value="<?php echo $v['sp_value_name'];?>" nc_type="<?php echo $v['sp_value_id'];?>" <?php if($k == '1'){?>class="sp_val"<?php }?> nc_type_change="<?php echo $k;?>" name="sp_val[<?php echo $k;?>][<?php echo $v['sp_value_id']?>]">
        ...
        <dd class="spec-dd">
          <table border="0" cellpadding="0" cellspacing="0" class="spec_table">
            <thead>
              <?php if(is_array($output['spec_list']) && !empty($output['spec_list'])){?>
              <?php foreach ($output['spec_list'] as $k=>$val){?>
            <th nctype="spec_name_<?php echo $k;?>" id="spec_name_<?php echo $k;?>" style="display:none;"><?php if (isset($output['goods']['spec_name'][$k])) { echo $output['goods']['spec_name'][$k];} else {echo $val['sp_name'];}?></th>
              <?php }?>
              <?php }?>
      ...
      <dl>
        ...
        <dd nc_type="no_spec">
          <p><input name="g_serial" value="<?php echo $output['goods']['goods_serial']; ?>" type="text"  class="text"  />
          </p>
          <p class="hint"><?php echo $lang['store_goods_index_goods_no_help'];?>
			<br />
			<?php if(intval($_SESSION['is_own_shop']) == 1 || intval($_SESSION['store_id']) == 22){ ?>
				<span style="color:red;">货号为空自动生成系统唯一的货号</span>
			<?php }else{ ?>
				<span style="color:red;">货号禁止以9开头否则视为违规操作</span>
			<?php } ?>
		  </p>
        </dd>
      </dl>
        ...
        <dd nc_type="no_spec">
          ...
          <table class="ncsc-default-table mb15">
            <thead>
			  ...
			  <tr>
                <th class="tl" colspan="2"><dt style="width: 300px;">当前商品</dt></th>
                <th class="w90">&nbsp;</th>
                <th class="w90"><input nctype="price" type="text" value="<?php echo $output['GetGoodsLink'][0]['title'][$output['goods_id']];?>" name="dangqian_title" class="text w70"></th>
                <th class="w90">&nbsp;</th>
              </tr>
            </thead>
            <tbody nctype="bundling_data"  class="bd-line tip" title="搭配销售的商品可上下<br/>拖移商品列可自定义显<br/>示排序；编辑、删除、<br/>排序等操作提交后生效。">
			  <tr <?php if(!empty($output['GetGoodsLink'])){?> style="display: none;" <?php }?>>
                <td colspan="20" class="norecord"><div class="no-promotion"><i class="zh"></i><span>关联商品还未选择添加商品。</span></div></td>
              </tr>
              <?php if(!empty($output['GetGoodsLink'])){?>
              <?php foreach($output['GetGoodsLink'] as $val){?>
			  <?php foreach($val['goods_list'] as $gl){?>
              <?php if(isset($gl['goods_id']) && $gl['goods_id'] != $output['goods_id']) {?>
              <tr id="bundling_tr_<?php echo $gl['goods_id']?>" class="off-shelf">
                <input type="hidden" value="<?php echo $val['id'];?>" name="goods[<?php echo $gl['goods_id'];?>][bundling_goods_id]" />
                <input type="hidden" value="<?php echo $gl['goods_id'];?>" name="goods[<?php echo $gl['goods_id'];?>][gid]" nctype="goods_id">
                <td class="w50"><div class="shelf-state"><div class="pic-thumb"><img src="<?php echo cthumb($gl['goods_image'], 60, $_SESSION['store_id']);?>" ncname="<?php echo $gl['goods_image'];?>" nctype="bundling_data_img">
                    </div></div>
                </td>
                <td class="tl"><dl class="goods-name">
                    <dt style="width: 300px;"><?php echo $gl['goods_name'];?></dt>
                  </dl></td>
                <td class="goods-price w90" nctype="bundling_data_price"><?php echo $gl['goods_price'];?></td>
                <td class="w90"><?php echo $gl['goods_store_price'];?>
                  <input nctype="price" type="text" value="<?php echo $val['title'][$gl['goods_id']];?>" name="goods[<?php echo $gl['goods_id'];?>][title]" class="text w70"></td>
                <td class="nscs-table-handle w90"><span><a onclick="bundling_operate_delete($('#bundling_tr_<?php echo $gl['goods_id']?>'), <?php echo $gl['goods_id']?>)" href="JavaScript:void(0);" class="btn-orange"><i class="icon-ban-circle"></i>
                  <p>移除</p>
                  </a></span></td>
              </tr>
              <?php }?>
          ...
          <a id="bundling_add_goods" href="index.php?act=store_goods_add&op=guanlian_add_goods" class="ncsc-btn ncsc-btn-acidblue">添加商品</a>
	...
	<?php }?>
<?php if($output['is_artist'] == '1'){?>
      <dl>
        <dt nc_type="no_spec">书画作品：</dt>
        <dd nc_type="no_spec">
          <p>


            <input style=" margin-top: 2px;" name="artist_name" id="artist_name" value="" type="text"  class="text"  />
            <a class="ncsc-btn" onclick="so_artist();">搜索</a>
			
            <select name="artist" id="artist" class="valid">
              <option value="" >请选择</option>
              <?php foreach($output['result_artist'] as $k => $v){?>

              
              <option value="<?php echo $v['A_Id'];?>" <?php if($output['goods']['artist_id'] == $v['A_Id']){echo "selected='selected'";}?> ><?php echo $v['A_Name'];?></option>
      ...
      <dl>
        <dt nc_type="no_spec">平尺数：</dt>
        <dd nc_type="no_spec">
          <em class="add-on" style="margin-top:2px;width:45px;">宽cm</em><input onblur="shPingChi();" onkeyup="value=value.replace(/[^\d]/g,'')" style="width: 50px; margin-top: 2px;" name="sh_width" id="sh_width" value="<?php echo $output['goods']['sh_width']?>" type="text"  class="text"  />&nbsp;&nbsp;X&nbsp;&nbsp;<em class="add-on" style="margin-top:2px;width:45px;">高cm</em><input onblur="shPingChi();" onkeyup="value=value.replace(/[^\d]/g,'')" style="width: 50px; margin-top: 2px;" name="sh_height" id="sh_height" value="<?php echo $output['goods']['sh_height']?>" type="text"  class="text"  />
&nbsp;&nbsp;=&nbsp;&nbsp;&nbsp;<em class="add-on" style="margin-top:2px;width:45px;">平尺</em><input style="width: 50px; margin-top: 2px;" readonly="readonly" name="sh_pingchi" id="sh_pingchi" value="<?php echo $output['goods']['sh_pingchi']?>" type="text"  class="text"  />
        </dd>
      </dl>
<?php }?>
      ...
      <!-- 商品物流信息 E -->
      <!-- add 货到付款状态 xin 20151114-->
        <h3 id="demo5" nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1 || ($output['store_info']['store_huodaofk'] != 1 && $output['store_info']['is_own_shop'] != 1)) {?>style="display:none;"<?php }?>>商品货到付款</h3>
        <dl nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1 || ($output['store_info']['store_huodaofk'] != 1 && $output['store_info']['is_own_shop'] != 1)) {?>style="display:none;"<?php }?>>
            <dt>是否支持货到付款：</dt>
            <dd>
                <ul class="ncsc-form-radio-list">
                    <li>
                        <label>
                            <input name="goods_daofu" value="1" <?php if (!empty($output['goods']) && $output['goods']['goods_daofu'] == '1') { ?>checked="checked" <?php } ?> type="radio" />
                            <?php echo $lang['nc_yes'];?></label>
                    </li>
                    <li>
                        <label>
                            <input name="goods_daofu" value="0" <?php if (empty($output['goods']) || $output['goods']['goods_daofu'] == '0') { ?>checked="checked" <?php } ?> type="radio"/>
...
var TYPE_CHANGE = new Array();
var TYPE_CHANGE_SUM = 0;
...
$(function(){
    ...
    var FOR_SUM = <?php echo $output['sign_i'];?>;

    var OBJ_FOR_SUM;

    for (var i = 1; i <= FOR_SUM; i++) {
          OBJ_FOR_SUM = $('#spec_change_'+ i +' input[type=checkbox]:checked').length;
    ...
    var v_change = $(this).attr('nc_type_change');

    if(!!v_change == true){

     var obj_change = $('#spec_change_'+ v_change +' input[type=checkbox]:checked').length;

     if(obj_change >= 1){

        TYPE_CHANGE[v_change] = 1;

      $("#spec_name_"+v_change).show();

     }else{
        TYPE_CHANGE[v_change] = 0;
     }
    ...
    for (var i = 1; i <= TYPE_CHANGE.length-1; i++) {

      if(!!TYPE_CHANGE[i] == true){
         SUM += TYPE_CHANGE[i];
      }
...
function goods_stock_set(){
    ...
    $.ajax({
        url:"index.php?act=store_goods_add&op=ajax_type_change",
        type:"POST",
        data:{"data":spec_group_checked,"sum":TYPE_CHANGE_SUM},
...
<script type="text/javascript">
  function so_artist(){
    var v = $("#artist_name").val();
    if(v != ''){
      $.ajax({
        type: "GET",
        cache: false,
        url : "index.php?act=store_goods_add&op=ajax_so_artist",
        data: 'artist_name=' + v,
  ...
  function shPingChi(){
    ...
    var h = $("#sh_height").val();
    if(w && h){
      var v = Math.floor(w * h);
      var v = w*h*0.0009;
      $('#sh_pingchi').val(v.toFixed(1));
...
function bundling_operate_delete(o, id){
	...
	$('li[nctype="'+id+'"]').children(':last').html('<a href="JavaScript:void(0);" onclick="bundling_goods_add($(this))" class="ncsc-btn-mini ncsc-btn-green"><i class="icon-plus"></i>添加关联商品</a>');
