...
$(function(){
	...
	jQuery.validator.addMethod("brand_name_exist", function(value, element, params) { 
        var result = true;
        $.ajax({  
            type:"GET",  
            url:'<?php echo urlShop('store_brand', 'brand_name');?>',  
            async:false,  
            data:{brand_name: $('#brand_name').val()},  
            success: function(data){  
                if(data == 'true') {
                    $.validator.messages.brand_name_exist = '<i class="icon-exclamation-sign"></i>该品牌已存在，如果您拥有该品牌，请联系运营进行申诉';
                    result = false;
                }
            }  
        });  
