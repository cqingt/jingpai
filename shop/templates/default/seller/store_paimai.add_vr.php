...
<div class="ncsc-form-default">
  <form id="add_form" action="index.php?act=store_paimai&op=paimai_save&vr=1" method="post" enctype="multipart/form-data">
    <dl>
    ...
    <dl>
      ...
      <dd>
          ...
          <p class="hint"><?php echo '抢购开始时间不能小于'.date('Y-m-d H:i', $output['paimai_start_time']);?></p>
      </dd>
    </dl>
    <dl>
...
<?php if (!$output['isOwnShop']) { ?>
            和抢购套餐的过期时间
            <span>（<?php echo date('Y-m-d H:i', $output['current_paimai_quota']['end_time']); ?>）</span>
<?php } ?>
          </p>
      </dd>
    </dl>
    ...
    <dl>
      ...
      <div id="div_search_goods" class="div-goods-select mt10" style="display: none;">
        ...
        <p class="hint"><?php echo $lang['paimai_goods_explain'];?></p>
        </dd>
    </dl>
    <dl nctype="paimai_goods_info" style="display:none;">
      <dt><?php echo $lang['paimai_index_store_price'].$lang['nc_colon'];?></dt>
      <dd> <?php echo $lang['currency'];?><span nctype="paimai_goods_price"></span></dd>
    </dl>
    <dl>
...
$("select[name=class]").change(function(){
    ...
    $.ajax({
        type:'GET',
        url:'index.php?act=store_paimai&op=ajax_vr_class&class_id='+class_id,
...
$("select[name=city]").change(function(){
    ...
    $.ajax({
        type:'GET',
        url:'index.php?act=store_paimai&op=ajax_vr_area&area_id='+area_id,
        success:function(json){
            var html = '<option value="">'+'请选择...'+'</option>';
...
$("select[name=area]").change(function(){
    ...
    $('#btn_search_goods').on('click', function() {
        var url = "<?php echo urlShop('store_paimai', 'search_vr_goods'); ?>";
