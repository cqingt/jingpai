...
<span class="fr mr5"> <a href="index.php?act=store_order_print&order_id=<?php echo $order_id;?>" class="ncsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>打印发货单</a></span>
    ...
    <?php $i = 0;?>
    <?php if(!empty($order['goods_list']) && is_array($order['goods_list'])){ ?>
          ...
          <div class="buyer-info"> <em></em>
            <div class="con">
              ...
              <dl>
                ...
                <dd><?php echo $order['extend_order_common']['reciver_info']['mob_phone'];?></dd>
              </dl>
              <dl>
                <dt>地址<?php echo $lang['nc_colon'];?></dt>
                <dd><?php 
$encode = mb_detect_encoding($order['extend_order_common']['reciver_info']['address'], array("ASCII",'UTF-8',"GB2312","GBK",'BIG5'));
if($encode != 'UTF-8'){
	echo iconv("GB2312","UTF-8//IGNORE",$order['extend_order_common']['reciver_info']['address']);
}else{
	echo $order['extend_order_common']['reciver_info']['address'];
}
?></dd>
              </dl>
            </div>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>">
        ...
        <p><a href="javascript:void(0)" class="ncsc-btn-mini ncsc-btn-green mt10" uri="index.php?act=store_order&op=change_state&state_type=spay_price&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_width="600" dialog_title="<?php echo $lang['store_order_modify_price'];?>" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" /><i class="icon-pencil"></i>修改价格</a></p>
		<?php }?>
        <!-- 发货 -->
