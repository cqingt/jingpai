...
<?php } ?>
      ...
      <select name="mansong_type" class="valid" id="mansong_type">
          <option value="1">普通满即送（可多个规则叠加）</option>
          <option value="2">每满N元送（只能添加一个规则）</option>
      </select>
      </span> </dd>
          </dl>
      <?php } ?>
    ...
    <dl>
      ...
      <select name="cate_rule" class="valid" id="cate_rule">
          <option value="1">全部店铺分类商品参加</option>
          <option value="2">按店铺分类选择参加</option>
          <option value="3">按商品选择参加</option>
      </select>
      </span> </dd>
    </dl>
