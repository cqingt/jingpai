...
<div class="ncsc-form-default">
  <form id="add_form" action="<?php echo urlShop('store_paimai', 'paimai_quota_add_save');?>" method="post">
    <dl>
      <dt><i class="required">*</i>购买数量</dt>
      <dd>
          <input id="paimai_quota_quantity" name="paimai_quota_quantity" type="text" class="text w30" /><em class="add-on">月</em><span></span>
        <p class="hint">购买单位为月(30天)，您可以在所购买的周期内发布抢购活动</p>
        <p class="hint"><?php echo '每月(30天)您需要支付'.$output['setting_config']['paimai_price'].$lang['nc_yuan'];?> ; </p>
...
$(document).ready(function(){
    ...
    $("#add_form").validate({
    	...
    	submitHandler:function(form){
            var unit_price = <?php echo C('paimai_price');?>;
            var quantity = $("#paimai_quota_quantity").val();
