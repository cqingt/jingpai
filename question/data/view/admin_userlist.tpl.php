...
<div class="page">
    ...
    <div class="fixed-empty"></div><? if(isset($message)) { $type=isset($type)?$type:'correctmsg';  ?>    <div class="alert  alert-warning"><?=$message?></div><? } ?>    <table class="table tb-type2" id="prompt">
        <tbody>
...
<? if(is_array($userlist)) { foreach($userlist as $member) { ?>
        <tr>
            ...
            <td class="altbg2">
                <? if($member['member_email_bind']==1) { ?>                <span class="text-danger"> <?=$lang['setting_userfrom_userlist_list_authenticated']?></span>
                <? } else { ?>                <?=$lang['setting_userfrom_userlist_list_no_authentication']?>
                <? } ?>            </td>
