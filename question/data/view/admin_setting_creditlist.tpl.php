...
<div class="page">
    ...
    <div class="fixed-empty"></div><? if(isset($message)) { $type=isset($type)?$type:'correctmsg';  ?><table class="table">
    <tr>
        <td class="<?=$type?>"><?=$message?></td>
    </tr>
</table><? } ?><table class="table tb-type2" id="prompt">
    <tbody>
...
<? if(is_array($creditlist['0']['operation_msg'])) { foreach($creditlist['0']['operation_msg'] as $key => $val) { ?>
                    <option value="<?=$key?>" <? if($search['srchoperation']==$val) { ?>selected="selected"<? } ?>><?=$val?></option>
...
<? if(is_array($creditlist)) { foreach($creditlist as $member) { ?>
        <tr>
