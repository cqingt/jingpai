...
try{
    ...
    function list_by_search($title = '', $author = '', $keyword = '', $datestart = '', $dateend = '', $start = 0, $limit = 10) {
        ...
        while ($answer = $this->db->fetch_array($query)) {
            $answer['time'] = tdate($answer['time']);
            $answer['content']=checkwordsglobal( $answer['content']);
            $answer['comment_num'] = $this -> comment_num( $answer['id'] );//获取当前回答下的评论数量
            $answerlist[] = $answer;
        }
        return $answerlist;
    }
     ...
     * @return mixed
     */
    function comment_num($aid)
    {
        $sql = "SELECT count(id) as num FROM `".DB_TABLEPRE."answer_comment` WHERE aid=".$aid;
        $query = $this -> db -> query($sql);
        $answer = $this->db->fetch_array($query);
        return $answer['num'];
    }
    ...
    function comment_show($id)
    {
        ...
        $where = 'aid='.$id;
        $sql = Common::selectSql($table, $where , array('id','author','authorid','content','time'));
        $query = $this -> db -> query( $sql );
        $data = array();
        while($comment_list = $this -> db -> fetch_array($query)){
            $comment_list['time'] = tdate($comment_list['time']);
            $data[] = $comment_list;
        }
        return $data;
    }
    ...
    function comment_del($id)
    {
        $table = 'answer_comment';
        $where = 'id='.$id;
        $sql = Common::deleteSql($table , $where);
