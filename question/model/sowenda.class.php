...
require ASK2_ROOT . '/lib/shopnc.func.php';
  ...
  //exit('ok');
    ...
    function run() {
        ...
        if (method_exists($control, $method)) {
            ...
            /* 判断是否是访问后台 */
            if(substr($controlname , 0 , 5) == 'admin'){
                //限定访问后台的来源url
                if(substr($_SERVER['HTTP_REFERER'],7) == 'index.php' || substr($_SERVER['HTTP_REFERER'],7,39) == 'm/question/index.php?admin_'){
                    //防非法登录
                    $md5_key = $control -> get_md5_key();
                    define('MD5_KEY',md5($md5_key));
                    $admin = $control -> systemLogin();
                    $control->$method();
                }else{
                                       $md5_key = $control -> get_md5_key();
                    define('MD5_KEY',md5($md5_key));
