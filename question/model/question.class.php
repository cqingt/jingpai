...
class questionmodel
{
    ...
    function rownum_by_cfield_cvalue_status( $cfield = 'cid1' , $cvalue = 0 , $status = 0 ,$keywords = '')
    {
        if( $keywords == '' ){
            $condition = " 1=1 ";
        }else{
    ...
    function list_by_cfield_cvalue_status( $cfield = 'cid1' , $cvalue = 0 , $status = 0 , $start = 0 , $limit = 10 , $keywords = '' )
    {
        $questionlist = array();
        if( $keywords == '' ) {
            $sql = "SELECT * FROM " . DB_TABLEPRE . "question WHERE 1=1 ";
        }else {
            $sql = "SELECT * FROM " . DB_TABLEPRE . "question WHERE `title` like '%$keywords%' ";
        }
        ...
        while ( $question = $this->db->fetch_array($query) ) {
            $question['category_name'] = $this->base->category[$question['cid']]['name'];
            $question['time'] = tdate($question['time']);
            $question['avatar'] = get_avatar_dir($question['authorid']);
            $question['url'] = url('question/view/' . $question['id'] , $question['url']);
            $question['title'] = checkwordsglobal($question['title']);
            $question['description'] = trim(strip_tags(checkwordsglobal($question['description'])));
            $questionlist[] = $question;
        }
        return $questionlist;
    }
    ...
    function add( $title , $description , $hidanswer , $price , $cid , $cid1 = 0 , $cid2 = 0 , $cid3 = 0 , $status = 0 )
    {
        ...
        //检测内容中是否有图片  有则将图片提取出来
        preg_match('#<img src=(.*)" title=(.*)>#isU' , $content , $img_path);
        if ( !empty( $img_path ) ) {
            $img_path = substr($img_path[1] , 2 , -1);
            $this->db->query("INSERT INTO " . DB_TABLEPRE . "question_img (`qid`,`img` , `cid`) values('$qid','$img_path' , '$cid')");
        }
    ...
    {
        $this->db->query("UPDATE `" . DB_TABLEPRE . "question` SET comment='$comment' WHERE `id`=$qids");
    }
     ...
     * @return array
     */
    function getIndexRecommend($cid = 0)
    {
        $table = array(
            ['table1' => 'question_img', 'table2' => 'question', 'join' => 'qid,id', 'join_type' => 'LEFT JOIN']
        );
        $where = DB_TABLEPRE . 'question_img.cid='.$cid;
        $field = array('qid', 'title', 'img');
        $order = '`time` DESC';
        $limit = 10;
     ...
     * @return array
     */
    function getRecommendQuestion()
    {
        $table = array(
            ['table1' => 'question', 'table2'=> 'question_img', 'join' => 'id,qid', 'join_type' => 'LEFT JOIN']
        );
        $where = 'status=6';
        $field = array(DB_TABLEPRE . 'question`.`id', 'img', 'author', 'time', 'title','description');
        $order = '`time` DESC';
        $limit = 4;
        $sql = Common::selectSql($table , $where , $field , $order , $limit);
        $query = $this -> db -> query($sql);
        $data = array();
        while( $arr = $this -> db -> fetch_array($query) ){
            $arr['time'] = tdate($arr['time']);
            $arr['description'] = trim(strip_tags(checkwordsglobal($arr['description'])));
            $data[] = $arr;
        }
        $count = count($data);
        $data_supplement = array();
        if($count < 4){
            $table = array(
                ['table1' => 'question_img', 'table2'=> 'question', 'join' => 'qid,id', 'join_type' => 'LEFT JOIN']
            );
            $limit = (4 - $count);
            $sql = Common::selectSql($table , '' , $field , $order , $limit);
            $query = $this -> db -> query($sql);
            while( $arr = $this -> db -> fetch_array($query) ){
                $arr['time'] = tdate($arr['time']);
                $arr['description'] = trim(strip_tags(checkwordsglobal($arr['description'])));
        ...
        return array_merge($data , $data_supplement);
    }

    /**
     * 返回第一个有图片的问题的图片信息
     */
    function getImgForQid($type = 1 , $id = ''){
        if($id == ''){
            $where = 'status=' . $type;
        }else{
            $where = 'id IN (' .$id . ') AND status=' . $type;
        }
        $table = array(
            ['table1' => 'question_img', 'table2'=> 'question', 'join' => 'qid,id', 'join_type' => 'LEFT JOIN']
        );
        $field = array(DB_TABLEPRE . 'question`.`id' , 'img' , 'author' , 'time' , 'title' , 'description');
        $order = '`time` DESC';
        $sql = Common::selectSql($table , $where , $field , $order);
     ...
     * @return array
     */
    function getHotAnswers($limit = 5)
    {
        $table = array(
            ['table1'=>'answer','table2'=>'question','join'=>'qid,id','join_type'=>'LEFT JOIN']
        );
        $field = array(DB_TABLEPRE . 'question`.`title',DB_TABLEPRE . 'question`.`id');
        $sql = Common::selectSql($table , '' , $field, 'supports DESC', $limit);
        $data = array();
        $data[0] = $this->getFirstImgQuestion(array('title','qid','img'));
        $query = $this->db->query($sql);
     ...
     * @return mixed
     */
    function getFirstImgQuestion($field , $order='answers DESC')
    {
        $table = array(
            ['table1'=>'question_img','table2'=>'question','join'=>'qid,id','join_type'=>'LEFT JOIN']
        );
        $sql = Common::selectSql($table, '', $field , $order , 1);
    ...
    function randQuestion(){
        $sql = "SELECT `id`,`title` FROM `" . DB_TABLEPRE . "question` WHERE id >= ((SELECT MAX(id) FROM " . DB_TABLEPRE . "question)-(SELECT MIN(id) FROM " . DB_TABLEPRE . "question)) * RAND() + (SELECT MIN(id) FROM " . DB_TABLEPRE . "question)  LIMIT 3";
        $query = $this->db->query($sql);
        $data = array();
        while($arr = $this->db->fetch_array($query)){
            $data[] = $arr;
