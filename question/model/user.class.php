...
class usermodel
{
    ...
    function get_by_uid( $uid , $loginstatus = 1 )
    {
        ...
        $username = $this->db->fetch_first("SELECT member_name,member_email,member_email_bind FROM " . DB_TABLEPRE_SHOP . "member WHERE member_id='$user[member_id]'");
        $user['username'] = $username['member_name'];
        $user['email'] = $username['member_email'];
        $user['active'] = $username['member_email_bind'];
        ...
        $user['usergroup'] = $this->get_usergroup($user['groupid']);
    ...
    {
        $user = $this->db->fetch_first("SELECT * FROM " . DB_TABLEPRE . "user AS a LEFT JOIN " . DB_TABLEPRE_SHOP . "member AS b ON a.member_id=b.member_id WHERE member_name='$username'");
        return $user;
    }
    ...
    {
        $userlist = array();
        $query = $this->db->query("SELECT * FROM " . DB_TABLEPRE . "user AS a LEFT JOIN " . DB_TABLEPRE_SHOP . "member AS b ON a.member_id=b.member_id ORDER BY uid DESC LIMIT $start,$limit");
     ...
     * @return array
     */
    function list_by_search_condition_username( $condition , $start = 0 , $limit = 10 )
    {
        $userlist = array();
        $query = $this->db->query('SELECT * FROM ' . DB_TABLEPRE . "user AS a LEFT JOIN ".DB_TABLEPRE_SHOP."member AS b ON a.member_id=b.member_id WHERE $condition ORDER BY a.`member_id` DESC LIMIT $start , $limit");
        while ( $user = $this->db->fetch_array($query) ) {
            $user['regtime'] = tdate($user['regtime']);
            $user['avatar'] = get_avatar_dir($user['uid']);
            $user['lastlogintime'] = tdate($user['lastlogin']);
    ...
    function list_by_credit( $type = 0 , $limit = 100 )
    {
        ...
        $sqlarray = array(
            'SELECT u.uid,u.groupid, u.username,u.gender,u.lastlogin,u.credit2,u.questions,u.answers,u.adopts,u.member_id FROM ' . DB_TABLEPRE . "user  u ORDER BY `credit2` DESC,u.answers DESC  LIMIT 0,$limit" ,
            "SELECT u.uid,u.groupid, u.username,u.gender,u.lastlogin,sum( c.credit2 ) credit2,u.questions,u.answers,u.adopts,u.member_id FROM " . DB_TABLEPRE . "user u," . DB_TABLEPRE . "credit c   WHERE u.uid=c.uid AND c.time>$starttime   GROUP BY u.uid ORDER BY credit2  DESC,u.answers DESC LIMIT 0,$limit" ,
            "SELECT u.uid,u.groupid, u.username,u.member_id,u.gender,u.lastlogin,sum( c.credit2 ) credit2,u.questions,u.answers,u.adopts  FROM " . DB_TABLEPRE . "user u," . DB_TABLEPRE . "credit c   WHERE u.uid=c.uid AND c.time>$starttime   GROUP BY u.uid ORDER BY credit2  DESC,u.answers DESC LIMIT 0,$limit"
        );
        $query = $this->db->query($sqlarray[$type]);
        while ( $user = $this->db->fetch_array($query) ) {
            $user['username'] = $this -> getmembername($user['member_id']);
    ...
    {
        $this->db->query("UPDATE " . DB_TABLEPRE . "user SET `activecode`='$activecode', WHERE `uid`=$uid");
        $member_id = $this->db->fetch_first('SELECT member_id FROM ' . DB_TABLEPRE . 'user WHERE `uid`='.$uid);
        $member_id = $member_id['member_id'];
        $this->db->query("UPDATE " . DB_TABLEPRE_SHOP . "member SET `member_email`='$email' , `member_email_bind`=0 WHERE `member_id`=$member_id");
    }
    ...
    function update_useractive( $uid )
    {
        $member_id = $this->db->fetch_first('SELECT member_id FROM ' . DB_TABLEPRE . 'user WHERE `uid`='.$uid);
        $member_id = $member_id['member_id'];
        $this->db->query("UPDATE " . DB_TABLEPRE_SHOP . "member SET `member_email_bind`=1 WHERE `member_id`=$member_id");
    }

    /* 后台更新用户信息 */

    function update_user( $uid , $groupid , $credit1 , $credit2 ,  $gender , $signature , $chuli = 0 )
    {
        $this->db->query("UPDATE " . DB_TABLEPRE . "user SET `groupid`='$groupid',`credit1`=$credit1,`credit2`=$credit2,gender='$gender',`signature`='$signature'   WHERE `uid`=$uid");
    }
    ...
    {
        $sql = Common::selectSql( 'usergroup' , 'groupid='.$groupid  , array('grouptitle' , 'grouptype') );
        $re = $this -> db -> fetch_first($sql);
        if($re['grouptype'] == 2){
            return $re['grouptitle'];
        }
    }
     ...
     * @return mixed
     */
    function get_search_num($where)
    {
        $query = $this->db->query("SELECT COUNT(uid) AS num FROM " . DB_TABLEPRE . "user AS a LEFT JOIN " . DB_TABLEPRE_SHOP ."member AS b ON a.member_id=b.member_id WHERE ".$where);
        $re = $this->db->fetch_array($query);
        return $re['num'];
    }
     ...
     * @return array
     */
    function getRecommendUserList( $limit = 8 )
    {
        $sql = "SELECT `member_name`,`uid`,`answers`,`supports`,`groupid`,`credit2`,`followers` FROM " . DB_TABLEPRE . "user AS a LEFT JOIN " . DB_TABLEPRE_SHOP . "member AS b ON a.member_id=b.member_id ORDER BY `followers` DESC LIMIT " . $limit;
        $query = $this->db->query($sql);
        $data = array();
        while ( $recommend_list = $this->db->fetch_array($query) ) {
            $recommend_list['group_name'] = $this->base->usergroup[$recommend_list['groupid']]['grouptitle'];
            $recommend_list['is_login'] = $this->is_login($recommend_list['uid']);
            $data[] = $recommend_list;
        }
     ...
     * @return int
     */
    function getUserListCount( $where )
    {
        if ( $where != '' ) {
            $table = array(
                [ 'table1' => 'user_category' , 'table2' => 'category' , 'join' => 'cid,id' , 'join_type' => 'LEFT JOIN' ]
            );
            $field = array( 'uid' );
            $sql = Common::selectSql($table , $where , $field);
            $query = $this->db->query($sql);
            $id = array();
            ...
            $sql = "SELECT count(uid) FROM " . DB_TABLEPRE . "user";
            $query = $this->db->query($sql);
     ...
     * @return array
     */
    function getUserList( $where , $order , $startindex , $pagesize )
    {
        $limit = $startindex . ',' . $pagesize;
        if ( $where != '' ) {
            $id = $this->getCategroySearchUser($where , $limit);
            $sql = "SELECT `uid`,`member_name`,`followers`,`answers` ,`signature` FROM " . DB_TABLEPRE . "user AS a LEFT JOIN " . DB_TABLEPRE_SHOP . "member AS b ON a.member_id=b.member_id WHERE uid IN ( $id ) ORDER BY  $order";
        } else {
            $sql = "SELECT `uid`,`member_name`,`followers`,`answers` ,`signature` FROM " . DB_TABLEPRE . "user AS a LEFT JOIN " . DB_TABLEPRE_SHOP . "member AS b ON a.member_id=b.member_id  ORDER BY  $order LIMIT $limit";
        }
        $query = $this->db->query($sql);
        $data = array();
        $uid = '';
        while ( $list = $this->db->fetch_array($query) ) {
            $list['avatar'] = get_avatar_dir($list['uid']);
            $data[] = $list;
            $uid .= $list['uid'] . ',';
        }
        $uid = substr($uid , 0 , -1);
        $category_name = $this->getUserCategory($uid);
        $i = 0;
        foreach ( $data as $key => $val ) {
            if ( $val['uid'] == $category_name[$i]['uid'] ) {
                $data[$key]['category_name'] = $category_name[$i]['categoryname'];
                $i++;
            } else {
     ...
     * @return string
     */
    function getCategroySearchUser( $where , $limit )
    {
        $table = array(
            [ 'table1' => 'user_category' , 'table2' => 'category' , 'join' => 'cid,id' , 'join_type' => 'LEFT JOIN' ]
        );
        $field = array( 'uid' );
        $sql = Common::selectSql($table , $where , $field , '' , $limit);
        $query = $this->db->query($sql);
        $id = '';
        while ( $uid = $this->db->fetch_array($query) ) {
            $id .= $uid['uid'] . ',';
    ...
    {
        $sql = "SELECT uid , GROUP_CONCAT(`name`) AS categoryname FROM " . DB_TABLEPRE . "user_category AS a LEFT JOIN " . DB_TABLEPRE . "category AS b ON a.cid=b.id WHERE uid IN($uid) GROUP BY `uid` ORDER BY FIELD(uid,$uid)";
        $query = $this->db->query($sql);
        $data = array();
        while ( $groupname = $this->db->fetch_array($query) ) {
            $data[] = $groupname;
        }
        return $data;
    }
