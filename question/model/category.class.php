...
/* 分类浏览页面显示子分类 */
    ...
    function list_by_cid_pid($cid, $pid) {
        $sublist = array();
        $query = $this->db->query("select id,name,questions,grade,alias from " . DB_TABLEPRE . "category where pid=$cid order by displayorder asc,id asc");
    ...
    function getIndexList(){
        $query = $this -> db -> query("SELECT `id`,`name` FROM " . DB_TABLEPRE . "category WHERE `grade`=1 ORDER BY `displayorder`");
        $data = array();
