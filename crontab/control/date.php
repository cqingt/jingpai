...
//         $this->_add_points();
        ...
        //乐拍订单自动完成
        $this->_lepai_order_auto_complete();
    ...
    private function _order_auto_complete() {
        ...
        */
        ...
        for ($i = 0; $i < 500; $i++){
            ...
            //新版查询，去掉CRM订单，只对第三方商家订单进行处理
            $order_list = $model_order->query("SELECT o.* FROM `shop_order`as o left join `shop_yw_info` as yw on o.order_id = yw.orderid WHERE yw.orderid is null and  o.order_state='".ORDER_STATE_SEND."' and o.lock_state='0' and o.delay_time < '".$dotime."'");
            if (empty($order_list)) break;
            foreach ($order_list as $order_info) {
     ...
     *  乐拍订单自动完成
     */
    private function _lepai_order_auto_complete(){
